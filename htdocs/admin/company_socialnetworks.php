	...
	foreach ($listofnetworks as $key => $value) {
		if (!empty($value['active'])) {
			...
			if (GETPOSTISSET($key.'url') && GETPOST($key.'url', 'alpha') != '') {
				gestimag_set_const($db, $networkconstname, GETPOST($key.'url', 'alpha'), 'chaine', 0, '', $conf->entity);
				gestimag_set_const($db, $networkconstid, GETPOST($key, 'alpha'), 'chaine', 0, '', $conf->entity);
				...
				if (!empty($listofnetworks[$key]['url'])) {
					...
					gestimag_set_const($db, $networkconstname, $url, 'chaine', 0, '', $conf->entity);
				}
				gestimag_set_const($db, $networkconstid, GETPOST($key, 'alpha'), 'chaine', 0, '', $conf->entity);
			} else {
				gestimag_del_const($db, $networkconstname, $conf->entity);
				gestimag_del_const($db, $networkconstid, $conf->entity);
			}
		}
	}

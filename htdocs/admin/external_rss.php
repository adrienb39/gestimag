...
if ($action == 'add' || GETPOST("modify")) {
	...
	if (GETPOST($external_rss_urlrss, 'alpha')) {
		...
		$result1 = gestimag_set_const($db, "EXTERNAL_RSS_TITLE_".GETPOSTINT("norss"), GETPOST($external_rss_title), 'chaine', 0, '', $conf->entity);
		if ($result1) {
			$consttosave = "EXTERNAL_RSS_URLRSS_".GETPOSTINT("norss");
			$urltosave = GETPOST($external_rss_urlrss, 'alpha');
			$result2 = gestimag_set_const($db, $consttosave, $urltosave, 'chaine', 0, '', $conf->entity);
...
if (GETPOST("delete")) {
	if (GETPOSTINT("norss")) {
		...
		$result1 = gestimag_del_const($db, "EXTERNAL_RSS_TITLE_".GETPOSTINT("norss"), $conf->entity);
		if ($result1) {
			$result2 = gestimag_del_const($db, "EXTERNAL_RSS_URLRSS_".GETPOSTINT("norss"), $conf->entity);
		}
...
print '<td>http://news.google.com/news?ned=us&topic=h&output=rss<br>http://www.gestimag.org/rss</td>';
print '</tr>';
print '</table>';

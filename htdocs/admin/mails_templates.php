...
// We save list of template email Gestimag can manage. This list can found by a grep into code on "->param['models']"
$elementList = array();
...
if ($action == 'create') {
	...
	foreach ($fieldsforcontent as $tmpfieldlist) {
		...
		if ($tmpfieldlist == 'topic') {
			...
			$doleditor = new DolEditor($tmpfieldlist, (!empty($obj->$tmpfieldlist) ? $obj->$tmpfieldlist : ''), '', 400, 'gestimag_mailings', 'In', false, $acceptlocallinktomedia, $okforextended, ROWS_6, '90%');
				...
				foreach ($fieldsforcontent as $tmpfieldlist) {
					...
					if ($showfield) {
						...
						if ($tmpfieldlist == 'content') {
							...
							$doleditor = new DolEditor($tmpfieldlist.'-'.$rowid, (!empty($obj->{$tmpfieldlist}) ? $obj->{$tmpfieldlist} : ''), '', 500, 'gestimag_mailings', 'In', 0, $acceptlocallinktomedia, $okforextended, ROWS_6, '90%', ($action != 'edit' ? 1 : 0));
							print $doleditor->Create(1);
						}
						if ($tmpfieldlist == 'content_lines') {
							print '<br>'."\n";
							...
							$doleditor = new DolEditor($tmpfieldlist.'-'.$rowid, (!empty($obj->{$tmpfieldlist}) ? $obj->{$tmpfieldlist} : ''), '', 140, 'gestimag_mailings', 'In', 0, $acceptlocallinktomedia, $okforextended, ROWS_6, '90%');
							print $doleditor->Create(1);
						}
					}
				}

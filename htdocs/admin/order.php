...
if ($action == 'updateMask') {
	...
	if ($maskconstorder && preg_match('/_MASK$/', $maskconstorder)) {
		$res = gestimag_set_const($db, $maskconstorder, $maskorder, 'chaine', 0, '', $conf->entity);
	}
	...
	if ($ret > 0) {
		if (getDolGlobalString('COMMANDE_ADDON_PDF') == $value) {
			gestimag_del_const($db, 'COMMANDE_ADDON_PDF', $conf->entity);
		}
	}
} elseif ($action == 'setdoc') {
	// Set default model
	if (gestimag_set_const($db, "COMMANDE_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
	...
	gestimag_set_const($db, "COMMANDE_ADDON", $value, 'chaine', 0, '', $conf->entity);
} elseif ($action == 'set_COMMANDE_DRAFT_WATERMARK') {
	$draft = GETPOST("COMMANDE_DRAFT_WATERMARK");
	$res = gestimag_set_const($db, "COMMANDE_DRAFT_WATERMARK", trim($draft), 'chaine', 0, '', $conf->entity);
	...
	$res = gestimag_set_const($db, "ORDER_FREE_TEXT", $freetext, 'chaine', 0, '', $conf->entity);
...
/*elseif ($action == 'set_BANK_ASK_PAYMENT_BANK_DURING_ORDER') {
	...
	$res = gestimag_set_const($db, "BANK_ASK_PAYMENT_BANK_DURING_ORDER", $value, 'chaine', 0, '', $conf->entity);
	...
	$res = gestimag_set_const($db, "WAREHOUSE_ASK_WAREHOUSE_DURING_ORDER", $value, 'chaine', 0, '', $conf->entity);

...
if ($action == 'set') {
		...
		if (in_array($syslogHandler, $syslogModules)) {
			...
			foreach ($module->configure() as $option) {
				if (GETPOSTISSET($option['constant'])) {
					gestimag_del_const($db, $option['constant'], -1);
					gestimag_set_const($db, $option['constant'], trim(GETPOST($option['constant'])), 'chaine', 0, '', 0);
				}
			}
		}
	}
	...
	gestimag_del_const($db, 'SYSLOG_HANDLERS', -1); // To be sure there is not a setup into another entity
	gestimag_set_const($db, 'SYSLOG_HANDLERS', json_encode($activeModules), 'chaine', 0, '', 0);
	$error = 0;
	$errors = [];
	// Check configuration
...
if ($action == 'setlevel') {
	$level = GETPOST("level");
	$res = gestimag_set_const($db, "SYSLOG_LEVEL", $level, 'chaine', 0, '', 0);
	...
	if (!$error) {
		...
		$res = gestimag_set_const($db, "SYSLOG_FILE_SAVES", $file_saves, 'chaine', 0, '', 0);
...
$syslogfacility = $defaultsyslogfacility = gestimag_get_const($db, "SYSLOG_FACILITY", 0);
$syslogfile = $defaultsyslogfile = gestimag_get_const($db, "SYSLOG_FILE", 0);

if (!$defaultsyslogfacility) {
	$defaultsyslogfacility = 'LOG_USER';
}
if (!$defaultsyslogfile) {
	$defaultsyslogfile = 'gestimag.log';
}
$optionmc = '';

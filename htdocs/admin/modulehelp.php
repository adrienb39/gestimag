...
if ($mode == 'desc') {
	...
	if ($objMod->isCoreOrExternalModule() == 'external') {
		...
		global $gestimag_allow_download_external_modules;
		if (!empty($gestimag_allow_download_external_modules) && preg_match('/\/custom\//', $dirofmodule)) {
		...
		if ($objMod->editor_name != 'gestimag') {
		...
		if (!empty($objMod->editor_url) && !preg_match('/gestimag\.org/i', $objMod->editor_url)) {
			$textexternal .= ($objMod->editor_name != 'gestimag' ? ' - ' : '').img_picto('', 'globe').' <a href="'.$editor_url.'" target="_blank" rel="noopener noreferrer external">'.$objMod->editor_url.'</a>';
		}
		$text .= $textexternal;
	} else {

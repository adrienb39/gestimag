...
if ($action == 'update' || $action == 'add') {
	...
	if (is_array($tmparray)) {
		foreach ($tmparray as $key => $val) {
			...
			$res = gestimag_set_const($db, $constname, $constvalue, $type[$consttype], 0, $constnote, $conf->entity);
...
if ($action == 'set') {
	$result = gestimag_set_const($db, GETPOST("name", 'aZ09'), GETPOST("value"), '', 0, '', $conf->entity);
	if ($result < 0) {
		dol_print_error($db);
	}
}
...
if ($action == 'unset') {
	$result = gestimag_del_const($db, GETPOST("name", 'aZ09'), $conf->entity);
	if ($result < 0) {
		dol_print_error($db);
	}
}

...
if ($action == 'updateform') {
	...
	if ($antivircommand && !empty($gestimag_main_restrict_os_commands)) {
		...
		dol_syslog("Command are restricted to ".$gestimag_main_restrict_os_commands.". We check that one of this command is inside ".$antivircommand);
	...
	if (!$error) {
		...
		$res3 = gestimag_set_const($db, 'MAIN_UPLOAD_DOC', GETPOST('MAIN_UPLOAD_DOC', 'alpha'), 'chaine', 0, '', $conf->entity);
		$res4 = gestimag_set_const($db, "MAIN_UMASK", $tmpumask, 'chaine', 0, '', $conf->entity);
		$res5 = gestimag_set_const($db, "MAIN_ANTIVIRUS_COMMAND", trim($antivircommand), 'chaine', 0, '', $conf->entity);
		$res6 = gestimag_set_const($db, "MAIN_ANTIVIRUS_PARAM", trim($antivirparam), 'chaine', 0, '', $conf->entity);

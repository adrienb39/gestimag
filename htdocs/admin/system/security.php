...
print '<span class="opacitymedium">'.$langs->trans("YouMayFindSecurityAdviceHere", 'hhttps://wiki.gestimag.org/index.php/Security_information').'</span>';
print ' &nbsp; &nbsp; ';
...
$perms = fileperms($gestimag_main_document_root.'/'.$conffile);
if ($perms) {
...
print '<strong>'.$langs->trans("GestimagSetup").'</strong>: ';
...
if (file_exists($installlock)) {	// If install not locked, no need to show this.
	if (file_exists($upgradeunlock)) {
		print '<strong>'.$langs->trans("GestimagUpgrade").'</strong>: ';
...
print '<strong>'.$langs->trans("GestimagAddonInstall").'</strong>: ';
...
print '<div class="divsection wordbreak">';
print '<strong>$gestimag_main_prod</strong>: '.($gestimag_main_prod ? $gestimag_main_prod : '0');
if (empty($gestimag_main_prod)) {
	print ' &nbsp; &nbsp; '.img_picto('', 'warning').' '.$langs->trans("IfYouAreOnAProductionSetThis", 1);
}
print '<br>';

print '<strong>$gestimag_nocsrfcheck</strong>: '.(empty($gestimag_nocsrfcheck) ? '0' : $gestimag_nocsrfcheck);
if (!empty($gestimag_nocsrfcheck)) {
...
print '<strong>$gestimag_main_restrict_ip</strong>: ';
if (empty($gestimag_main_restrict_ip)) {
	print '<span class="opacitymedium">'.$langs->trans("None").'</span>';
	//print ' <span class="opacitymedium">('.$langs->trans("RecommendedValueIs", $langs->transnoentitiesnoconv("IPsOfUsers")).')</span>';
} else {
	print $gestimag_main_restrict_ip;
}
print '<br>';

print '<strong>$gestimag_main_restrict_os_commands</strong>: ';
if (empty($gestimag_main_restrict_os_commands)) {
	print $langs->trans("None");
} else {
	print $gestimag_main_restrict_os_commands;
}
...
if (!getDolGlobalString('SECURITY_DISABLE_TEST_ON_OBFUSCATED_CONF')) {
	print '<strong>$gestimag_main_db_pass</strong>: ';
	if (!empty($gestimag_main_db_pass) && empty($gestimag_main_db_encrypted_pass)) {
...
/*
if (!empty($gestimag_main_stream_do_not_disable)) {
	print '<strong>$gestimag_main_stream_do_not_disable</strong>: ';
	if (empty($gestimag_main_stream_do_not_disable)) {
		print '<span class="opacitymedium">'.$langs->trans("Undefined").'</span>';
	} else {
		print join(', ', $gestimag_main_stream_do_not_disable);
	}
...
global $gestimag_main_db_pass, $gestimag_main_db_encrypted_pass;
...
print '<br>';
$urlexamplebase = 'https://github.com/Gestimag/gestimag/blob/develop/dev/setup/fail2ban/filter.d/';
print '<span class="fas fa-shield-alt"></span> Login or API authentication (see <a target="_blank" rel="noopener" href="'.$urlexamplebase.'web-gestimag-rulesbruteforce.conf">fail2ban example on GitHub</a>)<br>';
print '<span class="fas fa-shield-alt"></span> '.DOL_URL_ROOT.'/passwordforgotten.php (see <a target="_blank" rel="noopener" href="'.$urlexamplebase.'web-gestimag-rulespassforgotten.conf">fail2ban example on GitHub</a>)<br>';
print '<span class="fas fa-shield-alt"></span> '.DOL_URL_ROOT.'/public/* (see <a target="_blank" rel="noopener" href="'.$urlexamplebase.'web-gestimag-limitpublic.conf">fail2ban example on GitHub</a>)<br>';
print '<br>';
$urlexamplebase = 'https://github.com/Gestimag/gestimag/blob/develop/dev/setup/apache/';

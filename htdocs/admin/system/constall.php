 ...
 *		\brief      Page to show all Gestimag setup (config file and database constants)
 */
...
$configfileparameters = array(
							'gestimag_main_url_root',
							'gestimag_main_url_root_alt',
							'gestimag_main_document_root',
							'gestimag_main_document_root_alt',
							'gestimag_main_data_root',
							'separator',
							'gestimag_main_db_host',
							'gestimag_main_db_port',
							'gestimag_main_db_name',
							'gestimag_main_db_type',
							'gestimag_main_db_user',
							'gestimag_main_db_pass',
							'gestimag_main_db_character_set',
							'gestimag_main_db_collation',
							'?gestimag_main_db_prefix',
							'separator',
							'gestimag_main_authentication',
							'separator',
							'?gestimag_main_auth_ldap_login_attribute',
							'?gestimag_main_auth_ldap_host',
							'?gestimag_main_auth_ldap_port',
							'?gestimag_main_auth_ldap_version',
							'?gestimag_main_auth_ldap_dn',
							'?gestimag_main_auth_ldap_admin_login',
							'?gestimag_main_auth_ldap_admin_pass',
							'?gestimag_main_auth_ldap_debug',
							'separator',
							'?gestimag_lib_FPDF_PATH',
							'?gestimag_lib_TCPDF_PATH',
							'?gestimag_lib_FPDI_PATH',
							'?gestimag_lib_TCPDI_PATH',
							'?gestimag_lib_NUSOAP_PATH',
							'?gestimag_lib_GEOIP_PATH',
							'?gestimag_lib_ODTPHP_PATH',
							'?gestimag_lib_ODTPHP_PATHTOPCLZIP',
							'?gestimag_js_CKEDITOR',
							'?gestimag_js_JQUERY',
							'?gestimag_js_JQUERY_UI',
							'?gestimag_font_DOL_DEFAULT_TTF',
							'?gestimag_font_DOL_DEFAULT_TTF_BOLD',
							'separator',
							'?gestimag_mailing_limit_sendbyweb',
							'?gestimag_mailing_limit_sendbycli',
							'?gestimag_mailing_limit_sendbyday',
							'?gestimag_strict_mode'
						);
$configfilelib = array(
//					'separator',
					$langs->trans("URLRoot"),
...
foreach ($configfileparameters as $key) {
	...
	if ($key == 'gestimag_main_url_root_alt' && empty(${$key})) {
		$ignore = 1;
	}
	if ($key == 'gestimag_main_document_root_alt' && empty(${$key})) {
		$ignore = 1;
	}
	...
	if (empty($ignore)) {
		...
		if ($newkey == 'separator') {
			...
			// Value
			print "<td>";
			if ($newkey == 'gestimag_main_db_pass') {

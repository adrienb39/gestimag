 ...
 *  \brief      Page to check Gestimag files integrity
 */
...
print load_fiche_titre($langs->trans("FileCheckGestimag"), '', 'title_setup');
...
if (empty($error) && !empty($xml)) {
	...
	if (is_object($xml->gestimag_htdocs_dir[0])) {
		//var_dump($xml->gestimag_htdocs_dir[0]['includecustom']);exit;
		...
		print 'Error: Failed to found <b>gestimag_htdocs_dir</b> into content of XML file:<br>'.dol_escape_htmltag(dol_trunc($xmlfile, 500));
		print '</div><br>';
		$error++;
	}
	...
	// Scan scripts
	/*
	if (is_object($xml->gestimag_script_dir[0]))
	{
		$file_list = array();
		$ret = getFilesUpdated($file_list, $xml->gestimag_htdocs_dir[0], '', ???, $checksumconcat);		// Fill array $file_list
	}*/

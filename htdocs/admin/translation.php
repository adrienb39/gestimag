...
if ($action == 'setMAIN_ENABLE_OVERWRITE_TRANSLATION') {
	if (GETPOST('value')) {
		gestimag_set_const($db, 'MAIN_ENABLE_OVERWRITE_TRANSLATION', 1, 'chaine', 0, '', $conf->entity);
	} else {
		gestimag_set_const($db, 'MAIN_ENABLE_OVERWRITE_TRANSLATION', 0, 'chaine', 0, '', $conf->entity);
	}
}
...
if ($mode == 'overwrite') {
	...
	$urlwikitranslatordoc = 'https://wiki.gestimag.org/index.php/Translator_documentation';
	...
	foreach ($recordtoshow as $key => $val) {
		...
		if (!empty($newlangfileonly->tab_translate[$key])) {
			...
			if (getDolGlobalInt('MAIN_FEATURES_LEVEL')) {
				...
				//$transifexurl = 'https://www.transifex.com/gestimag-association/gestimag/translate/#'.$langcode.'/'.$transifexlangfile.'?key='.$key;
				$transifexurl = 'https://www.transifex.com/gestimag-association/gestimag/translate/#'.$langcode.'/'.$transifexlangfile.'?q=key%3A'.$key;

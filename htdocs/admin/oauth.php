...
if ($action == 'add') {		// $provider is OAUTH_XXX
	if ($provider && $provider != '-1') {
		...
		if (getDolGlobalString($constname)) {
			...
			gestimag_set_const($db, $constname, $langs->trans('ToComplete'), 'chaine', 0, '', $conf->entity);
	...
	foreach ($conf->global as $key => $val) {
		if (!empty($val) && preg_match('/^OAUTH_.+_ID$/', $key)) {
			...
			if (GETPOSTISSET($constvalue.'_ID')) {
				if (!gestimag_set_const($db, $newconstvalue.'_ID', GETPOST($constvalue.'_ID'), 'chaine', 0, '', $conf->entity)) {
					$error++;
				}
			}
			// If we reset this provider, we also remove the secret
			if (GETPOSTISSET($constvalue.'_SECRET')) {
				if (!gestimag_set_const($db, $newconstvalue.'_SECRET', GETPOST($constvalue.'_ID') ? GETPOST($constvalue.'_SECRET') : '', 'chaine', 0, '', $conf->entity)) {
					$error++;
				}
			}
			if (GETPOSTISSET($constvalue.'_URLAUTHORIZE')) {
				if (!gestimag_set_const($db, $newconstvalue.'_URLAUTHORIZE', GETPOST($constvalue.'_URLAUTHORIZE'), 'chaine', 0, '', $conf->entity)) {
					$error++;
				}
			}
			if (GETPOSTISSET($constvalue.'_TENANT')) {
				if (!gestimag_set_const($db, $constvalue.'_TENANT', GETPOST($constvalue.'_TENANT'), 'chaine', 0, '', $conf->entity)) {
					$error++;
				}
			}
			if (GETPOSTISSET($constvalue.'_SCOPE')) {
				...
				if (!gestimag_set_const($db, $newconstvalue.'_SCOPE', $scopestring, 'chaine', 0, '', $conf->entity)) {
					$error++;
				}
			} elseif ($newconstvalue !== $constvalue) {
				if (!gestimag_set_const($db, $newconstvalue.'_SCOPE', '', 'chaine', 0, '', $conf->entity)) {
					$error++;
				}
			}
			...
			if ($constvalue !== $newconstvalue) {
				gestimag_del_const($db, $constvalue.'_ID', $conf->entity);
				gestimag_del_const($db, $constvalue.'_SECRET', $conf->entity);
				gestimag_del_const($db, $constvalue.'_URLAUTHORIZE', $conf->entity);
				gestimag_del_const($db, $constvalue.'_SCOPE', $conf->entity);
				...
				if (getDolGlobalString('MAIN_MAIL_SMTPS_OAUTH_SERVICE') == $oldname) {
					if (!gestimag_set_const($db, 'MAIN_MAIL_SMTPS_OAUTH_SERVICE', strtoupper($oldprovider).'-'.$newlabel, 'chaine', 0, '', $conf->entity)) {
						$error++;
					}
				}
			}
...
if ($action == 'delete_entry') {
	...
	if (!gestimag_del_const($db, $globalkey.'_NAME', $conf->entity) || !gestimag_del_const($db, $globalkey.'_ID', $conf->entity) || !gestimag_del_const($db, $globalkey.'_SECRET', $conf->entity) ||  !gestimag_del_const($db, $globalkey.'_URLAUTHORIZE', $conf->entity) || !gestimag_del_const($db, $globalkey.'_SCOPE', $conf->entity)) {

...
if ($file && !$what) {
	//print DOL_URL_ROOT.'/gestimag_export.php';
	header("Location: ".DOL_URL_ROOT.'/admin/tools/gestimag_export.php?msg='.urlencode($langs->trans("ErrorFieldRequired", $langs->transnoentities("ExportMethod"))).(GETPOSTINT('page_y') ? '&page_y='.GETPOSTINT('page_y') : ''));
	exit;
}
...
if ($what == 'mysql') {
	...
	if (!empty($gestimag_main_restrict_os_commands)) {
		...
		dol_syslog("Command are restricted to ".$gestimag_main_restrict_os_commands.". We check that one of this command is inside ".$cmddump);
	...
	if (!$errormsg && $cmddump) {
		gestimag_set_const($db, 'SYSTEMTOOLS_MYSQLDUMP', $cmddump, 'chaine', 0, '', $conf->entity);
	}
...
if ($what == 'postgresql') {
	...
	if (!$errormsg && $cmddump) {
		gestimag_set_const($db, 'SYSTEMTOOLS_POSTGRESQLDUMP', $cmddump, 'chaine', 0, '', $conf->entity);
	}
...
if ($errormsg) {
	...
	/*else
	{
		setEventMessages($langs->trans("YouMustRunCommandFromCommandLineAfterLoginToUser",$gestimag_main_db_user,$gestimag_main_db_user), null, 'warnings');
	}*/
}
...
header("Location: gestimag_export.php".(GETPOSTINT('page_y') ? '?page_y='.GETPOSTINT('page_y') : ''));
exit();

...
if ($compression == 'zip') {
	...
	if ($export_type == 'externalmodule' && !empty($what)) {
		...
		if (empty($gestimag_allow_download_external_modules)) {
			print 'Download of external modules is not allowed by $gestimag_allow_download_external_modules in conf.php file';
			$db->close();
			exit();
		}
	}

...
/**
 *		\file 		htdocs/admin/tools/gestimag_import.php
 *		\ingroup	core
...
llxHeader('', '', $help_url, '', 0, 0, '', '', '', 'mod-admin page-tools_gestimag_import');
...
<?php
print '<span class="opacitymedium">';
print $langs->trans("RestoreDesc3", $gestimag_main_db_name).'<br><br>';
print '</span>';
?>

<?php print $langs->trans("DatabaseName").' : <b>'.$gestimag_main_db_name.'</b>'; ?><br><br>
...
if (in_array($type, array('mysql', 'mysqli'))) {
	...
	if (preg_match("/\s/", $command)) {
		...
		$param = $gestimag_main_db_name;
		$param .= " -h ".$gestimag_main_db_host;
	...
	if (!empty($gestimag_main_db_pass)) {
		...
		$paramclear .= " -p".$gestimag_main_db_pass;
	}
	...
	if (!GETPOST("showpass") && $gestimag_main_db_pass) {

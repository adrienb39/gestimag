	...
	foreach ($arrayofparameters as $constname => $val) {
		if ($val['enabled'] == 1) {
			...
			if ($val['type'] == 'textarea') {
				...
				$doleditor = new DolEditor($constname, getDolGlobalString($constname), '', 160, 'gestimag_notes', '', false, false, isModEnabled('fckeditor'), ROWS_5, '90%');
				$doleditor->Create();
			...
			if ($val['enabled'] == 1) {
				...
				if ($val['type'] == 'textarea') {
					...
					if (getDolGlobalString($constname)) {
						...
						print '<div class="select2-container-multi-gestimag" style="width: 90%;"><ul class="select2-choices-gestimag">' . implode(' ', $toprint) . '</ul></div>';
					}
				} elseif (preg_match('/thirdparty_type/', $val['type'])) {

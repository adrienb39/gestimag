...
$mode = GETPOST('mode') ? GETPOST('mode', 'alpha') : 'gestimag_notes';
...
foreach ($modules as $const => $desc) {
	if ($action == 'enable_'.strtolower($const)) {
		gestimag_set_const($db, "FCKEDITOR_ENABLE_".$const, "1", 'chaine', 0, '', $conf->entity);
		...
		if ($const == 'PRODUCTDESC' && getDolGlobalInt('PRODUIT_DESC_IN_FORM_ACCORDING_TO_DEVICE')) {
			gestimag_set_const($db, "FCKEDITOR_ENABLE_DETAILS", "1", 'chaine', 0, '', $conf->entity);
		}
	}
	if ($action == 'disable_'.strtolower($const)) {
		gestimag_set_const($db, "FCKEDITOR_ENABLE_".$const, "0", 'chaine', 0, '', $conf->entity);
	}
}
...
if (GETPOST('save', 'alpha')) {
	...
	if (!empty($fckeditor_skin)) {
		$result = gestimag_set_const($db, 'FCKEDITOR_SKIN', $fckeditor_skin, 'chaine', 0, '', $conf->entity);
		if ($result <= 0) {
			$error++;
		}
	} else {
	...
	if (!empty($fckeditor_test)) {
		$result = gestimag_set_const($db, 'FCKEDITOR_TEST', $fckeditor_test, 'chaine', 0, '', $conf->entity);
		if ($result <= 0) {
			$error++;
		}
	} else {
...
if (empty($conf->use_javascript_ajax)) {
	...
	$listofmodes = array('gestimag_mailings', 'gestimag_notes', 'gestimag_details', 'gestimag_readonly', 'Full', 'Full_inline');
	$linkstomode = '';
	...
	if ($mode != 'Full_inline') {
		...
		$readonly = ($mode == 'gestimag_readonly' ? 1 : 0);

...
if (preg_match('/set(.*)/', $action, $reg)) {
	if (!gestimag_set_const($db, $reg[1], '1', 'chaine', 0, '', $conf->entity) > 0) {
		dol_print_error($db);
	}
}

if (preg_match('/del(.*)/', $action, $reg)) {
	if (!gestimag_set_const($db, $reg[1], '0', 'chaine', 0, '', $conf->entity) > 0) {
		dol_print_error($db);
	}
}
...
if ($action == 'setvarworkflow') {	// Test on permission already done
	if (GETPOSTISSET('product_category_id')) {
		...
		$res = gestimag_set_const($db, 'TICKET_PRODUCT_CATEGORY', $param_ticket_product_category, 'chaine', 0, '', $conf->entity);
	}
}

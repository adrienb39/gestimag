...
if (GETPOSTISSET('mode')) {
	...
	if ($mode == 'common' || $mode == 'commonkanban') {
		gestimag_set_const($db, "MAIN_MODULE_SETUP_ON_LIST_BY_DEFAULT", $mode, 'chaine', 0, '', $conf->entity);
	}
} else {
...
$urlgestimagmodules = 'https://www.dolistore.com/';
...
if ($action == 'install' && $allowonlineinstall) {
	...
	if (!$original_file) {
		...
		if (!$error && !preg_match('/\.zip$/i', $original_file)) {
			$langs->load("errors");
			setEventMessages($langs->trans("ErrorFileMustBeAGestimagPackage", $original_file), null, 'errors');
			$error++;
		}
		if (!$error && !preg_match('/^(module[a-zA-Z0-9]*_|theme_|).*\-([0-9][0-9\.]*)(\s\(\d+\)\s)?\.zip$/i', $original_file)) {
			$langs->load("errors");
			setEventMessages($langs->trans("ErrorFilenameDosNotMatchGestimagPackageRules", $original_file, 'modulename-x[.y.z].zip'), null, 'errors');
			$error++;
		}
				...
				if ($modName) {
					...
					try {
						...
						if (class_exists($modName)) {
							...
							'@phan-var-force GestimagModules $objMod';
	...
	foreach ($orders as $key => $value) {
		...
		if ($objMod->isCoreOrExternalModule() == 'external'
			&& (
				...
				// Adding a cron job could be a good idea: see GestimagModules::checkForUpdate()
...
if ($mode == 'deploy') {
	...
	if ($allowonlineinstall) {
		if (!in_array('/custom', explode(',', $gestimag_main_url_root_alt))) {
		...
		if (getDolGlobalString('MAIN_MESSAGE_INSTALL_MODULES_DISABLED_CONTACT_US')) {
			...
			$message = info_admin($langs->trans("InstallModuleFromWebHasBeenDisabledByFile", $gestimagdataroot.'/installmodules.lock'), 0, 0, 'warning');
		}
		$allowfromweb = 0;
	}

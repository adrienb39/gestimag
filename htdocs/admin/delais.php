...
if ($action == 'update') {
		...
		if (isModEnabled($module)) {
			foreach ($delays as $delay) {
				if (GETPOST($delay['code']) != '') {
					gestimag_set_const($db, $delay['code'], GETPOST($delay['code']), 'chaine', 0, '', $conf->entity);
				}
			}
		}
	}

	gestimag_set_const($db, "MAIN_DISABLE_METEO", GETPOST("MAIN_DISABLE_METEO"), 'chaine', 0, '', $conf->entity);
	gestimag_set_const($db, "MAIN_USE_METEO_WITH_PERCENTAGE", GETPOST("MAIN_USE_METEO_WITH_PERCENTAGE"), 'chaine', 0, '', $conf->entity);
	...
	for ($i = 0; $i < 4; $i++) {
		if (GETPOSTISSET('MAIN_METEO'.$plus.'_LEVEL'.$i)) {
			gestimag_set_const($db, 'MAIN_METEO'.$plus.'_LEVEL'.$i, GETPOSTINT('MAIN_METEO'.$plus.'_LEVEL'.$i), 'chaine', 0, '', $conf->entity);
		}
	}

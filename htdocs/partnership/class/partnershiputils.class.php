...
class PartnershipUtils
{
	...
	/**
	 * Action executed by scheduler to check if Gestimag backlink not found on partner website. (Max number of action batch per call = $conf->global->PARTNERSHIP_MAX_WARNING_BACKLINK_PER_CALL)
	 *
	 * CAN BE A CRON TASK
	 *
	 ...
	 * @return  int                 	0 if OK, <>0 if KO (this function is used also by cron so only 0 is OK)
	 */
	public function doWarningOfPartnershipIfGestimagBacklinkNotfound($maxpercall = 0)
	{
		global $conf, $langs, $user;
		...
		dol_syslog(get_class($this)."::doWarningOfPartnershipIfGestimagBacklinkNotfound Warning of partnership");
		...
		if ($resql) {
			...
			while ($ifetchpartner < $numofexpiredmembers) {
				...
				if ($obj) {
					...
					if ($somethingdoneonpartnership >= $maxpercall) {
						dol_syslog("We reach the limit of ".$maxpercall." partnership processed, so we quit loop for this batch doWarningOfPartnershipIfGestimagBacklinkNotfound to avoid to reach email quota.", LOG_WARNING);
						break;
					}
					...
					if (empty($website)) {
						...
						$backlinkfound = $this->checkGestimagBacklink($website);
					}
	...
	/**
	 * Action to check if Gestimag backlink not found on partner website
	 *
	 * @param  	string	$website      	Partner's website URL
	 * @return  int                 	0 if KO, 1 if OK
	 */
	private function checkGestimagBacklink($website = null)
	{
		global $conf;

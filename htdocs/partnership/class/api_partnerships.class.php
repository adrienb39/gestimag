 ...
 */
class Partnerships extends GestimagApi
{
	/**
	...
	{
		if (!GestimagApiAccess::$user->hasRight('partnership', 'read')) {
		...
		if (!GestimagApi::_checkAccessToResource('partnership', $this->partnership->id, 'partnership')) {
			throw new RestException(403, 'Access to instance id='.$this->partnership->id.' of object not allowed for login '.GestimagApiAccess::$user->login);
		}
	...
	{
		if (!GestimagApiAccess::$user->hasRight('partnership', 'write')) {
			throw new RestException(403);
		}
		...
		if ($this->partnership->create(GestimagApiAccess::$user)<0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->partnership->update(GestimagApiAccess::$user, false) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('partnership', 'delete')) {
			throw new RestException(403);
		}
		$result = $this->partnership->fetch($id);
		if (!$result) {
		...
		if (!$this->partnership->delete(GestimagApiAccess::$user)) {

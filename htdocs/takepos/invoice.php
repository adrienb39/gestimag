...
if (empty($reshook)) {
	...
	if (($action == "valid" || $action == "history" || $action == 'creditnote') && $user->hasRight('takepos', 'run')) {
		...
		if (getDolGlobalInt('TAKEPOS_PRINT_INVOICE_DOC_INSTEAD_OF_RECEIPT')) {
			...
			$sectionwithinvoicelink .= ' <button id="buttonprint" type="button" onclick="GestimagTakeposPrinting('.$placeid.')">'.$langs->trans('PrintTicket').'</button>';
		} else {
...
function GestimagTakeposPrinting(id) {
	console.log("GestimagTakeposPrinting Printing invoice ticket " + id);
	$.ajax({
		type: "GET",

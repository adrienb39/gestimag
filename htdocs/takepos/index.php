...
if ($setterminal > 0) {
	...
	setcookie("takeposterminal", (string) $setterminal, (time() + (86400 * 354)), '/', '', (empty($gestimag_main_force_https) ? false : true), true); // Permanent takeposterminal var in a cookie
}
...
// Title
$title = 'TakePOS - Gestimag '.DOL_VERSION;
...
function GestimagOpenDrawer() {
	console.log("GestimagOpenDrawer call ajax url /takepos/ajax/ajax.php?action=opendrawer&token=<?php echo newToken();?>&term=<?php print urlencode(empty($_SESSION["takeposterminal"]) ? '' : $_SESSION["takeposterminal"]); ?>");
	$.ajax({
		type: "GET",
...
if (getDolGlobalString('TAKEPOS_BAR_RESTAURANT')) {
	...
	if (getDolGlobalString('TAKEPOS_BAR_RESTAURANT')) {
		if (getDolGlobalString('TAKEPOS_PRINT_METHOD') == "takeposconnector") {
			...
			$menus[$r++] = array('title' => '<span class="fa fa-receipt paddingrightonly"></span><div class="trunc">'.$langs->trans("Receipt").'</div>', 'action' => 'GestimagTakeposPrinting(placeid);');
		} else {

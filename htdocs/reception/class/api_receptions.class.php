 ...
 */
class Receptions extends GestimagApi
{
	/**
	...
	{
		if (!GestimagApiAccess::$user->hasRight('reception', 'lire')) {
		...
		if (!GestimagApi::_checkAccessToResource('reception', $this->reception->id)) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('reception', 'creer')) {
		...
		if ($this->reception->create(GestimagApiAccess::$user) < 0) {
	...
	*/
	...
	public function deleteLine($id, $lineid)
	{
		...
		$updateRes = $this->reception->deleteLine(GestimagApiAccess::$user, $lineid);
		if ($updateRes < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->reception->update(GestimagApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('reception', 'supprimer')) {
			throw new RestException(403);
		}
		$result = $this->reception->fetch($id);
		if (!$result) {
		...
		if (!$this->reception->delete(GestimagApiAccess::$user)) {
	...
	public function validate($id, $notrigger = 0)
	{
		...
		$result = $this->reception->valid(GestimagApiAccess::$user, $notrigger);
		if ($result == 0) {
	...
	public function setinvoiced($id)
	{
		...
		$result = $this->reception->classifyBilled(GestimagApiAccess::$user);
		if ($result < 0) {
	...
	*/
	...
	public function createShipmentFromOrder($orderid)
	{
		...
		$result = $this->reception->createFromOrder($order, GestimagApiAccess::$user);
		if( $result < 0) {
				throw new RestException(405, $this->reception->error);
		}
		$this->reception->fetchObjectLinked();

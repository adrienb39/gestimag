...
$authBackend = new \Sabre\DAV\Auth\Backend\BasicCallBack(
	...
	static function ($username, $password) {
		global $user, $conf;
		global $gestimag_main_authentication, $gestimag_auto_user;
		...
		if ($gestimag_main_authentication == 'forceuser') {
			...
			if ($gestimag_auto_user != $username) {
				dol_syslog("Warning: your instance is set to use the automatic forced login '".$gestimag_auto_user."' that is not the requested login. DAV usage is forbidden in this mode.");
				return false;
			}
		}
...
// Principals Backend
//$principalBackend = new \Sabre\DAVACL\PrincipalBackend\Gestimag($user,$db);
// /principals
//$nodes[] = new \Sabre\DAVACL\PrincipalCollection($principalBackend);
// CardDav & CalDav Backend
//$carddavBackend   = new \Sabre\CardDAV\Backend\Gestimag($user,$db,$langs);
//$caldavBackend    = new \Sabre\CalDAV\Backend\Gestimag($user,$db,$langs, $cdavLib);
// /addressbook

...
if ($action == 'create') {
	...
	echo load_fiche_titre($langs->trans("SupplierProposalNew"), '', 'supplier_proposal');
	...
	echo '<form name="addprop" action="'.$_SERVER["PHP_SELF"].'" method="POST">';
	echo '<input type="hidden" name="token" value="'.newToken().'">';
	echo '<input type="hidden" name="action" value="add">';
	if ($origin != 'project' && $originid) {
		echo '<input type="hidden" name="origin" value="'.$origin.'">';
		echo '<input type="hidden" name="originid" value="'.$originid.'">';
	}
	...
	if (empty($reshook)) {
		...
		// Reference
		echo '<tr><td class="titlefieldcreate fieldrequired">'.$langs->trans('Ref').'</td><td colspan="2">'.$langs->trans("Draft").'</td></tr>';

		// Third party
		echo '<tr>';
		echo '<td class="fieldrequired">'.$langs->trans('Supplier').'</td>';
		if ($socid > 0) {
			echo '<td colspan="2">';
			echo $soc->getNomUrl(1, 'supplier');
			echo '<input type="hidden" name="socid" value="'.$soc->id.'">';
			echo '</td>';
		} else {
			echo '<td colspan="2">';
			$filter = '((s.fournisseur:=:1) AND (s.status:=:1))';
			echo img_picto('', 'company', 'class="pictofixedwidth"').$form->select_company((empty($socid) ? '' : $socid), 'socid', $filter, 'SelectThirdParty', 1, 0, null, 0, 'minwidth175 maxwidth500 widthcentpercentminusxx');
			...
			echo ' <a href="'.DOL_URL_ROOT.'/societe/card.php?action=create&client=0&fournisseur=1&backtopage='.urlencode($_SERVER["PHP_SELF"].'?action=create').'"><span class="fa fa-plus-circle valignmiddle paddingleft" title="'.$langs->trans("AddThirdParty").'"></span></a>';
			echo '</td>';
		}
		echo '</tr>'."\n";
		...
		if ($soc->id > 0) {
			...
			echo '<tr><td>'.$langs->trans('Discounts').'</td><td>';
		...
		// Terms of payment
		echo '<tr><td class="nowrap">'.$langs->trans('PaymentConditionsShort').'</td><td colspan="2">';
		echo img_picto('', 'payment', 'class="pictofixedwidth"');
		echo $form->getSelectConditionsPaiements(GETPOST('cond_reglement_id') > 0 ? GETPOST('cond_reglement_id') : $cond_reglement_id, 'cond_reglement_id', -1, 1);
		echo '</td></tr>';

		// Mode of payment
		echo '<tr><td>'.$langs->trans('PaymentMode').'</td><td colspan="2">';
		echo img_picto('', 'bank', 'class="pictofixedwidth"');
		...
		if (getDolGlobalString('BANK_ASK_PAYMENT_BANK_DURING_PROPOSAL') && isModEnabled("bank")) {
			echo '<tr><td>'.$langs->trans('BankAccount').'</td><td colspan="2">';
			echo img_picto('', 'bank', 'class="pictofixedwidth"');
		...
		if (isModEnabled("shipping")) {
			echo '<tr><td>'.$langs->trans('SendingMethod').'</td><td colspan="2">';
			echo img_picto('', 'dolly', 'class="pictofixedwidth"');
			$form->selectShippingMethod(GETPOST('shipping_method_id') > 0 ? GETPOSTINT('shipping_method_id') : "", 'shipping_method_id', '', 1);
			echo '</td></tr>';
		}

		// Delivery date (or manufacturing)
		echo '<tr><td>'.$langs->trans("DeliveryDate").'</td>';
		echo '<td colspan="2">';
		echo img_picto('', 'action', 'class="pictofixedwidth"');
		...
		if (is_numeric(getDolGlobalString('DATE_LIVRAISON_WEEK_DELAY'))) {	// If value set to 0 or a num, not empty
			...
			$sday = date("d", $tmpdte);
			echo $form->selectDate($syear."-".$smonth."-".$sday, 'liv_', 0, 0, 0, "addask");
		} else {
			echo $form->selectDate($datedelivery ? $datedelivery : -1, 'liv_', 0, 0, 0, "addask", 1, 1);
		}
		echo '</td></tr>';
		...
		// Model
		echo '<tr>';
		echo '<td>'.$langs->trans("DefaultModel").'</td>';
		echo '<td colspan="2">';
		echo img_picto('', 'pdf', 'class="pictofixedwidth"');
		...
		echo $form->selectarray('model', $list, $preselected, 0, 0, 0, '', 0, 0, 0, '', '', 1);
		echo "</td></tr>";
		...
		if (isModEnabled('project')) {
			...
			echo '<tr>';
			echo '<td>'.$langs->trans("Project").'</td><td colspan="2">';
			echo img_picto('', 'project', 'class="pictofixedwidth"').$formproject->select_projects(($soc->id > 0 ? $soc->id : -1), $projectid, 'projectid', 0, 0, 1, 1, 0, 0, 0, '', 1, 0, 'maxwidth500');
			echo ' &nbsp; <a href="'.DOL_URL_ROOT.'/projet/card.php?socid='.$soc->id.'&action=create&status=1&backtopage='.urlencode($_SERVER["PHP_SELF"].'?action=create&socid='.$soc->id).'"><span class="fa fa-plus-circle valignmiddle" title="'.$langs->trans("AddProject").'"></span></a>';
		...
		if (isModEnabled("multicurrency")) {
			...
			echo $form->selectMultiCurrency($currency_code, 'multicurrency_code');
			echo '</td></tr>';
		}
		...
		if (empty($reshook)) {
			echo $object->showOptionals($extrafields, 'create', $parameters);
		}
		...
		if (!empty($origin) && !empty($originid) && is_object($objectsrc)) {
			...
			echo "\n<!-- ".$classname." info -->";
			echo "\n";
			echo '<input type="hidden" name="amount"   value="'.$objectsrc->total_ht.'">'."\n";
			echo '<input type="hidden" name="total"    value="'.$objectsrc->total_ttc.'">'."\n";
			echo '<input type="hidden" name="tva"      value="'.$objectsrc->total_tva.'">'."\n";
			echo '<input type="hidden" name="origin"   value="'.$objectsrc->element.'">';
			echo '<input type="hidden" name="originid" value="'.$objectsrc->id.'">';

			echo '<tr><td>'.$langs->trans('CommRequest').'</td><td colspan="2">'.$objectsrc->getNomUrl(1).'</td></tr>';
			echo '<tr><td>'.$langs->trans('AmountHT').'</td><td colspan="2">'.price($objectsrc->total_ht).'</td></tr>';
			echo '<tr><td>'.$langs->trans('AmountVAT').'</td><td colspan="2">'.price($objectsrc->total_tva)."</td></tr>";
			if ($mysoc->localtax1_assuj == "1" || $objectsrc->total_localtax1 != 0) { 		// Localtax1
				echo '<tr><td>'.$langs->transcountry("AmountLT1", $mysoc->country_code).'</td><td colspan="2">'.price($objectsrc->total_localtax1)."</td></tr>";
			}

			if ($mysoc->localtax2_assuj == "1" || $objectsrc->total_localtax2 != 0) { 		// Localtax2
				echo '<tr><td>'.$langs->transcountry("AmountLT2", $mysoc->country_code).'</td><td colspan="2">'.price($objectsrc->total_localtax2)."</td></tr>";
			}
			echo '<tr><td>'.$langs->trans('AmountTTC').'</td><td colspan="2">'.price($objectsrc->total_ttc)."</td></tr>";

			if (isModEnabled("multicurrency")) {
				echo '<tr><td>'.$langs->trans('MulticurrencyAmountHT').'</td><td colspan="2">'.price($objectsrc->multicurrency_total_ht).'</td></tr>';
				echo '<tr><td>'.$langs->trans('MulticurrencyAmountVAT').'</td><td colspan="2">'.price($objectsrc->multicurrency_total_tva)."</td></tr>";
				echo '<tr><td>'.$langs->trans('MulticurrencyAmountTTC').'</td><td colspan="2">'.price($objectsrc->multicurrency_total_ttc)."</td></tr>";
			}
		}
		...
		if (!getDolGlobalString('SUPPLIER_PROPOSAL_CLONE_ON_CREATE_PAGE')) {
			echo '<input type="hidden" name="createmode" value="empty">';
		}
		...
		if (getDolGlobalString('SUPPLIER_PROPOSAL_CLONE_ON_CREATE_PAGE')) {
			...
			echo '<tr>';
			echo '<td><input type="radio" name="createmode" value="copy"></td>';
			echo '<td>'.$langs->trans("CopyAskFrom").' </td>';
			echo '<td>';
			$liste_ask = array();
			$liste_ask [0] = '';
			...
			if ($resql) {
				...
				echo $form->selectarray("copie_supplier_proposal", $liste_ask, 0);
			} else {
				dol_print_error($db);
			}
			echo '</td></tr>';

			echo '<tr><td class="tdtop"><input type="radio" name="createmode" value="empty" checked></td>';
			echo '<td valign="top" colspan="2">'.$langs->trans("CreateEmptyAsk").'</td></tr>';
		}
	...
	echo $form->buttonsSaveCancel("CreateDraft");
	...
	echo dol_get_fiche_head($head, 'comm', $langs->trans('CommRequest'), -1, 'supplier_proposal');
	...
	if (empty($reshook)) {
		...
		echo '<tr><td class="titlefield">'.$langs->trans('Discounts').'</td><td>';
		...
		echo '<table class="nobordernopadding" width="100%"><tr><td>';
		echo $langs->trans('PaymentConditionsShort');
		echo '</td>';
		if ($action != 'editconditions' && $object->statut != SupplierProposal::STATUS_NOTSIGNED) {
			echo '<td class="right"><a class="editfielda" href="'.$_SERVER["PHP_SELF"].'?action=editconditions&token='.newToken().'&id='.$object->id.'">'.img_edit($langs->transnoentitiesnoconv('SetConditions'), 1).'</a></td>';
		}
		echo '</tr></table>';
		echo '</td><td class="valuefield">';
		...
		echo '<tr><td>';
		echo '<table class="nobordernopadding" width="100%"><tr><td>';
		echo $langs->trans('DeliveryDate');
		echo '</td>';
		if ($action != 'editdate_livraison' && $object->statut == SupplierProposal::STATUS_VALIDATED) {
			echo '<td class="right"><a class="editfielda" href="'.$_SERVER["PHP_SELF"].'?action=editdate_livraison&token='.newToken().'&id='.$object->id.'">'.img_edit($langs->transnoentitiesnoconv('SetDeliveryDate'), 1).'</a></td>';
		}
		echo '</tr></table>';
		echo '</td><td class="valuefield">';
		if ($action == 'editdate_livraison') {
			echo '<form name="editdate_livraison" action="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'" method="post" class="formconsumeproduce">';
			echo '<input type="hidden" name="token" value="'.newToken().'">';
			echo '<input type="hidden" name="action" value="setdate_livraison">';
			echo '<input type="hidden" name="backtopage" value="'.$backtopage.'">';
			echo $form->selectDate($object->delivery_date, 'liv_', 0, 0, 0, "editdate_livraison");
			echo '<input type="submit" class="button button-edit" value="'.$langs->trans('Modify').'">';
			echo '</form>';
		} else {
			echo dol_print_date($object->delivery_date, 'daytext');
		}
		echo '</td>';
		echo '</tr>';
		...
		echo '<td>';
		echo '<table class="nobordernopadding" width="100%"><tr><td>';
		echo $langs->trans('PaymentMode');
		echo '</td>';
		if ($action != 'editmode' && $object->statut != SupplierProposal::STATUS_NOTSIGNED) {
			echo '<td class="right"><a class="editfielda" href="'.$_SERVER["PHP_SELF"].'?action=editmode&token='.newToken().'&id='.$object->id.'">'.img_edit($langs->transnoentitiesnoconv('SetMode'), 1).'</a></td>';
		}
		echo '</tr></table>';
		echo '</td><td class="valuefield">';
		if ($action == 'editmode') {
		...
		if (isModEnabled("multicurrency")) {
			...
			echo '<td>';
			echo '<table class="nobordernopadding" width="100%"><tr><td>';
			echo $form->editfieldkey('Currency', 'multicurrency_code', '', $object, 0);
			echo '</td>';
			if ($action != 'editmulticurrencycode' && $object->statut == $object::STATUS_VALIDATED) {
				echo '<td class="right"><a class="editfielda" href="'.$_SERVER["PHP_SELF"].'?action=editmulticurrencycode&token='.newToken().'&id='.$object->id.'">'.img_edit($langs->transnoentitiesnoconv('SetMultiCurrencyCode'), 1).'</a></td>';
			}
			echo '</tr></table>';
			echo '</td><td class="valuefield">';
			...
			if ($object->multicurrency_code != $conf->currency || $object->multicurrency_tx != 1) {
				...
				echo '<td>';
				echo '<table class="nobordernopadding" width="100%"><tr><td>';
				echo $form->editfieldkey('CurrencyRate', 'multicurrency_tx', '', $object, 0);
				echo '</td>';
				if ($action != 'editmulticurrencyrate' && $object->statut == $object::STATUS_VALIDATED && $object->multicurrency_code && $object->multicurrency_code != $conf->currency) {
					echo '<td class="right"><a class="editfielda" href="'.$_SERVER["PHP_SELF"].'?action=editmulticurrencyrate&token='.newToken().'&id='.$object->id.'">'.img_edit($langs->transnoentitiesnoconv('SetMultiCurrencyCode'), 1).'</a></td>';
				}
				echo '</tr></table>';
				echo '</td><td class="valuefield">';
				if ($action == 'editmulticurrencyrate' || $action == 'actualizemulticurrencyrate') {
					...
					if ($object->statut == $object::STATUS_DRAFT && $object->multicurrency_code && $object->multicurrency_code != $conf->currency) {
						...
						echo '<a href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&action=actualizemulticurrencyrate">'.$langs->trans("ActualizeCurrency").'</a>';
						echo '</div>';
					}
				}
				echo '</td></tr>';
		...
		if ($soc->outstanding_limit) {
			// Outstanding Bill
			echo '<tr><td>';
			echo $langs->trans('OutstandingBill');
			echo '</td><td class="valuefield">';
			$arrayoutstandingbills = $soc->getOutstandingBills('supplier');
			$outstandingBills = $arrayoutstandingbills['opened'];
			echo price($soc->outstanding_limit, 0, '', 1, - 1, - 1, $conf->currency);
			echo '</td>';
			echo '</tr>';
		}*/
		...
		if (getDolGlobalString('BANK_ASK_PAYMENT_BANK_DURING_PROPOSAL') && isModEnabled("bank")) {
			// Bank Account
			echo '<tr><td>';
			echo '<table width="100%" class="nobordernopadding"><tr><td>';
			echo $langs->trans('BankAccount');
			echo '</td>';
			if ($action != 'editbankaccount' && $usercancreate) {
				echo '<td class="right"><a class="editfielda" href="'.$_SERVER["PHP_SELF"].'?action=editbankaccount&token='.newToken().'&id='.$object->id.'">'.img_edit($langs->trans('SetBankAccount'), 1).'</a></td>';
			}
			echo '</tr></table>';
			echo '</td><td class="valuefield">';
		...
		echo '<tr>';
		// Amount HT
		echo '<td class="titlefieldmiddle">' . $langs->trans('AmountHT') . '</td>';
		echo '<td class="nowrap amountcard right">' . price($object->total_ht, 0, $langs, 0, -1, -1, $conf->currency) . '</td>';
		if (isModEnabled("multicurrency") && ($object->multicurrency_code && $object->multicurrency_code != $conf->currency)) {
			// Multicurrency Amount HT
			echo '<td class="nowrap amountcard right">' . price($object->multicurrency_total_ht, 0, $langs, 0, -1, -1, $object->multicurrency_code) . '</td>';
		}
		echo '</tr>';

		echo '<tr>';
		// Amount VAT
		echo '<td class="titlefieldmiddle">' . $langs->trans('AmountVAT') . '</td>';
		echo '<td class="nowrap amountcard right">' . price($object->total_tva, 0, $langs, 0, -1, -1, $conf->currency) . '</td>';
		if (isModEnabled("multicurrency") && ($object->multicurrency_code && $object->multicurrency_code != $conf->currency)) {
			// Multicurrency Amount VAT
			echo '<td class="nowrap amountcard right">' . price($object->multicurrency_total_tva, 0, $langs, 0, -1, -1, $object->multicurrency_code) . '</td>';
		}
		echo '</tr>';
		...
		if ($mysoc->localtax1_assuj == "1" || $object->total_localtax1 != 0) {
			echo '<tr>';
			echo '<td class="titlefieldmiddle">' . $langs->transcountry("AmountLT1", $mysoc->country_code) . '</td>';
			echo '<td class="nowrap amountcard right">' . price($object->total_localtax1, 0, $langs, 0, -1, -1, $conf->currency) . '</td>';
			if (isModEnabled("multicurrency") && ($object->multicurrency_code && $object->multicurrency_code != $conf->currency)) {
				echo '<td class="nowrap amountcard right">' . price($object->total_localtax1, 0, $langs, 0, -1, -1, $object->multicurrency_code) . '</td>';
			}
			echo '</tr>';

			if ($mysoc->localtax2_assuj == "1" || $object->total_localtax2 != 0) {
				echo '<tr>';
				echo '<td>' . $langs->transcountry("AmountLT2", $mysoc->country_code) . '</td>';
				echo '<td class="nowrap amountcard right">' . price($object->total_localtax2, 0, $langs, 0, -1, -1, $conf->currency) . '</td>';
				if (isModEnabled("multicurrency") && ($object->multicurrency_code && $object->multicurrency_code != $conf->currency)) {
					echo '<td class="nowrap amountcard right">' . price($object->total_localtax2, 0, $langs, 0, -1, -1, $object->multicurrency_code) . '</td>';
				}
				echo '</tr>';
			}
		}
		...
		echo '<tr>';
		// Amount TTC
		echo '<td>' . $langs->trans('AmountTTC') . '</td>';
		echo '<td class="nowrap amountcard right">' . price($object->total_ttc, 0, $langs, 0, -1, -1, $conf->currency) . '</td>';
		if (isModEnabled("multicurrency") && ($object->multicurrency_code && $object->multicurrency_code != $conf->currency)) {
			// Multicurrency Amount TTC
			echo '<td class="nowrap amountcard right">' . price($object->multicurrency_total_ttc, 0, $langs, 0, -1, -1, $object->multicurrency_code) . '</td>';
		}
		echo '</tr>';
		...
		echo '	<form name="addproduct" id="addproduct" action="'.$_SERVER["PHP_SELF"].'?id='.$object->id.(($action != 'editline') ? '#add' : '').'" method="POST">
		...
		echo '<div class="div-table-responsive-no-min">';
		echo '<table id="tablelines" class="noborder noshadow centpercent">';
			...
			if ($action != 'statut' && $action != 'editline') {
				...
				if ($object->statut == SupplierProposal::STATUS_DRAFT && $object->total_ttc >= 0 && count($object->lines) > 0 && $usercanvalidate) {
					if (count($object->lines) > 0) {
						echo '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=validate&token='.newToken().'">'.$langs->trans('Validate').'</a></div>';
					}
					// else echo '<a class="butActionRefused classfortooltip" href="#">'.$langs->trans('Validate').'</a>';
				}

				// Edit
				if ($object->statut == SupplierProposal::STATUS_VALIDATED && $usercancreate) {
					echo '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&action=modif&token='.newToken().'">'.$langs->trans('Modify').'</a></div>';
				}
				...
				if (($object->statut == SupplierProposal::STATUS_SIGNED || $object->statut == SupplierProposal::STATUS_NOTSIGNED || $object->statut == SupplierProposal::STATUS_CLOSE) && $usercanclose) {
					echo '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&action=reopen&token='.newToken().(!getDolGlobalString('MAIN_JUMP_TAG') ? '' : '#reopen').'"';
					echo '>'.$langs->trans('ReOpen').'</a></div>';
				}
				...
				if (empty($user->socid)) {
					if ($object->statut == SupplierProposal::STATUS_VALIDATED || $object->statut == SupplierProposal::STATUS_SIGNED) {
						if ($usercansend) {
							echo '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&action=presend&token='.newToken().'&mode=init#formmailbeforetitle">'.$langs->trans('SendMail').'</a></div>';
						} else {
							echo '<div class="inline-block divButAction"><a class="butActionRefused classfortooltip" href="#">'.$langs->trans('SendMail').'</a></div>';
						}
					}
				}
				...
				if (isModEnabled("supplier_order") && $object->statut == SupplierProposal::STATUS_SIGNED) {
					if ($usercancreateorder) {
						echo '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/fourn/commande/card.php?action=create&amp;origin='.$object->element.'&amp;originid='.$object->id.'&amp;socid='.$object->socid.'&amp;token='.newToken().'">'.$langs->trans("AddSupplierOrderShort").'</a></div>';
					}
				}

				// Set accepted/refused
				if ($object->statut == SupplierProposal::STATUS_VALIDATED && $usercanclose) {
					echo '<div class="inline-block divButAction"><a class="butAction reposition" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;token='.newToken().'&amp;action=statut'.(!getDolGlobalString('MAIN_JUMP_TAG') ? '' : '#acceptedrefused').'"';
					echo '>'.$langs->trans('SetAcceptedRefused').'</a></div>';
				}

				// Close
				if ($object->statut == SupplierProposal::STATUS_SIGNED && $usercanclose) {
					echo '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;token='.newToken().'&amp;action=close'.(!getDolGlobalString('MAIN_JUMP_TAG') ? '' : '#close').'"';
					echo '>'.$langs->trans('Close').'</a></div>';
				}

				// Clone
				if ($usercancreate) {
					echo '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&amp;socid='.$object->socid.'&amp;action=clone&object='.$object->element.'&amp;token='.newToken().'">'.$langs->trans("ToClone").'</a></div>';
				}

				// Delete
				echo dolGetButtonAction($langs->trans("Delete"), '', 'delete', $_SERVER["PHP_SELF"].'?id='.$object->id.'&action=delete&token='.newToken(), 'delete', (($object->statut == SupplierProposal::STATUS_DRAFT && $usercancreate) || $usercandelete));
			}
		}
	...
	if ($action != 'presend') {
		...
		echo $formfile->showdocuments('supplier_proposal', $filename, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', 0, '', $soc->default_lang);

 ...
 */
class SupplierProposals extends GestimagApi
{
	/**
	...
	{
		if (!GestimagApiAccess::$user->hasRight('supplier_proposal', 'supprimer')) {
		...
		if (!GestimagApi::_checkAccessToResource('supplier_proposal', $this->supplier_proposal->id)) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}

		if (!$this->supplier_proposal->delete(GestimagApiAccess::$user)) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('supplier_proposal', 'lire')) {
			throw new RestException(403);
		}
	...
	{
		if (!GestimagApiAccess::$user->hasRight('supplier_proposal', 'creer')) {
		...
		if ($this->supplier_proposal->create(GestimagApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if (!empty($this->supplier_proposal->fin_validite)) {
			if ($this->supplier_proposal->set_echeance(GestimagApiAccess::$user, $this->supplier_proposal->fin_validite) < 0) {
				throw new RestException(500, $this->supplier_proposal->error);
			}
		}

		if ($this->supplier_proposal->update(GestimagApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
			throw new RestException(500, $this->supplier_proposal->error);
		}

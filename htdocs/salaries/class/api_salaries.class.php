 ...
 */
class Salaries extends GestimagApi
{
	/**
	...
	public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0)
	{
		...
		if (!GestimagApiAccess::$user->hasRight('salaries', 'read')) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('salaries', 'write')) {
			throw new RestException(403);
		}
		...
		if ($salary->create(GestimagApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($salary->update(GestimagApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('salaries', 'delete')) {
			throw new RestException(403);
		}
		$salary = new Salary($this->db);
		$result = $salary->fetch($id);
		...
		if ($salary->delete(GestimagApiAccess::$user) < 0) {
	...
	public function addPayment($id, $request_data = null)
	{
		...
		if ($paymentsalary->create(GestimagApiAccess::$user, 1) < 0) {
	...
	public function updatePayment($id, $request_data = null)
	{
		...
		if ($paymentsalary->update(GestimagApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	/*public function delete($id)
	 ...
	 if ($paymentsalary->delete(GestimagApiAccess::$user) < 0) {

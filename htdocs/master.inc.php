 ...
 * 	\ingroup	core
 *  \brief      File that defines environment for all Gestimag process (pages or scripts)
...
$conf->db->host = empty($gestimag_main_db_host) ? '' : $gestimag_main_db_host;
$conf->db->port = empty($gestimag_main_db_port) ? '' : $gestimag_main_db_port;
$conf->db->name = empty($gestimag_main_db_name) ? '' : $gestimag_main_db_name;
$conf->db->user = empty($gestimag_main_db_user) ? '' : $gestimag_main_db_user;
$conf->db->pass = empty($gestimag_main_db_pass) ? '' : $gestimag_main_db_pass;
$conf->db->type = $gestimag_main_db_type;
$conf->db->prefix = $gestimag_main_db_prefix;
...
$conf->file->main_limit_users = $gestimag_main_limit_users;
$conf->file->mailing_limit_sendbyweb = empty($gestimag_mailing_limit_sendbyweb) ? 0 : $gestimag_mailing_limit_sendbyweb;
$conf->file->mailing_limit_sendbycli = empty($gestimag_mailing_limit_sendbycli) ? 0 : $gestimag_mailing_limit_sendbycli;
$conf->file->mailing_limit_sendbyday = empty($gestimag_mailing_limit_sendbyday) ? 0 : $gestimag_mailing_limit_sendbyday;
$conf->file->main_authentication = empty($gestimag_main_authentication) ? 'gestimag' : $gestimag_main_authentication; // Identification mode
$conf->file->main_force_https = empty($gestimag_main_force_https) ? '' : $gestimag_main_force_https; // Force https
$conf->file->strict_mode = empty($gestimag_strict_mode) ? '' : $gestimag_strict_mode; // Force php strict mode (for debug)
$conf->file->instance_unique_id = empty($gestimag_main_instance_unique_id) ? (empty($gestimag_main_cookie_cryptkey) ? '' : $gestimag_main_cookie_cryptkey) : $gestimag_main_instance_unique_id; // Unique id of instance
$conf->file->dol_main_url_root = $gestimag_main_url_root;	// Define url inside the config file
$conf->file->dol_document_root = array('main' => (string) DOL_DOCUMENT_ROOT); // Define array of document root directories ('/home/htdocs')
$conf->file->dol_url_root = array('main' => (string) DOL_URL_ROOT); // Define array of url root path ('' or '/gestimag')
if (!empty($gestimag_main_document_root_alt)) {
	// gestimag_main_document_root_alt can contains several directories
	$values = preg_split('/[;,]/', $gestimag_main_document_root_alt);
	$i = 0;
	foreach ($values as $value) {
		$conf->file->dol_document_root['alt'.($i++)] = (string) $value;
	}
	$values = preg_split('/[;,]/', $gestimag_main_url_root_alt);
	$i = 0;
	foreach ($values as $value) {
...
//unset($gestimag_main_db_pass); 	// We comment this because this constant is used in some other pages

 ...
 */
class Proposals extends GestimagApi
{
	/**
	...
	private function _fetch($id, $ref = '', $ref_ext = '', $contact_list = 1)
	{
		...
		if (!GestimagApi::_checkAccessToResource('propal', $this->propal->id)) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('propal', 'creer')) {
		...
		if ($this->propal->create(GestimagApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if (!empty($this->propal->fin_validite)) {
			if ($this->propal->set_echeance(GestimagApiAccess::$user, $this->propal->fin_validite) < 0) {
				throw new RestException(500, $this->propal->error);
			}
		}

		if ($this->propal->update(GestimagApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
			throw new RestException(500, $this->propal->error);
		}
	...
	{
		if (!GestimagApiAccess::$user->hasRight('propal', 'supprimer')) {
			throw new RestException(403);
		}
		$result = $this->propal->fetch($id);
		if (!$result) {
		...
		if (!$this->propal->delete(GestimagApiAccess::$user)) {
	...
	public function settodraft($id)
	{
		...
		$result = $this->propal->setDraft(GestimagApiAccess::$user);
		if ($result == 0) {
	...
	public function validate($id, $notrigger = 0)
	{
		...
		$result = $this->propal->valid(GestimagApiAccess::$user, $notrigger);
		if ($result == 0) {
	...
	public function close($id, $status, $note_private = '', $notrigger = 0)
	{
		...
		$result = $this->propal->closeProposal(GestimagApiAccess::$user, $status, $note_private, $notrigger);
		if ($result == 0) {
	...
	public function setinvoiced($id)
	{
		...
		$result = $this->propal->classifyBilled(GestimagApiAccess::$user);
		if ($result < 0) {
			throw new RestException(500, 'Error : '.$this->propal->error);
		}

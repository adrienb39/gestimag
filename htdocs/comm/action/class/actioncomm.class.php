...
class ActionComm extends CommonObject
{
	...
	public function build_exportfile($format, $type, $cachedelay, $filename, $filters, $exportholiday = 0)
	{
		...
		// phpcs:enable
		global $conf, $langs, $gestimag_main_url_root, $mysoc;
		...
		if ($buildfile) {
			...
			if ($resql) {
				...
				while ($obj = $this->db->fetch_object($resql)) {
					...
					$event = array();
					$event['uid'] = 'gestimagagenda-'.$this->db->database_name.'-'.$obj->id."@".$_SERVER["SERVER_NAME"];
					$event['type'] = $type;
					...
					if ($eventorganization != '') {
						$link_subscription = $gestimag_main_url_root.'/public/eventorganization/attendee_new.php?id='.((int) $obj->id).'&type=global&noregistration=1';
			...
			if ($exportholiday == 1) {
				...
				if ($resql) {
					...
					while ($i < $num) {
						...
						$event['uid']          = 'gestimagholiday-'.$this->db->database_name.'-'.$obj->rowid."@".$_SERVER["SERVER_NAME"];
						$event['author']       = dolGetFirstLastname($obj->firstname, $obj->lastname);
						$event['type']         = 'event';
			...
			if ($more) {
				if (empty($title)) {
					$title = 'Gestimag actions '.$mysoc->name.' - '.$more;
				}
				$desc = $more;
				$desc .= ' ('.$mysoc->name.' - built by Gestimag)';
			} else {
				if (empty($title)) {
					$title = 'Gestimag actions '.$mysoc->name;
				}

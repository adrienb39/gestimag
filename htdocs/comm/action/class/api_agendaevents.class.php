 ...
 */
class AgendaEvents extends GestimagApi
{
	/**
	...
	{
		if (!GestimagApiAccess::$user->hasRight('agenda', 'myactions', 'read')) {
		...
		if (!GestimagApiAccess::$user->hasRight('agenda', 'allactions', 'read') && $this->actioncomm->userownerid != GestimagApiAccess::$user->id) {
			throw new RestException(403, 'Insufficient rights to read event of this owner id. Your id is '.GestimagApiAccess::$user->id);
		}

		if (!GestimagApi::_checkAccessToResource('agenda', $this->actioncomm->id, 'actioncomm', '', 'fk_soc', 'id')) {
	...
	public function index($sortfield = "t.id", $sortorder = 'ASC', $limit = 100, $page = 0, $user_ids = '', $sqlfilters = '', $properties = '')
	{
		...
		$search_sale = 0;
		if (!GestimagApiAccess::$user->hasRight('societe', 'client', 'voir') && !$socid) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('agenda', 'myactions', 'create')) {
			throw new RestException(403, "Insufficient rights to create your Agenda Event");
		}
		if (!GestimagApiAccess::$user->hasRight('agenda', 'allactions', 'create') && GestimagApiAccess::$user->id != $request_data['userownerid']) {
			throw new RestException(403, "Insufficient rights to create an Agenda Event for owner id ".$request_data['userownerid'].' Your id is '.GestimagApiAccess::$user->id);
		}
		...
		if ($this->actioncomm->create(GestimagApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if (!GestimagApi::_checkAccessToResource('actioncomm', $this->actioncomm->id, 'actioncomm', '', 'fk_soc', 'id')) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}
		foreach ($request_data as $field => $value) {
			if ($field == 'id') {
		...
		if ($this->actioncomm->update(GestimagApiAccess::$user, 1) > 0) {
			return $this->get($id);
		}
	...
	{
		if (!GestimagApiAccess::$user->hasRight('agenda', 'myactions', 'delete')) {
		...
		if (!GestimagApiAccess::$user->hasRight('agenda', 'allactions', 'delete') && GestimagApiAccess::$user->id != $this->actioncomm->userownerid) {
			throw new RestException(403, "Insufficient rights to delete an Agenda Event of owner id ".$this->actioncomm->userownerid.' Your id is '.GestimagApiAccess::$user->id);
		}
		...
		if (!$this->actioncomm->delete(GestimagApiAccess::$user)) {

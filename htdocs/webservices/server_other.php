 ...
 *       \brief      File that is entry point to call Gestimag WebServices
 */
...
dol_syslog("Call Gestimag webservices interfaces");
...
$server->configureWSDL('WebServicesGestimagOther', $ns);
...
$server->wsdl->addComplexType(
	...
	'',
	array(
		'gestimagkey' => array('name'=>'gestimagkey', 'type'=>'xsd:string'),
...
$server->register(
	...
	// Exit values
	array('result'=>'tns:result', 'gestimag'=>'xsd:string', 'os'=>'xsd:string', 'php'=>'xsd:string', 'webserver'=>'xsd:string'),
	$ns,
	$ns.'#getVersions',
	$styledoc,
	$styleuse,
...
function getVersions($authentication)
{
	...
	if (!$error) {
		...
		$objectresp['gestimag'] = version_gestimag();

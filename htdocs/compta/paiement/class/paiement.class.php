...
class Paiement extends CommonObject
{
	...
	public function create($user, $closepaidinvoices = 0, $thirdparty = null)
	{
		...
		$error = 0;
		$way = $this->getWay();	// 'gestimag' to use amount, 'customer' to use foreign multicurrency amount
		...
		if ($way == 'gestimag') {	// Payments were entered into the column of main currency
		...
		foreach ($amounts as $key => $value) {	// How payment is dispatched
			...
			// $key is id of invoice, $value is amount, $way is a 'gestimag' if amount is in main currency, 'customer' if in foreign currency
	...
	public function addPaymentToBank($user, $mode, $label, $accountid, $emetteur_nom, $emetteur_banque, $notrigger = 0, $accountancycode = '', $addbankurl = '')
	{
		...
		if (isModEnabled("bank")) {
			...
			// if gestimag currency != bank currency then we received an amount in customer currency (currently I don't manage the case : my currency is USD, the customer currency is EUR and he paid me in GBP. Seems no sense for me)
	 ...
	 *
	 * 	@return 	string 	'gestimag' if standard comportment or paid in main currency, 'customer' if payment received from multicurrency inputs
	 */

 ...
 */
class Invoices extends GestimagApi
{
	/**
	 *
	...
	private function _fetch($id, $ref = '', $ref_ext = '', $contact_list = 1)
	{
		...
		if (!GestimagApi::_checkAccessToResource('facture', $this->invoice->id)) {
	...
	public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $thirdparty_ids = '', $status = '', $sqlfilters = '', $properties = '')
	{
		...
		$socids = GestimagApiAccess::$user->socid ? GestimagApiAccess::$user->socid : $thirdparty_ids;
	...
	{
		if (!GestimagApiAccess::$user->hasRight('facture', 'creer')) {
		...
		if ($this->invoice->create(GestimagApiAccess::$user, 0, (empty($request_data["date_lim_reglement"]) ? 0 : $request_data["date_lim_reglement"])) < 0) {
	...
	public function createInvoiceFromOrder($orderid)
	{
		...
		if (!GestimagApiAccess::$user->hasRight('commande', 'lire')) {
			throw new RestException(403);
		}
		if (!GestimagApiAccess::$user->hasRight('facture', 'creer')) {
			throw new RestException(403);
		...
		$result = $this->invoice->createFromOrder($order, GestimagApiAccess::$user);
		if ($result < 0) {
	...
	*/
	public function createInvoiceFromContract($contractid)
	{
		...
		if (!GestimagApiAccess::$user->hasRight('contrat', 'lire')) {
			throw new RestException(403);
		}
		if (!GestimagApiAccess::$user->hasRight('facture', 'creer')) {
			throw new RestException(403);
		...
		$result = $this->invoice->createFromContract($contract, GestimagApiAccess::$user);
		if ($result < 0) {
			throw new RestException(405, $this->invoice->error);
		}
		$this->invoice->fetchObjectLinked();
	...
	public function postContact($id, $contactid, $type)
	{
		...
		if (!GestimagApi::_checkAccessToResource('invoice', $this->invoice->id)) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}
	...
	public function deleteLine($id, $lineid)
	{
		...
		if (!GestimagApi::_checkAccessToResource('facture', $id)) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->invoice->update(GestimagApiAccess::$user)) {
			return $this->get($id);
		}
	...
	{
		if (!GestimagApiAccess::$user->hasRight('facture', 'supprimer')) {
			throw new RestException(403);
		}
		$result = $this->invoice->fetch($id);
		if (!$result) {
		...
		$result = $this->invoice->delete(GestimagApiAccess::$user);
		if ($result < 0) {
	...
	public function settodraft($id, $idwarehouse = -1)
	{
		...
		$result = $this->invoice->setDraft(GestimagApiAccess::$user, $idwarehouse);
		if ($result == 0) {
	...
	public function validate($id, $force_number = '', $idwarehouse = 0, $notrigger = 0)
	{
		...
		$result = $this->invoice->validate(GestimagApiAccess::$user, $force_number, $idwarehouse, $notrigger);
		if ($result == 0) {
	...
	public function settopaid($id, $close_code = '', $close_note = '')
	{
		...
		$result = $this->invoice->setPaid(GestimagApiAccess::$user, $close_code, $close_note);
		if ($result == 0) {
			throw new RestException(304, 'Error nothing done. May be object is already validated');
		}
		if ($result < 0) {
	...
	public function settounpaid($id)
	{
		...
		$result = $this->invoice->setUnpaid(GestimagApiAccess::$user);
		if ($result == 0) {
			throw new RestException(304, 'Nothing done');
		}
		if ($result < 0) {
	...
	public function markAsCreditAvailable($id)
	{
		...
		if ($canconvert) {
			...
			if ($this->invoice->type == Facture::TYPE_STANDARD || $this->invoice->type == Facture::TYPE_REPLACEMENT || $this->invoice->type == Facture::TYPE_SITUATION) {
				...
				$result = $discount->create(GestimagApiAccess::$user);
				if ($result < 0) {
					$error++;
				}
			}
			...
			if (empty($error)) {
				if ($this->invoice->type != Facture::TYPE_DEPOSIT) {
					...
					$result = $this->invoice->setPaid(GestimagApiAccess::$user);
					if ($result >= 0) {
						$this->db->commit();
					} else {
						$this->db->rollback();
	...
	public function addPayment($id, $datepaye, $paymentid, $closepaidinvoices, $accountid, $num_payment = '', $comment = '', $chqemetteur = '', $chqbank = '')
	{
		...
		$payment_id = $paymentobj->create(GestimagApiAccess::$user, ($closepaidinvoices == 'yes' ? 1 : 0)); // This include closing invoices
		if ($payment_id < 0) {
			$this->db->rollback();
		...
		if (isModEnabled("bank")) {
			...
			$result = $paymentobj->addPaymentToBank(GestimagApiAccess::$user, 'payment', $label, $accountid, $chqemetteur, $chqbank);
			if ($result < 0) {
				$this->db->rollback();
	...
	public function addPaymentDistributed($arrayofamounts, $datepaye, $paymentid, $closepaidinvoices, $accountid, $num_payment = '', $comment = '', $chqemetteur = '', $chqbank = '', $ref_ext = '', $accepthigherpayment = false)
	{
		...
		foreach ($arrayofamounts as $id => $amount) {
			...
			if (!GestimagApi::_checkAccessToResource('facture', $id)) {
				throw new RestException(403, 'Access not allowed on invoice ID '.$id.' for login '.GestimagApiAccess::$user->login);
			}
		}
	...
	private function _fetchTemplateInvoice($id, $ref = '', $ref_ext = '', $contact_list = 1)
	{
		...
		if (!GestimagApi::_checkAccessToResource('facturerec', $this->template_invoice->id)) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}

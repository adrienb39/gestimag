 ...
 */
class BankAccounts extends GestimagApi
{
	/**
	...
	public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $category = 0, $sqlfilters = '', $properties = '')
	{
		...
		if (!GestimagApiAccess::$user->hasRight('banque', 'lire')) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('banque', 'configurer')) {
			throw new RestException(403);
		}
	...
	{
		if (!GestimagApiAccess::$user->hasRight('banque', 'modifier')) {
			throw new RestException(403);
		}
	...
	{
		if (!GestimagApiAccess::$user->rights->banque->modifier) {
			throw new RestException(403);
		}
	...
	public function deleteLine($id, $line_id)
	{
		...
		if ($accountLine->delete(GestimagApiAccess::$user) < 0) {

...
if (empty($reshook)) {
	...
	if ($action == 'modify') {
		for ($i = 1; $i < 9; $i++) {
			gestimag_set_const($db, GETPOST("nom".$i), GETPOST("value".$i), 'chaine', 0, '', $conf->entity);
		}
	}
	*/
	if ($action == 'create' && $permissiontocreate) {
		...
		if ($id_bankaccount != getDolGlobalInt($default_account)) {
			$res = gestimag_set_const($db, $default_account, $id_bankaccount, 'chaine', 0, '', $conf->entity); // Set as default
		}
		require_once DOL_DOCUMENT_ROOT.'/compta/bank/class/account.class.php';
		$bank = new Account($db);

...
// Use its own include to not share the include of Gestimag
// This is a standalone feature with no information from Gestimag to show
// and no database access to do.
include_once 'inc.php';

$uri = preg_replace('/^http(s?):\/\//i', '', $gestimag_main_url_root);
...
pHeader($langs->trans("GestimagHelpCenter"), $_SERVER["PHP_SELF"]);
...
// If a global var $gestimag_main_url_root exists, install is done and complete, we can show the link to home page
if (!empty($gestimag_main_url_root)) {
	print $langs->trans("ToGoBackToGestimag", $homeurl);
}
...
$urlwiki = 'https://wiki.gestimag.org';
if (preg_match('/fr/i', $langs->defaultlang)) {
	$urlwiki = 'https://wiki.gestimag.org/index.php/Accueil';
}
if (preg_match('/es/i', $langs->defaultlang)) {
	$urlwiki = 'https://wiki.gestimag.org/index.php/Portada';
}
if (preg_match('/de/i', $langs->defaultlang)) {
	$urlwiki = 'https://wiki.gestimag.org/index.php/Hauptseite';
}
print '<div class="wordbreak">';
print '<br>'.$langs->trans("ForDocumentationSeeWiki", $urlwiki, $urlwiki);
print '<br>';
$urlforum = 'https://www.gestimag.org/forum/';
$urlforumlocal = 'https://www.gestimag.org/forum/';
if (preg_match('/fr/i', $langs->defaultlang)) {
	$urlforumlocal = 'https://www.gestimag.fr/forum/';
}
if (preg_match('/es/i', $langs->defaultlang)) {
	$urlforumlocal = 'https://www.gestimag.es/foro/';
}
if (preg_match('/it/i', $langs->defaultlang)) {
	$urlforumlocal = 'http://www.gestimag.it/forum/';
}
if (preg_match('/gr/i', $langs->defaultlang)) {
	$urlforumlocal = 'https://www.gestimag.gr/forum/';
}
if (preg_match('/de/i', $langs->defaultlang)) {
	$urlforumlocal = 'https://www.gestimag.de/forum/';
}
...
print '</td>';
print '</tr><tr>';
$urlwiki = 'https://partners.gestimag.org';

 ...
 */
class Tasks extends GestimagApi
{
	/**
	...
	{
		if (!GestimagApiAccess::$user->hasRight('projet', 'lire')) {
		...
		if (!GestimagApi::_checkAccessToResource('task', $this->task->id)) {
	...
	public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $sqlfilters = '', $properties = '')
	{
		...
		$socids = GestimagApiAccess::$user->socid ? GestimagApiAccess::$user->socid : 0;
	...
	{
		if (!GestimagApiAccess::$user->hasRight('projet', 'creer')) {
		...
		if ($this->task->create(GestimagApiAccess::$user) < 0) {
	...
	public function getLines($id, $includetimespent=0)
	{
		...
		if( ! GestimagApi::_checkAccessToResource('project',$this->project->id)) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}
		$this->project->getLinesArray(GestimagApiAccess::$user);
		$result = array();
	...
	*/
	...
	public function getRoles($id, $userid = 0)
	{
		...
		if (!GestimagApi::_checkAccessToResource('tasks', $this->task->id)) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}

		$usert = GestimagApiAccess::$user;
		if ($userid > 0) {
	...
	*/
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->task->update(GestimagApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('projet', 'supprimer')) {
			throw new RestException(403);
		}
		$result = $this->task->fetch($id);
		if (!$result) {
		...
		if (!$this->task->delete(GestimagApiAccess::$user)) {
	...
	public function addTimeSpent($id, $date, $duration, $user_id = 0, $note = '')
	{
		...
		if (!GestimagApi::_checkAccessToResource('project', $this->task->fk_project)) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}

		$uid = $user_id;
		if (empty($uid)) {
			$uid = GestimagApiAccess::$user->id;
		}
		...
		$result = $this->task->addTimeSpent(GestimagApiAccess::$user, 0);
		if ($result == 0) {
	...
	public function putTimeSpent($id, $timespent_id, $date, $duration, $user_id = 0, $note = '')
	{
		...
		$this->task->timespent_withhour = 1;
		$this->task->timespent_duration = $duration;
		$this->task->timespent_fk_user  = $user_id ?? GestimagApiAccess::$user->id;
		$this->task->timespent_note     = $note;

		$result = $this->task->updateTimeSpent(GestimagApiAccess::$user, 0);
		if ($result == 0) {
	...
	public function deleteTimeSpent($id, $timespent_id)
	{
		...
		if ($this->task->delTimeSpent(GestimagApiAccess::$user, 0) < 0) {

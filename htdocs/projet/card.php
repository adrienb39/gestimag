...
if (empty($reshook)) {
	...
	if ($action == 'confirm_clone' && $permissiontoadd && $confirm == 'yes') {
		...
		if ($result <= 0) {
			...
			setEventMessages($langs->trans("ProjectCreatedInGestimag", $newobject->ref), "", 'mesgs');
...
if ($action == 'create' && $user->hasRight('projet', 'creer')) {
	...
	print '<td>';
	$doleditor = new DolEditor('description', GETPOST("description", 'restricthtml'), '', 90, 'gestimag_notes', '', false, true, isModEnabled('fckeditor') && getDolGlobalString('FCKEDITOR_ENABLE_SOCIETE'), ROWS_3, '90%');
	$doleditor->Create();
	print '</td></tr>';
	...
	if ($action == 'edit' && $userWrite > 0) {
		...
		print '<tr><td class="tdtop">'.$langs->trans("Description").'</td>';
		print '<td>';
		$doleditor = new DolEditor('description', $object->description, '', 90, 'gestimag_notes', '', false, true, isModEnabled('fckeditor') && getDolGlobalInt('FCKEDITOR_ENABLE_SOCIETE'), ROWS_3, '90%');
		$doleditor->Create();
		print '</td></tr>';

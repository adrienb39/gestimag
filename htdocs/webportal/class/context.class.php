...
class Context
{
	...
	public function getThirdPartyAccountFromLogin($login, $pass)
	{
		...
		$sql .= " AND sa.site = 'gestimag_portal'";
		$sql .= " AND sa.status = 1";
		$sql .= " AND sa.entity IN (" . getEntity('societe') . ")";

		dol_syslog(__METHOD__ . ' Try to find the third-party account id for login"' . $login . '" and site="gestimag_portal"', LOG_DEBUG);
		...
		if ($result) {
			if ($this->db->num_rows($result) == 1) {
				...
				dol_syslog(__METHOD__ . ' Many third-party account found for login"' . $login . '" and site="gestimag_portal"', LOG_ERR);
				return -2;
			}
		} else {

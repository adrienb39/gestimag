...
class Stripe extends CommonObject
{
	 ...
	 * @param	int		$did								ID of an existing line into llx_prelevement_demande (Gestimag intent). If provided, no new line will be created.
	...
	public function getPaymentIntent($amount, $currency_code, $tag, $description = '', $object = null, $customer = null, $key = null, $status = 0, $usethirdpartyemailforreceiptemail = 0, $mode = 'automatic', $confirmnow = false, $payment_method = null, $off_session = 0, $noidempotency_key = 1, $did = 0)
	{
		...
		if (empty($paymentintent)) {
			...
			$dataforintent = array(
				...
				/*
				'return_url' => $gestimag_main_url_root.'/public/payment/paymentok.php',
			...
			if ($num) {
				...
				if ($cardref) {
					...
					try {
						...
						dol_syslog($this->error, LOG_WARNING);	// Error from Stripe, so a warning on Gestimag
					}
				}
			}
		} else {

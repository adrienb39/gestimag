...
class MultiCurrency extends CommonObject
{
	 ...
	 */
	public function addRateFromGestimag($code, $rate)
	{
		global $user;
	 ...
	 * @param	string			$way					'gestimag' mean the amount is in gestimag currency
	 ...
	 */
	public static function getAmountConversionFromInvoiceRate($fk_facture, $amount, $way = 'gestimag', $table = 'facture', $invoice_rate = null)
	{
				...
				if ($this->recalculRates($TRate) >= 0) {
					foreach ($TRate as $currency_code => $rate) {
						...
						if ($obj->fetch(0, $code) > 0) {
							...
							$this->addRateFromGestimag($code, $rate);
						}
					}
				}

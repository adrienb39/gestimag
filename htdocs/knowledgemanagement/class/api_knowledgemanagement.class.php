 ...
 * Copyright (C) 2021 SuperAdmin <test@gestimag.com>
 *
 ...
 */
class KnowledgeManagement extends GestimagApi
{
	/**
	...
	{
		if (!GestimagApiAccess::$user->hasRight('knowledgemanagement', 'knowledgerecord', 'read')) {
		...
		if (!GestimagApi::_checkAccessToResource('knowledgerecord', $this->knowledgerecord->id, 'knowledgemanagement_knowledgerecord')) {
			throw new RestException(403, 'Access to instance id='.$this->knowledgerecord->id.' of object not allowed for login '.GestimagApiAccess::$user->login);
		}
	...
	{
		if (!GestimagApiAccess::$user->hasRight('knowledgemanagement', 'knowledgerecord', 'write')) {
			throw new RestException(403);
		}
		...
		if ($this->knowledgerecord->create(GestimagApiAccess::$user)<0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->knowledgerecord->update(GestimagApiAccess::$user, false) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('knowledgemanagement', 'knowledgerecord', 'delete')) {
			throw new RestException(403);
		}
		$result = $this->knowledgerecord->fetch($id);
		if (!$result) {
		...
		if (!$this->knowledgerecord->delete(GestimagApiAccess::$user)) {

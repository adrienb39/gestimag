 ...
 */
class Donations extends GestimagApi
{
	/**
	...
	{
		if (!GestimagApiAccess::$user->hasRight('don', 'lire')) {
		...
		if (!GestimagApi::_checkAccessToResource('don', $this->don->id)) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('don', 'creer')) {
		...
		if ($this->don->create(GestimagApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if (!GestimagApi::_checkAccessToResource('donation', $this->don->id)) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}
		foreach ($request_data as $field => $value) {
			if ($field == 'id') {
		...
		if ($this->don->update(GestimagApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('don', 'supprimer')) {
			throw new RestException(403);
		}
		...
		if (!$this->don->delete(GestimagApiAccess::$user)) {
	...
	public function validate($id, $idwarehouse = 0, $notrigger = 0)
	{
		...
		$result = $this->don->valid_promesse($id, GestimagApiAccess::$user->id, $notrigger);
		if ($result == 0) {

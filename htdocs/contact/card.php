...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
	...
	if ($user->hasRight('societe', 'contact', 'creer')) {
		if ($action == 'create') {
			...
			$doleditor = new DolEditor('note_public', $object->note_public, '', 80, 'gestimag_notes', 'In', 0, false, !getDolGlobalString('FCKEDITOR_ENABLE_NOTE_PUBLIC') ? 0 : 1, ROWS_3, '90%');
			print $doleditor->Create(1);
			print '</td></tr>';

			// Note Private
			print '<tr><td class="tdtop"><label for="note_private">'.$langs->trans("NotePrivate").'</label></td><td colspan="3">';
			$doleditor = new DolEditor('note_private', $object->note_private, '', 80, 'gestimag_notes', 'In', 0, false, !getDolGlobalString('FCKEDITOR_ENABLE_NOTE_PRIVATE') ? 0 : 1, ROWS_3, '90%');
			print $doleditor->Create(1);
			print '</td></tr>';
			...
			// Login Gestimag
			print '<tr><td>'.$langs->trans("GestimagLogin").'</td><td colspan="3">';
			if ($object->user_id) {
				$gestimag_user = new User($db);
				$result = $gestimag_user->fetch($object->user_id);
				print $gestimag_user->getLoginUrl(1);
			} else {
	...
	if (!empty($id) && $action != 'edit' && $action != 'create') {
		...
		print '<tr><td>'.$langs->trans("GestimagLogin").'</td><td>';
		if ($object->user_id) {
			$gestimag_user = new User($db);
			$result = $gestimag_user->fetch($object->user_id);
			print $gestimag_user->getLoginUrl(-1);
			...
			if (!$object->user_id && $user->hasRight('user', 'user', 'creer')) {
				print '<a class="aaa" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&action=create_user&token='.newToken().'">'.img_picto($langs->trans("CreateGestimagLogin"), 'add', 'class="pictofixedwidth"').$langs->trans("CreateGestimagLogin").'</a>';
			}
		}
		print '</td></tr>';

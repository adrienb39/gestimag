 ...
 * htdocs/modulebuilder/index.php?module=Inventory@/pathtogestimag/htdocs/product
 */
 ...
 *
 *       You can add parameter dirins=/home/ldestailleur/git/gestimag/htdocs/mymodule to force generation of module
...
// Dir for custom dirs
$tmp = explode(',', $gestimag_main_document_root_alt);
$dirins = $tmp[0];
$dirread = $dirins;
$forceddirread = 0;
...
foreach ($dirsrootforscan as $tmpdirread) {
	...
	// htdocs/modulebuilder/index.php?module=Inventory@/home/ldestailleur/git/gestimag/htdocs/product
	if (empty($i)) {
...
if ($dirins && $action == 'addlanguage' && !empty($module)) {
	...
	if ($newlangcode) {
		...
		if ($diroflang == $gestimag_main_document_root.'/'.$modulelowercase) {
			// This is not a custom module, we force diroflang to htdocs root
			$diroflang = $gestimag_main_document_root;
...
if ($dirins && $action == 'confirm_deleteobject' && $objectname) {
	...
	if (isModEnabled(strtolower($module))) {
		$result = unActivateModule(strtolower($module));
		gestimag_set_const($db, "MAIN_IHM_PARAMS_REV", (int) $conf->global->MAIN_IHM_PARAMS_REV + 1, 'chaine', 0, '', $conf->entity);
		if ($result) {
			setEventMessages($result, null, 'errors');
		}
		setEventMessages($langs->trans('WarningModuleNeedRefresh', $langs->transnoentities($module)), null, 'warnings');
...
print '<span class="opacitymedium hideonsmartphone">'.$langs->trans("ModuleBuilderDesc", 'https://wiki.gestimag.org/index.php/Module_development#Create_your_module').'</span>';
...
if (empty($allowonlineinstall)) {
	if (getDolGlobalString('MAIN_MESSAGE_INSTALL_MODULES_DISABLED_CONTACT_US')) {
		...
		$message = info_admin($langs->trans("InstallModuleFromWebHasBeenDisabledByFile", $gestimagdataroot.'/installmodules.lock'), 0, 0, 1, 'warning');
	}
			...
			foreach ($objMod->config_page_url as $page) {
				...
				if ($i++) {
					...
					if (preg_match('/^([^@]+)@([^@]+)$/i', $urlpage, $regs)) {
						...
						// module but a page provided by gestimag)
...
if ($module == 'initmodule') {
	...
	print ' - ';
	print '<a href="https://wiki.gestimag.org/index.php/List_of_modules_id" target="_blank" rel="noopener noreferrer external">'.$langs->trans("SeeReservedIDsRangeHere").'</a>';
	print ')';
	print '</span>';
	print '</div></div>';
				...
				if ($action != 'editfile' || empty($file)) {
					try {
						...
						if (!empty($tmpobject)) {
							...
							$htmltext = $langs->trans("PropertyDesc").'<br><br><a class="" href="https://wiki.gestimag.org/index.php/Language_and_development_rules#Table_and_fields_structures" target="_blank" rel="noopener noreferrer external">'.$langs->trans("SeeExamples").'</a>';
			...
			if ($action != 'editfile' || empty($file)) {
				...
				if (count($tabs)) {
					foreach ($tabs as $tab) {
						...
						// See: https://wiki.gestimag.org/index.php?title=Tabs_system#To_remove_an_existing_tab
						if ($tabName[0] === '-') {
							$tabTitle = '';

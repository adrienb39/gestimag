 ...
 *  \ingroup    core
 *  \brief		Set of function used for gestimag security (not common functions).
 ...
 *	@param		array	$authmode			Array list of selected authentication mode array('http', 'gestimag', 'xxx'...)
...
if (!function_exists('dol_loginfunction')) {
	/**
	 * Show Gestimag default login page.
	...
	{
		global $gestimag_main_demo, $gestimag_main_force_https;
		...
		$titletruegestimagversion = constant('DOL_VERSION'); // $title used by login template after the @ to inform of true Gestimag version
		...
		*/
		...
		if (getDolGlobalString('MAIN_SESSION_TIMEOUT')) {
			if (session_status() != PHP_SESSION_ACTIVE) {
				...
				setcookie($sessiontimeout, $conf->global->MAIN_SESSION_TIMEOUT, 0, "/", '', (empty($gestimag_main_force_https) ? false : true), true);
			}
		}
		...
		$demologin = '';
		$demopassword = '';
		if (!empty($gestimag_main_demo)) {
			$tab = explode(',', $gestimag_main_demo);
			$demologin = $tab[0];
			$demopassword = $tab[1];
		}
...
function encodedecode_dbpassconf($level = 0)
{
	...
	if ($fp = fopen(DOL_DOCUMENT_ROOT.'/conf/conf.php', 'r')) {
		while (!feof($fp)) {
			...
			$reg = array();
			if (preg_match('/^[^#]*gestimag_main_db_encrypted_pass[\s]*=[\s]*(.*)/i', $buffer, $reg)) {	// Old way to save encrypted value
			...
			} elseif (preg_match('/^[^#]*gestimag_main_db_pass[\s]*=[\s]*(.*)/i', $buffer, $reg)) {
				$val = trim($reg[1]); // This also remove CR/LF
				$val = preg_replace('/^["\']/', '', $val);
				$val = preg_replace('/["\'][\s;]*$/', '', $val);
			...
			if ($lineofpass) {
				...
				if ($level == 0) {
					$config .= '$gestimag_main_db_pass=\''.$passwd.'\';'."\n";
				}
				if ($level == 1) {

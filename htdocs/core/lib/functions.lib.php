 ...
 *	\brief			A set of functions for Gestimag
 ...
 *
 * @param 	CommonObject 	$object 	Gestimag common object
...
/**
 * Return gestimag global constant string value
 *
...
/**
 * Return a Gestimag global constant int value.
...
/**
 * Return Gestimag user constant string value
 *
...
/**
 * Return Gestimag user constant int value
 *
 * @param string 	$key 			Key to return value, return 0 if not set
 * @param int 		$default 		Value to return
 * @param User   	$tmpuser   		To get another user than current user
...
function GETPOST($paramname, $check = 'alphanohtml', $method = 0, $filter = null, $options = null, $noreplace = 0)
{
	...
	if ($paramname == 'backtopage' || $paramname == 'backtolist' || $paramname == 'backtourl') {
		...
		do {
			...
			$out = preg_replace(array('/^[^\?]*%/'), '', $out);				// We remove any % chars before the ?. Example in url: '/product/stock/card.php?action=create&backtopage=%2Fgestimag_dev%2Fhtdocs%2Fpro%25duct%2Fcard.php%3Fid%3Dabc'
...
if (!function_exists('dol_getprefix')) {
	/**
	 *  Return a prefix to use for this Gestimag instance, for session/cookie names or email id.
	...
	function dol_getprefix($mode = '')
	{
		...
		if ($mode == 'email') {
			...
			if (!empty($conf->file->instance_unique_id)) {
				return sha1('gestimag'.$conf->file->instance_unique_id);
			}
		...
		global $gestimag_main_instance_unique_id, $gestimag_main_cookie_cryptkey;	// This is loaded by filefunc.inc.php
		$tmp_instance_unique_id = empty($gestimag_main_instance_unique_id) ? (empty($gestimag_main_cookie_cryptkey) ? '' : $gestimag_main_cookie_cryptkey) : $gestimag_main_instance_unique_id; // Unique id of instance

		// The recommended value (may be not defined for old versions)
		if (!empty($tmp_instance_unique_id)) {
			return sha1('gestimag'.$tmp_instance_unique_id);
		}
 ...
 *  @param	int		$type						0=Used for a Filesystem path, 1=Used for an URL path (output relative), 2=Used for an URL path (output full path using same host that current url), 3=Used for an URL path (output full path using host defined into $gestimag_main_url_root of conf file)
 ...
 *												On Windows LOG_ERR=4, LOG_WARNING=5, LOG_NOTICE=LOG_INFO=6, LOG_DEBUG=6 if define_syslog_variables ou PHP 5.3+, 7 if gestimag
 ...
 *
 *	@param	    string		$domain	    			Domain name (Ex: "yahoo.com", "yhaoo.com", "gestimag.fr")
...
function dol_print_error($db = null, $error = '', $errors = null)
{
	...
	if ($_SERVER['DOCUMENT_ROOT']) {    // Mode web
		$out .= $langs->trans("GestimagHasDetectedError").".<br>\n";
		...
		$out .= "<b>".$langs->trans("Gestimag").":</b> ".DOL_VERSION." - https://www.gestimag.org<br>\n";
	...
	if (empty($gestimag_main_prod) && $_SERVER['DOCUMENT_ROOT'] && function_exists('xdebug_print_function_stack') && function_exists('xdebug_call_file')) {
	...
	if (empty($gestimag_main_prod)) {
		...
		print $langs->trans("GestimagHasDetectedError").'. ';
		print $langs->trans("YouCanSetOptionGestimagMainProdToZero");
...
function price2num($amount, $rounding = '', $option = 0)
{
	...
	if ($option != 1) {	// If not a PHP number or unknown, we change or clean format
		...
		// Convert amount to format with gestimag dec and thousand (this is because PHP convert a number
		// to format defined by LC_NUMERIC after a calculation and we want source format to be like defined by Gestimag setup.
		// So if number was already a good number, it is converted into local Gestimag setup.
		if (is_numeric($amount)) {
	...
	if ($rounding) {
		...
		// Convert amount to format with gestimag dec and thousand (this is because PHP convert a number
		// to format defined by LC_NUMERIC after a calculation and we want source format to be defined by Gestimag setup.
		if (is_numeric($amount)) {
			// We put in temps value of decimal ("0.00001"). Works with 0 and 2.0E-5 and 9999.10
			$temps = sprintf("%10.10F", $amount - intval($amount)); // temps=0.0000000000 or 0.0000200000 or 9999.1000000000
			$temps = preg_replace('/([\.1-9])0+$/', '\\1', $temps); // temps=0. or 0.00002 or 9999.1
			$nbofdec = max(0, dol_strlen($temps) - 2); // -2 to remove "0."
			$amount = number_format($amount, min($nbofdec, $nbofdectoround), $dec, $thousand); // Convert amount to format with gestimag dec and thousand
		}
		//print "TT".$amount.'<br>';
...
function dol_eval($s, $returnvalue = 1, $hideerrors = 1, $onlysimplestring = '1')
{
	...
	try {
		...
		$forbiddenphpfunctions = array_merge($forbiddenphpfunctions, array("dol_eval", "executeCLI", "verifCond"));	// native gestimag functions
		$forbiddenphpfunctions = array_merge($forbiddenphpfunctions, array("eval", "create_function", "assert", "mb_ereg_replace")); // function with eval capabilities
		$forbiddenphpfunctions = array_merge($forbiddenphpfunctions, array("dol_compress_dir", "dol_decode", "dol_delete_file", "dol_delete_dir", "dol_delete_dir_recursive", "dol_copy", "archiveOrBackupFile")); // more gestimag functions
 ...
 * @param string	$link			(optional) The link to a internal gestimag page, where to go on clicking on the number or the ... (without the first "/")
...
/**
 * Function to forge a SQL criteria from a Gestimag filter syntax string.
...
/**
 * Helper function that combines values of a gestimag DatePicker (such as Form::selectDate) for year, month, day (and
 ...
 * @global Conf $conf Gestimag configuration object (global)

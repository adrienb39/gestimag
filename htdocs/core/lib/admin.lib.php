 ...
 *  To check if Gestimag version is lower than (x,y,z), do "if versioncompare(versiongestimagarray(), array(x.y.z)) <= 0"
 *  For example: if (versioncompare(versiongestimagarray(),array(4,0,-5)) >= 0) is true if version is 4.0 alpha or higher.
 *  For example: if (versioncompare(versiongestimagarray(),array(4,0,0)) >= 0) is true if version is 4.0 final or higher.
 *  For example: if (versioncompare(versiongestimagarray(),array(4,0,1)) >= 0) is true if version is 4.0.1 or higher.
...
/**
 *	Return version Gestimag
 *
 *	@return     array               Tableau de version (vermajeur,vermineur,autre)
 *  @see versioncompare()
 */
function versiongestimagarray()
{
 ...
 *  - Migrate process (gestimag-xyz-abc.sql)
				...
				if ($qualified) {
					if (!empty($reg[2])) {
						if (is_numeric($reg[2])) {	// This is a version
							...
							$dbcollation = strtoupper(preg_replace('/_/', '', $conf->db->gestimag_main_db_collation));
							//var_dump($reg[2]);
							//var_dump($dbcollation);
							if (empty($conf->db->gestimag_main_db_collation) || ($reg[2] != $dbcollation)) {
								$qualified = 0;
							}
							//var_dump($qualified);
						}
 ...
 *	@return     int         			Return integer <0 if KO, >0 if OK
 *
 *	@see		gestimag_get_const(), gestimag_set_const(), dol_set_user_param()
 */
function gestimag_del_const($db, $name, $entity = 1)
{
	global $conf;
	...
	dol_syslog("admin.lib::gestimag_del_const", LOG_DEBUG);
	$resql = $db->query($sql);
	if ($resql) {
		$conf->global->$name = '';
		return 1;
 ...
 *
 *	@see		gestimag_del_const(), gestimag_set_const(), dol_set_user_param()
 */
function gestimag_get_const($db, $name, $entity = 1)
{
	$value = '';
	...
	dol_syslog("admin.lib::gestimag_get_const", LOG_DEBUG);
	$resql = $db->query($sql);
	if ($resql) {
		$obj = $db->fetch_object($resql);
		if ($obj) {
 ...
 *
 *	@see		gestimag_del_const(), gestimag_get_const(), dol_set_user_param()
 */
function gestimag_set_const($db, $name, $value, $type = 'chaine', $visible = 0, $note = '', $entity = 1)
{
	global $conf;
	...
	// Check parameters
	if (empty($name)) {
		dol_print_error($db, "Error: Call to function gestimag_set_const with wrong parameters");
		exit;
	}

	//dol_syslog("gestimag_set_const name=$name, value=$value type=$type, visible=$visible, note=$note entity=$entity");
		...
		while (($file = @readdir($dh)) !== false) {
			if (preg_match('/^sess_/i', $file) && $file != "." && $file != "..") {
				...
				if (!@is_dir($fullpath) && is_readable($fullpath)) {
					...
					// dol_login|s:5:"admin";dol_authmode|s:8:"gestimag";dol_tz|s:1:"1";dol_tz_string|s:13:"Europe/Berlin";dol_dst|i:0;dol_dst_observed|s:1:"1";dol_dst_first|s:0:"";dol_dst_second|s:0:"";dol_screenwidth|s:4:"1920";
					...
					if (preg_match('/dol_login/i', $sessValues) && // limit to gestimag session
 ...
 *	Show array with constants to edit
 *
 *	@param	GestimagModules[]	$modules	Array of all modules

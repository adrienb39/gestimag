...
function dolWebsiteReplacementOfLinks($website, $content, $removephppart = 0, $contenttype = 'html', $containerid = 0)
{
	...
	// Replace relative link '/' with gestimag URL
	...
	// Replace relative link /xxx.php#aaa or /xxx.php with gestimag URL (we discard param ?...)
	$content = preg_replace('/(href=")\/?([^:\"\!]*)\.php(#[^\"<>]*)?\"/', '\1!~!~!~'.DOL_URL_ROOT.'/website/index.php?website='.$website->ref.'&pageref=\2\3"', $content, -1, $nbrep);
	// Replace relative link /xxx.php?a=b&c=d#aaa or /xxx.php?a=b&c=d with gestimag URL
	...
	// <img src="medias/...image.png... => <img src="gestimag/viewimage.php/modulepart=medias&file=image.png...
	// <img src="...image.png... => <img src="gestimag/viewimage.php/modulepart=medias&file=image.png...
	...
	// <img src="viewimage.php/modulepart=medias&file=image.png" => <img src="gestimag/viewimage.php/modulepart=medias&file=image.png"
	$content = preg_replace('/(<img[^>]*src=")(\/?viewimage\.php)/', '\1!~!~!~'.DOL_URL_ROOT.'/viewimage.php', $content, -1, $nbrep);

	// action="newpage.php" => action="gestimag/website/index.php?website=...&pageref=newpage
 ...
 * Used to output the page when viewed from a server (Gestimag or Apache).
 *
...
function dolWebsiteOutput($content, $contenttype = 'html', $containerid = 0)
{
	...
	global $gestimag_main_url_root, $gestimag_main_data_root;
	global $website;
	...
	if (defined('USEDOLIBARREDITOR')) {		// REPLACEMENT OF LINKS When page called from Gestimag editor
	...
	} elseif (defined('USEDOLIBARRSERVER')) {	// REPLACEMENT OF LINKS When page called from Gestimag server
		...
		// Replace relative link / with gestimag URL:  ...href="/"...
		$content = preg_replace('/(href=")\/\"/', '\1!~!~!~'.DOL_URL_ROOT.'/public/website/index.php?website='.$website->ref.'"', $content, -1, $nbrep);
		// Replace relative link /xxx.php#aaa or /xxx.php with gestimag URL:  ...href="....php" (we discard param ?...)
		...
		// Replace occurrence like _service_XXX.php with gestimag URL
		...
		// Replace occurrence like _library_XXX.php with gestimag URL
		...
		// Replace relative link without .php like /xxx#aaa or /xxx with gestimag URL:  ...href="....php"
		...
		// Fix relative link /document.php with correct URL after the DOL_URL_ROOT:  href="/document.php?modulepart=" => href="/gestimag/document.php?modulepart="
		$content = preg_replace('/(href=")(\/?document\.php\?[^\"]*modulepart=[^\"]*)(\")/', '\1!~!~!~'.DOL_URL_ROOT.'\2\3', $content, -1, $nbrep);
		$content = preg_replace('/(src=")(\/?document\.php\?[^\"]*modulepart=[^\"]*)(\")/', '\1!~!~!~'.DOL_URL_ROOT.'\2\3', $content, -1, $nbrep);

		// Fix relative link /viewimage.php with correct URL after the DOL_URL_ROOT: href="/viewimage.php?modulepart=" => href="/gestimag/viewimage.php?modulepart="
		...
		// <img alt="" src="/gestimag_dev/htdocs/viewimage.php?modulepart=medias&amp;entity=1&amp;file=image/ldestailleur_166x166.jpg" style="height:166px; width:166px" />
		// become
...
function redirectToContainer($containerref, $containeraliasalt = '', $containerid = 0, $permanent = 0, $parameters = array())
{
	...
	if (defined('USEDOLIBARRSERVER')) {	// When page called from Gestimag server
...
function getPagesFromSearchCriterias($type, $algo, $searchstring, $max = 25, $sortfield = 'date_creation', $sortorder = 'DESC', $langcode = '', $otherfilters = [], $status = 1)
{
	...
	*/
	...
	if (!$error && (empty($max) || ($found < $max)) && (preg_match('/sitefiles/', $algo))) {
		...
		$pathofwebsite = $gestimag_main_data_root.($conf->entity > 1 ? '/'.$conf->entity : '').'/website/'.$website->ref;

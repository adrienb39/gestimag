 ...
 *  \ingroup    core
 *  \brief		Set of function used for gestimag security (common function included into filefunc.inc.php)
 ...
 *
 *	@param   string		$chain		String to encode
 *	@param   string		$key		If '', we use $conf->file->instance_unique_id (so $gestimag_main_instance_unique_id in conf.php)
...
{
	global $conf;
	global $gestimag_disable_dolcrypt_for_debug;
	...
	if (function_exists('openssl_encrypt') && empty($gestimag_disable_dolcrypt_for_debug)) {
		if (empty($key)) {
			return $chain;
		}
...
function dolGetLdapPasswordHash($password, $type = 'md5')
{
	...
	if ($type === 'md5') {
		...
		return '{MD5}' . base64_encode(hex2bin($password)); // Create OpenLDAP MD5 password from Gestimag MD5 password
	} elseif ($type === 'smd5') {

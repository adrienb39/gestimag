...
function dol_move($srcfile, $destfile, $newmask = '0', $overwriteifexists = 1, $testvirus = 0, $indexdatabase = 1, $moreinfo = array())
{
	...
	if ($overwriteifexists || !$destexists) {
		...
		global $gestimag_main_restrict_os_commands;
		if (!empty($gestimag_main_restrict_os_commands)) {
			$arrayofallowedcommand = explode(',', $gestimag_main_restrict_os_commands);
...
function dol_check_secure_access_document($modulepart, $original_file, $entity, $fuser = null, $refname = '', $mode = 'read')
{
	...
	global $gestimag_main_data_root, $gestimag_main_document_root_alt;
	global $object;
	...
	} elseif ($modulepart == 'medias' && !empty($gestimag_main_data_root)) {
	...
	} elseif ($modulepart == 'logs' && !empty($gestimag_main_data_root)) {
		// Wrapping for *.log files, like when used with url http://.../document.php?modulepart=logs&file=gestimag.log
		$accessallowed = ($user->admin && basename($original_file) == $original_file && preg_match('/^gestimag.*\.(log|json)$/', basename($original_file)));
		$original_file = $gestimag_main_data_root.'/'.$original_file;
	} elseif ($modulepart == 'doctemplates' && !empty($gestimag_main_data_root)) {
		// Wrapping for doctemplates
		$accessallowed = $user->admin;
		$original_file = $gestimag_main_data_root.'/doctemplates/'.$original_file;
	} elseif ($modulepart == 'doctemplateswebsite' && !empty($gestimag_main_data_root)) {
		// Wrapping for doctemplates of websites
		$accessallowed = ($fuser->hasRight('website', 'write') && preg_match('/\.jpg$/i', basename($original_file)));
		$original_file = $gestimag_main_data_root.'/doctemplates/websites/'.$original_file;
	} elseif ($modulepart == 'packages' && !empty($gestimag_main_data_root)) {	// To download zip of modules
		...
		if (preg_match('/^(\d+)\/photos\//', $original_file, $reg)) {
			if ($reg[1]) {
				...
				if (getDolUserInt('USER_ENABLE_PUBLIC', 0, $tmpobject)) {
					...
					$valuetouse = $gestimag_main_instance_unique_id ? $gestimag_main_instance_unique_id : $gestimag_main_cookie_cryptkey; // Use $gestimag_main_instance_unique_id first then $gestimag_main_cookie_cryptkey
...
function getFilesUpdated(&$file_list, SimpleXMLElement $dir, $path = '', $pathref = '', &$checksumconcat = array())
{
	...
	foreach ($dir->md5file as $file) {    // $file is a simpleXMLElement
		...
		if (!file_exists($pathref.'/'.$filename)) {
			...
			if ($conffile == '/etc/gestimag/conf.php' && $filename == '/filefunc.inc.php') {	// For install with deb or rpm, we ignore test on filefunc.inc.php that was modified by package

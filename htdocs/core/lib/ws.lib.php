 ...
 *
 *  @param	array{login:string,password:string,entity:?int,gestimagkey:string}	$authentication     Array with authentication information ('login'=>,'password'=>,'entity'=>,'gestimagkey'=>)
...
function check_authentication($authentication, &$error, &$errorcode, &$errorlabel)
{
	...
	if (!$error && ($authentication['gestimagkey'] != $conf->global->WEBSERVICES_KEY)) {
		$error++;
		$errorcode = 'BAD_VALUE_FOR_SECURITY_KEY';
		$errorlabel = 'Value provided into gestimagkey entry field does not match security key defined in Webservice module setup';
	}
	...
	if (!$error) {
		...
		if (!$error) {
			...
			if (empty($gestimag_main_authentication)) {
				$gestimag_main_authentication = 'http,gestimag';
			}

...
function dolSavePageAlias($filealias, $object, $objectpage)
{
	...
	$aliascontent .= 'global $gestimag_main_data_root;'."\n";
	$aliascontent .= 'if (empty($gestimag_main_data_root)) require \'./page'.$objectpage->id.'.tpl.php\'; ';
	$aliascontent .= 'else require $gestimag_main_data_root.\'/website/\'.$website->ref.\'/page'.$objectpage->id.'.tpl.php\';'."\n";
	$aliascontent .= '?>'."\n";
...
function dolSavePageContent($filetpl, Website $object, WebsitePage $objectpage, $backupold = 0)
{
	...
	if (!isset($originalcontentonly)) {
		...
		$tplcontent .= '<meta name="generator" content="'.DOL_APPLICATION_TITLE.' '.DOL_VERSION.' (https://www.gestimag.org)" />'."\n";
		$tplcontent .= '<meta name="gestimag:pageid" content="'.dol_string_nohtmltag($objectpage->id).'" />'."\n";
		...
		$tplcontent .= '<!-- File generated by Gestimag website module editor -->'."\n";

...
if ($action == 'redirect') {
	...
	//var_dump($gestimag_main_url_root);
	//var_dump(DOL_MAIN_URL_ROOT);
	//var_dump($tmpurlrootwithouthttp);
	$url = preg_replace('/'.preg_quote($gestimag_main_url_root, '/').'/', '', $url);
...
if (empty($gestimag_nocache) && GETPOSTINT('cache')) {
...
// URL http://mygestimag/core/search_page?dol_use_jmobile=1 can be used for tests

...
?>
// Javascript libraries for Gestimag ERP CRM (https://www.gestimag.org)
 ...
 * Minute | mm (2 digits)
 * Second | ss (2 digits)
 * Author: Laurent Destailleur Author: Matelli (see http://matelli.fr/showcases/patchs-gestimag/update-date-input-in-action-form.html)
 * Licence: GPL
 * ==================================================================
 */
	...
	$.widget( "ui.combobox", {
		...
		_create: function() {
			...
			var input = this.input = $( "<input>" )
				...
				.addClass( "ui-widget ui-widget-content ui-corner-left gestimagcombobox" );
 ...
 * @param  {string} force_locale     ISO code locale to use (if empty, will use Gestimag's current locale code)
...
function pricejs(amount, mode = 'MT', currency_code = '', force_locale = '') {
	...
	// No Intl -> attempt to format the number in a way similar to Gestimag PHP's `price()` function

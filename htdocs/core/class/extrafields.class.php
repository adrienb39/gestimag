				...
				if ($filter_categorie === false) {
					...
					if ($obj->rowid) {
						...
						if ($result > 0) {
							...
							foreach ($ways as $way) {
								$toprint[] = '<li class="select2-search-choice-gestimag noborderoncategories"' . ($c->color ? ' style="background: #' . $c->color . ';"' : ' style="background: #bbb"') . '>' . img_object('', 'category') . ' ' . $way . '</li>';
							}
						}
					}
			...
			if ($resql) {
				if ($filter_categorie === false) {
					...
					while ($obj = $this->db->fetch_object($resql)) {
						...
						if (is_array($value_arr) && in_array($obj->rowid, $value_arr)) {
							if (is_array($fields_label) && count($fields_label) > 1) {
								$label = '<li class="select2-search-choice-gestimag noborderoncategories" style="background: #bbb">';
								foreach ($fields_label as $field_toshow) {
									$translabel = '';
									if (!empty($obj->$field_toshow)) {
										$translabel = $outputlangs->trans($obj->$field_toshow);

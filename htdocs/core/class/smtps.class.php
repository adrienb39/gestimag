...
class SMTPs
{
	...
	private function _server_authenticate()
	{
		...
		if (getDolGlobalString('MAIL_SMTP_USE_FROM_FOR_HELO')) {
			if (!is_numeric(getDolGlobalString('MAIL_SMTP_USE_FROM_FOR_HELO'))) {
				...
				// If value of MAIL_SMTP_USE_FROM_FOR_HELO is 2, we use the domain in the $gestimag_main_url_root.
				global $gestimag_main_url_root;
				$hosth = getDomainFromURL($gestimag_main_url_root, 1);
			}
		}
	...
	public function getHeader()
	{
		...
		*/
		...
		if ($trackid) {
			$_header .= 'Message-ID: <'.time().'.SMTPs-gestimag-'.$trackid.'@'.$host.">\r\n";
			$_header .= 'X-Gestimag-TRACKID: '.$trackid.'@'.$host."\r\n";
		} else {
		...
		$_header .= 'X-Mailer: Gestimag version '.DOL_VERSION.' (using SMTPs Mailer)'."\r\n";
		$_header .= 'X-Gestimag-Option: '.($conf->global->MAIN_MAIL_USE_MULTI_PART ? 'MAIN_MAIL_USE_MULTI_PART' : 'No MAIN_MAIL_USE_MULTI_PART')."\r\n";
	...
	private function _setBoundary()
	{
		...
		$this->_smtpsRelatedBoundary = 'mul_'.dol_hash(uniqid("gestimag2"), 3);
		$this->_smtpsAlternativeBoundary = 'mul_'.dol_hash(uniqid("gestimag3"), 3);
	}

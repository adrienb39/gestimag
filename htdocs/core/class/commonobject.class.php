...
abstract class CommonObject
{
	...
	{
		global $user, $gestimag_main_url_root;
							...
							if ($algo != '') {
								...
								if (is_object($this->oldcopy)) {	// If this->oldcopy is not defined, we can't know if we change attribute or not, so we must keep value
									...
									if (isset($this->oldcopy->array_options[$key]) && $this->array_options[$key] == $this->oldcopy->array_options[$key]) {
										...
										if ($algo == 'dolcrypt') {	// gestimag reversible encryption
									...
									if ($algo == 'dolcrypt' && !preg_match('/^dolcrypt:/', $this->array_options[$key])) {	// gestimag reversible encryption
										$new_array_options[$key] = dolEncrypt($this->array_options[$key]);	// warning, must be called when on the master
									} else {
										$new_array_options[$key] = $this->array_options[$key]; // Value is kept
									}
					...
					if ($this->array_options["options_".$key] != '' && is_array($extrafields->attributes[$this->table_element]['param'][$attributeKey]['options'])) {
						...
						if ($algo != '') {
							...
							if (is_object($this->oldcopy)) {		// If this->oldcopy is not defined, we can't know if we change attribute or not, so we must keep value
								...
								if ($algo == 'dolcrypt' && !preg_match('/^dolcrypt:/', $this->array_options["options_".$key])) {	// gestimag reversible encryption
									$new_array_options["options_".$key] = dolEncrypt($this->array_options["options_".$key]);	// warning, must be called when on the master
								} else {
									$new_array_options["options_".$key] = $this->array_options["options_".$key]; // Value is kept
								}
	...
	public function showInputField($val, $key, $value, $moreparam = '', $keysuffix = '', $keyprefix = '', $morecss = 0, $nonewbutton = 0)
	{
		...
		if (in_array($type, array('date'))) {
			...
			if (!preg_match('/search_/', $keyprefix)) {		// If keyprefix is search_ or search_options_, we must just use a simple text field
				...
				$doleditor = new DolEditor($keyprefix.$key.$keysuffix, $value, '', 200, 'gestimag_notes', 'In', false, false, false, ROWS_5, '90%');
			...
			if (!preg_match('/search_/', $keyprefix)) {		// If keyprefix is search_ or search_options_, we must just use a simple text field
				require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
				$doleditor = new DolEditor($keyprefix.$key.$keysuffix, $value, '', 200, 'gestimag_notes', 'In', false, false, isModEnabled('fckeditor') && getDolGlobalInt('FCKEDITOR_ENABLE_SOCIETE'), ROWS_5, '90%');
		...
		if (in_array($key, array('rowid', 'ref')) && method_exists($this, 'getNomUrl')) {
			...
			if ($resql) {
				if (!$filter_categorie) {
					...
					foreach ($ways as $way) {
						$toprint[] = '<li class="select2-search-choice-gestimag noborderoncategories"' . ($c->color ? ' style="background: #' . $c->color . ';"' : ' style="background: #aaa"') . '>' . img_object('', 'category') . ' ' . $way . '</li>';
					}
					$value = '<div class="select2-container-multi-gestimag" style="width: 90%;"><ul class="select2-choices-gestimag">'.implode(' ', $toprint).'</ul></div>';
				}
			} else {
						...
						if (is_array($value_arr) && in_array($obj->rowid, $value_arr)) {
							if (is_array($fields_label) && count($fields_label) > 1) {
								foreach ($fields_label as $field_toshow) {
									...
									if ($translabel != $field_toshow) {
										$toprint[] = '<li class="select2-search-choice-gestimag noborderoncategories" style="background: #bbb">' . dol_trunc($translabel, 18) . '</li>';
									} else {
										$toprint[] = '<li class="select2-search-choice-gestimag noborderoncategories" style="background: #bbb">' . $obj->$field_toshow . '</li>';
									}
								}
							} else {
								$translabel = '';
								...
								if ($translabel != $obj->{$InfoFieldList[1]}) {
									...
									$toprint[] = '<li class="select2-search-choice-gestimag noborderoncategories" style="background: #bbb">' . $obj->{$InfoFieldList[1]} . '</li>';
								}
							}
						}
					}
			...
			if ($value) {
				...
				if (!empty($classpath)) {
					...
					if ($classname && !class_exists($classname)) {
						// from V19 of Gestimag, In some cases link use element instead of class, example project_task

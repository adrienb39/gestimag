...
class Utils
{
	...
	public function purgeFiles($choices = 'tempfilesold+logfiles', $nbsecondsold = 86400)
	{
		...
		foreach ($choicesarray as $choice) {
			...
			if ($choice == 'tempfiles' || $choice == 'tempfilesold') {
				...
				if ($gestimag_main_data_root) {
					$filesarray = dol_dir_list($gestimag_main_data_root, "directories", 1, '^temp$', '', 'name', SORT_ASC, 2, 0, '', 1); // Do not follow symlinks
			...
			if ($choice == 'allfiles') {
				...
				if ($gestimag_main_data_root) {
					$filesarray = dol_dir_list($gestimag_main_data_root, "all", 0, '', '(\.lock|\.unlock)$', 'name', SORT_ASC, 0, 0, '', 1);	// No need to use recursive, we will delete directory
				}
			}
			...
			if ($choice == 'allfilesold') {
				// Delete all files (except .lock and .unlock files, do not follow symbolic links)
				if ($gestimag_main_data_root) {
					$filesarray = dol_dir_list($gestimag_main_data_root, "files", 1, '', '(\.lock|\.unlock)$', 'name', SORT_ASC, 0, 0, '', 1, $nbsecondsold);	// No need to use recursive, we will delete directory
				}
			}
			...
			if ($choice == 'logfile' || $choice == 'logfiles') {
				// Define files log
				if ($gestimag_main_data_root) {
					$filesarray = dol_dir_list($gestimag_main_data_root, "files", 0, '.*\.log[\.0-9]*(\.gz)?$', '(\.lock|\.unlock)$', 'name', SORT_ASC, 0, 0, '', 1);
				}
	...
	{
		global $db, $conf, $langs, $gestimag_main_data_root;
		global $gestimag_main_db_name, $gestimag_main_db_host, $gestimag_main_db_user, $gestimag_main_db_port, $gestimag_main_db_pass;
		...
		if ($file == 'auto') {
			...
			$file = $prefix.'_'.$gestimag_main_db_name.'_'.dol_sanitizeFileName(DOL_VERSION).'_'.dol_print_date(dol_now('gmt'), "dayhourlogsmall", 'tzuser').'.'.$ext;
		}
		...
		if ($type == 'mysql' || $type == 'mysqli') {
			...
			//$param=escapeshellarg($gestimag_main_db_name)." -h ".escapeshellarg($gestimag_main_db_host)." -u ".escapeshellarg($gestimag_main_db_user)." -p".escapeshellarg($gestimag_main_db_pass);
			$param = $gestimag_main_db_name." -h ".$gestimag_main_db_host;
			$param .= " -u ".$gestimag_main_db_user;
			if (!empty($gestimag_main_db_port)) {
				$param .= " -P ".$gestimag_main_db_port." --protocol=tcp";
			}
			...
			if (!empty($gestimag_main_db_pass)) {
				$paramcrypted .= ' -p"'.preg_replace('/./i', '*', $gestimag_main_db_pass).'"';
				$paramclear .= ' -p"'.str_replace(array('"', '`', '$'), array('\"', '\`', '\$'), $gestimag_main_db_pass).'"';
			}
		...
		if ($type == 'postgresql' || $type == 'pgsql') {
			...
			//$param="-F c";
			$param = "-F p";
			$param .= " --no-tablespaces --inserts -h ".$gestimag_main_db_host;
			$param .= " -U ".$gestimag_main_db_user;
			if (!empty($gestimag_main_db_port)) {
				$param .= " -p ".$gestimag_main_db_port;
			}
			...
			/*if (!empty($gestimag_main_db_pass))
			 {
			 $paramcrypted.=" -W".preg_replace('/./i','*',$gestimag_main_db_pass);
			 $paramclear.=" -W".$gestimag_main_db_pass;
			 }*/
			$paramcrypted .= " -w ".$gestimag_main_db_name;
			$paramclear .= " -w ".$gestimag_main_db_name;
	...
	public function compressSyslogs()
	{
		...
		$tabfiles = dol_dir_list(DOL_DATA_ROOT, 'files', 0, '^(gestimag_.+|odt2pdf)\.log$'); // Also handle other log files like gestimag_install.log
	 ...
	 *	Note about foreign keys constraints: for Gestimag, since there are a lot of constraints and when imported the tables will be inserted in the dumped order, not in constraints order, then we ABSOLUTELY need to use SET FOREIGN_KEY_CHECKS=0; when importing the sql dump.
	...
	public function backupTables($outputfile, $tables = '*')
	{
		...
		$sqlhead = '';
		$sqlhead .= "-- ".$db::LABEL." dump via php with Gestimag ".DOL_VERSION."
--
	...
	public function sendBackup($sendto = '', $from = '', $subject = '', $message = '', $filename = '', $filter = '', $sizelimit = 100000000)
	{
		...
		if ($filepath) {
			if ($filesize > $sizelimit) {
				...
				$documenturl =  $gestimag_main_url_root.'/document.php?modulepart=systemtools&atachement=1&file=backup/'.urlencode($filename[0]);

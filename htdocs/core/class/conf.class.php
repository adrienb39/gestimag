...
class Conf extends stdClass
{
	...
	public function setValues($db)
	{
		...
		if (!is_null($db) && is_object($db)) {
			...
			if ($resql) {
				...
				while ($i < $numr) {
					...
					if ($key) {
						...
						$this->global->$key = dolDecrypt($value);	// decrypt data excrypted with gestimag_set_const($db, $name, $value)
			...
			// conf->email_from = email by default to send Gestimag automatic emails
			...
			// conf->notification->email_from = email by default to send Gestimag notifications
			...
			if (!isset($this->global->MAIL_SMTP_USE_FROM_FOR_HELO)) {
				$this->global->MAIL_SMTP_USE_FROM_FOR_HELO = 2;	// Use the domain in $gestimag_main_url_root (mydomain.com)
			}
			...
			if (!empty($this->global->LDAP_SYNCHRO_ACTIVE)) {
				if ($this->global->LDAP_SYNCHRO_ACTIVE == 'gestimag2ldap') {
					$this->global->LDAP_SYNCHRO_ACTIVE = 1;
				} elseif ($this->global->LDAP_SYNCHRO_ACTIVE == 'ldap2gestimag') {
					$this->global->LDAP_SYNCHRO_ACTIVE = 2;
				}
			}
			// For backward compatibility
			if (!empty($this->global->LDAP_MEMBER_ACTIVE) && $this->global->LDAP_MEMBER_ACTIVE == 'ldap2gestimag') {
				$this->global->LDAP_MEMBER_ACTIVE = 2;
			}
			// For backward compatibility
			if (!empty($this->global->LDAP_MEMBER_TYPE_ACTIVE) && $this->global->LDAP_MEMBER_TYPE_ACTIVE == 'ldap2gestimag') {

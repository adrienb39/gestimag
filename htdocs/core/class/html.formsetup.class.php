	...
	public function generateOutput($editMode = false, $hideTitle = false)
	{
		...
		if ($reshook > 0) {
			...
			if ($reshook > 0) {
				...
				$out .= '<input class="button button-save" type="submit" value="' . $this->langs->trans("Save") . '">'; // Todo fix gestimag style for <button and use <button instead of input
				/*$out .= ' &nbsp;&nbsp; ';
...
class FormSetupItem
{
	...
	public function saveConfValue()
	{
		...
		if ($this->type != 'title') {
			$result = gestimag_set_const($this->db, $this->confKey, $this->fieldValue, 'chaine', 0, '', $this->entity);
			if ($result < 0) {
				return -1;
			} else {
				return 1;
	...
	public function generateInputField()
	{
		...
		if ($this->type == 'title') {
			...
			$out .= $this->generateInputFieldPassword('gestimag');
	...
	public function generateInputFieldHtml()
	{
		...
		$doleditor = new DolEditor($this->confKey, $this->fieldValue, '', 160, 'gestimag_notes', '', false, false, isModEnabled('fckeditor'), ROWS_5, '90%');
	 ...
	 *
	 * @param   string  $type  'gestimag' (gestimag password rules apply) or 'generic'
	 *
	 * @return  string
	 */
	...
	public function generateOutputField()
	{
		...
		if ($this->type == 'title') {
			...
			$out .= '<div class="select2-container-multi-gestimag" style="width: 90%;"><ul class="select2-choices-gestimag">' . implode(' ', $toprint) . '</ul></div>';
		} elseif (preg_match('/thirdparty_type/', $this->type)) {
	...
	/**
	 * Set type of input as a password with gestimag password rules apply.
	...
	/**
	 * Set type of input as a generic password without gestimag password rules (for external passwords for example).
	 * Hide entry on display.
	 *
	 * @return self
	 */

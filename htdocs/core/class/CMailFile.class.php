...
class CMailFile
{
	...
	{
		global $conf, $gestimag_main_data_root, $user;
		...
		$this->related_boundary = 'mul_'.dol_hash(uniqid("gestimag2"), 3); // Force md5 hash (does not contain special chars)

		// On defini alternative_boundary
		$this->alternative_boundary = 'mul_'.dol_hash(uniqid("gestimag3"), 3); // Force md5 hash (does not contain special chars)
		...
		if ($this->msgishtml) {
			...
			if (getDolGlobalString('MAIN_MAIL_ADD_INLINE_IMAGES_IF_IN_MEDIAS')) {	// Off by default
				...
				$findimg = $this->findHtmlImages($gestimag_main_data_root.'/medias');
				if ($findimg < 0) {
		...
		if ($this->sendmode == 'mail') {
			...
			//X-Gestimag-TRACKID, In-Reply-To, References and $moreinheader will be added to header inside the smtps->getHeader
			...
			$this->msgid = time().'.SMTPs-gestimag-'.$this->trackid.'@'.$host;
			...
			$headers->addTextHeader('X-Gestimag-TRACKID', $this->trackid.'@'.$host);
			$this->msgid = time().'.swiftmailer-gestimag-'.$this->trackid.'@'.$host;
			$headerID = $this->msgid;
	...
	public function dump_mail()
	{
		...
		if (@is_writable($gestimag_main_data_root)) {	// Avoid fatal error on fopen with open_basedir
			$outputfile = $gestimag_main_data_root."/gestimag_mail.log";
			...
			// Move gestimag_mail.log into a gestimag_mail.YYYYMMDD.log
			if (getDolGlobalString('MAIN_MAIL_DEBUG_LOG_WITH_DATE')) {
				$destfile = $gestimag_main_data_root."/gestimag_mail.".dol_print_date(dol_now(), 'dayhourlog', 'gmt').".log";
	...
	public function save_dump_mail_in_err($message = '')
	{
		...
		if (@is_writable($gestimag_main_data_root)) {	// Avoid fatal error on fopen with open_basedir
			$srcfile = $gestimag_main_data_root."/gestimag_mail.log";

			// Add message to gestimag_mail.log. We do not use dol_syslog() on purpose,
			// to be sure to write into gestimag_mail.log
			if ($message) {
			...
			// Move gestimag_mail.log into a gestimag_mail.err or gestimag_mail.date.err
			if (getDolGlobalString('MAIN_MAIL_DEBUG_ERR_WITH_DATE')) {
				$destfile = $gestimag_main_data_root."/gestimag_mail.".dol_print_date(dol_now(), 'dayhourlog', 'gmt').".err";
			} else {
				$destfile = $gestimag_main_data_root."/gestimag_mail.err";
			}
	...
	public function write_smtpheaders()
	{
		...
		if ($trackid) {
			$this->msgid = time().'.phpmail-gestimag-'.$trackid.'@'.$host;
			$out .= 'Message-ID: <'.$this->msgid.">".$this->eol2; // Uppercase seems replaced by phpmail
			$out .= 'X-Gestimag-TRACKID: '.$trackid.'@'.$host.$this->eol2;
		} else {
		...
		$out .= "X-Mailer: Gestimag version ".DOL_VERSION." (using php mail)".$this->eol2;
	 ...
	 *
	 * @param	string	$images_dir		Path to store physical images files. For example $gestimag_main_data_root.'/medias'
	 ...
	 *
	 * @param	string	$images_dir		Location of where to store physically images files. For example $gestimag_main_data_root.'/medias'
	 * @return	int 		        	>0 if OK, <0 if KO
	 */

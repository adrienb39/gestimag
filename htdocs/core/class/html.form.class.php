...
class Form
{
	 ...
	 * @param 	string 	$typeofdata 	Type of data ('string' by default, 'email', 'amount:99', 'numeric:99', 'text' or 'textarea:rows:cols', 'datepicker' ('day' do not work, don't know why), 'dayhour' or 'datehourpicker' 'checkbox:ckeditor:gestimag_zzz:width:height:savemethod:1:rows:cols', 'select;xxx[:class]'...)
	 ...
	 *                                  'datepicker' ('day' do not work, don't know why), 'dayhour' or 'datehourpicker', 'ckeditor:gestimag_zzz:width:height:savemethod:toolbarstartexpanded:rows:cols', 'select;xkey:xval,ykey:yval,...')
		...
		if (getDolGlobalString('MAIN_USE_JQUERY_JEDITABLE') && !preg_match('/^select;|day|datepicker|dayhour|datehourpicker/', $typeofdata)) { // TODO add jquery timepicker and support select
			...
			if ($editmode) {	// edit mode
				...
				if (preg_match('/^(string|safehtmlstring|email|phone|url)/', $typeofdata)) {
					...
					$tmp = explode(':', $typeofdata); // Example: ckeditor:gestimag_zzz:width:height:savemethod:toolbarstartexpanded:rows:cols:uselocalbrowser
					require_once DOL_DOCUMENT_ROOT . '/core/class/doleditor.class.php';
					$doleditor = new DolEditor($htmlname, ($editvalue ? $editvalue : $value), (empty($tmp[2]) ? '' : $tmp[2]), (empty($tmp[3]) ? '100' : $tmp[3]), (empty($tmp[1]) ? 'gestimag_notes' : $tmp[1]), 'In', (empty($tmp[5]) ? 0 : $tmp[5]), (isset($tmp[8]) ? ($tmp[8] ? true : false) : true), true, (empty($tmp[6]) ? '20' : $tmp[6]), (empty($tmp[7]) ? '100' : $tmp[7]));
	 ...
	 * @param 	boolean $perm 		Permission to allow button to edit parameter. Set it to 0 to have a not edited field.
	 * @param 	string 	$typeofdata Type of data ('string' by default, 'email', 'amount:99', 'numeric:99', 'text' or 'textarea:rows:cols', 'datepicker' ('day' do not work, don't know why), 'ckeditor:gestimag_zzz:width:height:savemethod:1:rows:cols', 'select;xxx[:class]'...)
	 ...
	 * @param 	string 	$inputType 	Type of input ('string', 'numeric', 'datepicker' ('day' do not work, don't know why), 'textarea:rows:cols', 'ckeditor:gestimag_zzz:width:height:?:1:rows:cols', 'select:loadmethod:savemethod:buttononly')
	...
	public static function selectArrayAjax($htmlname, $url, $id = '', $moreparam = '', $moreparamtourl = '', $disabled = 0, $minimumInputLength = 1, $morecss = '', $callurlonselect = 0, $placeholder = '', $acceptdelayedhtml = 0)
	{
		...
		// TODO Use an internal gestimag component instead of select2
	...
	public function showCategories($id, $type, $rendermode = 0, $nolink = 0)
	{
		...
		if ($rendermode == 1) {
			...
			foreach ($categories as $c) {
				...
				foreach ($ways as $way) {
					$toprint[] = '<li class="select2-search-choice-gestimag noborderoncategories"' . ($c->color ? ' style="background: #' . $c->color . ';"' : ' style="background: #bbb"') . '>' . $way . '</li>';
				}
			}
			if (empty($toprint)) {
				return '';
			} else {
				return '<div class="select2-container-multi-gestimag"><ul class="select2-choices-gestimag">' . implode(' ', $toprint) . '</ul></div>';
			}
		}

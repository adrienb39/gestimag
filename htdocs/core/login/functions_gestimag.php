...
/**
 *      \file       htdocs/core/login/functions_gestimag.php
 *      \ingroup    core
 *      \brief      Authentication functions for Gestimag mode (check user on login or email and check pass)
 */
 ...
 * Note: On critical error (hack attempt), we put a log "functions_gestimag::check_user_password_gestimag authentication KO"
 *
 ...
 */
function check_user_password_gestimag($usertotest, $passwordtotest, $entitytotest = 1)
{
	...
	if (!empty($usertotest)) {
		...
		dol_syslog("functions_gestimag::check_user_password_gestimag usertotest=".$usertotest." passwordtotest=".preg_replace('/./', '*', $passwordtotest)." entitytotest=".$entitytotest);
		...
		if ($nbevents <= getDolGlobalInt("MAIN_SECURITY_MAX_NUMBER_FAILED_AUTH", 100)) {
			...
			if ($resql) {
				...
				if ($obj) {
					...
					if ($cryptType == 'auto') {
						if ($passcrypted && dol_verifyHash($passtyped, $passcrypted, '0')) {
							$passok = true;
							dol_syslog("functions_gestimag::check_user_password_gestimag Authentication ok - hash ".$cryptType." of pass is ok");
						}
					}
					...
					if (!$passok) {
						if ((!$passcrypted || $passtyped)
							...
							$passok = true;
							dol_syslog("functions_gestimag::check_user_password_gestimag Authentication ok - found old pass in database", LOG_WARNING);
						}
					}
					...
					if ($passok) {
						...
						dol_syslog("functions_gestimag::check_user_password_gestimag Authentication KO bad password for '".$usertotest."', cryptType=".$cryptType, LOG_NOTICE);
					...
					if ($passok && isModEnabled('multicompany')) {	// We must check entity
						...
						if (!isset($mc)) {
							...
							if ($ret < 0) {
								dol_syslog("functions_gestimag::check_user_password_gestimag Authentication KO entity '".$entitytotest."' not allowed for user '".$obj->rowid."'", LOG_NOTICE);
					...
					dol_syslog("functions_gestimag::check_user_password_gestimag Authentication KO user not found for '".$usertotest."'", LOG_NOTICE);
				...
				dol_syslog("functions_gestimag::check_user_password_gestimag Authentication KO db error for '".$usertotest."' error=".$db->lasterror(), LOG_ERR);
				sleep(1);
				$_SESSION["dol_loginmesg"] = $db->lasterror();
			}
		} else {
			dol_syslog("functions_gestimag::check_user_password_gestimag Authentication KO Too many attempts", LOG_NOTICE);
			sleep(1);	// Anti brut force protection. Must be same delay when password is not valid
			// Load translation files required by the page
			$langs->loadLangs(array('main', 'errors'));

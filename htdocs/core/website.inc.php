 ...
 *  				This file is included in top of all container pages (in edit mode, in gestimag web server mode and in external web server mode).
...
if (!empty($pageid) && $pageid > 0) {
	...
	if (empty($srclang)) {
		$reg = array();
		// With Gestimag server, url is in parameter pageref
...
if ($_SERVER['PHP_SELF'] != DOL_URL_ROOT.'/website/index.php') {	// If we browsing page using Gestimag server or a Native web server
...
unset($gestimag_main_instance_unique_id);
unset($gestimag_main_db_host);
unset($gestimag_main_db_port);
unset($gestimag_main_db_name);
unset($gestimag_main_db_user);
unset($gestimag_main_db_pass);
unset($$gestimag_main_db_type);
unset($gestimag_main_document_root);
unset($gestimag_main_document_root_alt);

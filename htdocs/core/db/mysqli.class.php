 ...
 *	\brief      Class file to manage Gestimag database access for a MySQL database
 */

require_once DOL_DOCUMENT_ROOT.'/core/db/DoliDB.class.php';

/**
 *	Class to manage Gestimag database access for a MySQL database using the MySQLi extension
 */
class DoliDBMysqli extends DoliDB
{
	...
	public function __construct($type, $host, $user, $pass, $name = '', $port = 0)
	{
		...
		if (!empty($conf->db->gestimag_main_db_collation)) {
			$this->forcecollate = $conf->db->gestimag_main_db_collation;
		}
		...
		if ($this->connected && $name) {
			if ($this->select_db($name)) {
				...
				if (empty($disableforcecharset) && $this->db->character_set_name() != $clientmustbe) {
					try {
						dol_syslog(get_class($this)."::DoliDBMysqli You should set the \$gestimag_main_db_character_set and \$gestimag_main_db_collation for the PHP to the same as the database default, so to ".$this->db->character_set_name(). " or upgrade database default to ".$clientmustbe.".", LOG_WARNING);
					...
					$collation = (empty($conf) ? 'utf8_unicode_ci' : $conf->db->gestimag_main_db_collation);
			...
			if ($this->connected) {
				...
				if ($this->db->character_set_name() != $clientmustbe) {
					...
					$collation = $conf->db->gestimag_main_db_collation;
					if (preg_match('/latin1/', $collation)) {
						$collation = 'utf8_unicode_ci';
					}
	...
	{
		global $gestimag_main_db_readonly;
		...
		if (!empty($gestimag_main_db_readonly)) {
	...
	public function errno()
	{
		if (!$this->connected) {
			...
			// Constants to convert a MySql error code to a generic Gestimag error code
			$errorcode_map = array(
	...
	public function encrypt($fieldorvalue, $withQuotes = 1)
	{
		...
		$cryptType = (!empty($conf->db->gestimag_main_db_encryption) ? $conf->db->gestimag_main_db_encryption : 0);

		//Encryption key
		$cryptKey = (!empty($conf->db->gestimag_main_db_cryptkey) ? $conf->db->gestimag_main_db_cryptkey : '');
	...
	public function DDLCreateDb($database, $charset = '', $collation = '', $owner = '')
	{
		...
		// ALTER DATABASE gestimag_db DEFAULT CHARACTER SET latin DEFAULT COLLATE latin1_swedish_ci
	 ...
	 *
	 *	@param	string	$gestimag_main_db_host 		Ip server or '%'
	 *	@param	string	$gestimag_main_db_user 		Nom new user
	 *	@param	string	$gestimag_main_db_pass 		Password for the new user
	 *	@param	string	$gestimag_main_db_name		Database name where user must be granted
	 *	@return	int									Return integer <0 if KO, >=0 if OK
	 */
	public function DDLCreateUser($gestimag_main_db_host, $gestimag_main_db_user, $gestimag_main_db_pass, $gestimag_main_db_name)
	{
		// phpcs:enable
		$sql = "CREATE USER '".$this->escape($gestimag_main_db_user)."' IDENTIFIED BY '".$this->escape($gestimag_main_db_pass)."'";
		...
		$sql = "CREATE USER '".$this->escape($gestimag_main_db_user)."'@'localhost' IDENTIFIED BY '".$this->escape($gestimag_main_db_pass)."'";
		$resql = $this->query($sql);

		$sql = "GRANT ALL PRIVILEGES ON ".$this->escape($gestimag_main_db_name).".* TO '".$this->escape($gestimag_main_db_user)."'@'".$this->escape($gestimag_main_db_host)."'";
		dol_syslog(get_class($this)."::DDLCreateUser", LOG_DEBUG); // No sql to avoid password in log
		$resql = $this->query($sql);
		if (!$resql) {
			$this->error = "Connected user not allowed to GRANT ALL PRIVILEGES ON ".$this->escape($gestimag_main_db_name).".* TO '".$this->escape($gestimag_main_db_user)."'@'".$this->escape($gestimag_main_db_host)."'";
			return -1;
		}

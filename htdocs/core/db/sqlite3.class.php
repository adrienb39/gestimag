 ...
 *	\brief      Class file to manage Gestimag database access for a SQLite database
 */

require_once DOL_DOCUMENT_ROOT.'/core/db/DoliDB.class.php';

/**
 *	Class to manage Gestimag database access for a SQLite database
 */
class DoliDBSqlite3 extends DoliDB
{
	...
	public function convertSQLFromMysql($line, $type = 'ddl')
	{
		...
		if ($line != "") {
			...
			if ($type == 'dml') {
				...
				// ALTER TABLE llx_gestimag_modules ADD PRIMARY KEY pk_gestimag_modules (numero, entity);
				...
				// ALTER TABLE llx_gestimag_modules DROP FOREIGN KEY fk_xxx;
	...
	{
		global $conf, $gestimag_main_db_readonly;
	...
	public function errno()
	{
		if (!$this->connected) {
			...
			// Constants to convert error code to a generic Gestimag error code
			/*$errorcode_map = array(
	...
	public function decrypt($value)
	{
		...
		// Type of encryption (2: AES (recommended), 1: DES , 0: no encryption)
		$cryptType = ($conf->db->gestimag_main_db_encryption ? $conf->db->gestimag_main_db_encryption : 0);
	 ...
	 *
	 *	@param	string	$gestimag_main_db_host 		Ip serveur
	 *	@param	string	$gestimag_main_db_user 		Nom user a creer
	 *	@param	string	$gestimag_main_db_pass 		Password user a creer
	...
	public function DDLCreateUser($gestimag_main_db_host, $gestimag_main_db_user, $gestimag_main_db_pass, $gestimag_main_db_name)
	{
		...
		$sql .= " VALUES ('".$this->escape($gestimag_main_db_host)."','".$this->escape($gestimag_main_db_user)."',password('".addslashes($gestimag_main_db_pass)."')";
		...
		$sql .= " VALUES ('".$this->escape($gestimag_main_db_host)."','".$this->escape($gestimag_main_db_name)."','".addslashes($gestimag_main_db_user)."'";
		$sql .= ",'Y','Y','Y','Y','Y','Y','Y','Y','Y')";

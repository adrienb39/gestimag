 ...
 */
class modTakePos extends GestimagModules
{
	/**
	...
	public function __construct($db)
	{
		...
		// Use here a free id (See in Home -> System information -> Gestimag for list of used modules id).
		$this->numero = 50150;
		...
		// Possible values for version are: 'development', 'experimental', 'gestimag', 'gestimag_deprecated' or a version string like 'x.y.z'
		$this->version = 'gestimag';
		...
		$this->need_gestimag_version = array(4, 0); // Minimum version of Gestimag required by module
	 ...
	 *	The init function add constants, boxes, permissions and menus (defined in constructor) into Gestimag database.
	...
	public function init($options = '')
	{
		...
		gestimag_set_const($this->db, "TAKEPOS_PRINT_METHOD", "browser", 'chaine', 0, '', $conf->entity);
		...
		if (!getDolGlobalInt('CASHDESK_ID_THIRDPARTY1')) {	// If a customer has already ben set into the TakePos setup page
			...
			if ($searchcompanyid > 0) {
				...
				gestimag_set_const($this->db, "CASHDESK_ID_THIRDPARTY1", $searchcompanyid, 'chaine', 0, '', $conf->entity);
			} else {
		...
		if (is_array($cate_arbo)) {
			if (!count($cate_arbo) || !getDolGlobalString('TAKEPOS_ROOT_CATEGORY_ID')) {
				...
				if ($result > 0) {
					gestimag_set_const($this->db, 'TAKEPOS_ROOT_CATEGORY_ID', $result, 'chaine', 0, 'Id of category for products visible in TakePOS', $conf->entity);
		...
		if (!getDolGlobalInt('CASHDESK_ID_BANKACCOUNT_CASH1')) {
			...
			if ($searchaccountid > 0) {
				gestimag_set_const($this->db, "CASHDESK_ID_BANKACCOUNT_CASH1", $searchaccountid, 'chaine', 0, '', $conf->entity);
			} else {
	 ...
	 *	Remove from database constants, boxes and permissions from Gestimag database.

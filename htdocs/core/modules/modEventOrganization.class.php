 ...
 *  https://wiki.gestimag.org/index.php/Draft:Module_Event_Organization
 */
class modEventOrganization extends GestimagModules
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->need_gestimag_version = array(13, -3); // Minimum version of Gestimag required by module
	...
	public function init($options = '')
	{
		...
		*/
		...
		if ($template->id > 0) {
			gestimag_set_const($this->db, 'EVENTORGANIZATION_TEMPLATE_EMAIL_ASK_CONF', $template->id, 'chaine', 0, '', $conf->entity);
		}
		$template = $formmail->getEMailTemplate($this->db, 'conferenceorbooth', $user, $langs, 0, 1, '(EventOrganizationEmailAskBooth)');
		if ($template->id > 0) {
			gestimag_set_const($this->db, 'EVENTORGANIZATION_TEMPLATE_EMAIL_ASK_BOOTH', $template->id, 'chaine', 0, '', $conf->entity);
		}
		$template = $formmail->getEMailTemplate($this->db, 'conferenceorbooth', $user, $langs, 0, 1, '(EventOrganizationEmailBoothPayment)');
		if ($template->id > 0) {
			gestimag_set_const($this->db, 'EVENTORGANIZATION_TEMPLATE_EMAIL_AFT_SUBS_BOOTH', $template->id, 'chaine', 0, '', $conf->entity);
		}
		$template = $formmail->getEMailTemplate($this->db, 'conferenceorbooth', $user, $langs, 0, 1, '(EventOrganizationEmailRegistrationPayment)');
		if ($template->id > 0) {
			gestimag_set_const($this->db, 'EVENTORGANIZATION_TEMPLATE_EMAIL_AFT_SUBS_EVENT', $template->id, 'chaine', 0, '', $conf->entity);
		}

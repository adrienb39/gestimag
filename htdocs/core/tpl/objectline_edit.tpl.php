...
?>
	...
	<?php if ($line->fk_product > 0) { ?>
		...
		if (empty($canchangeproduct)) {
			...
			if ($line->product_type == 1) {
				...
				echo img_object($langs->trans('ShowProduct'), 'product');
			}
			echo  ' '.$line->ref; ?>
			</a>
			<?php
			echo  ' - '.nl2br($line->product_label);
			echo '<input type="hidden" id="product_id" name="productid" value="'.(!empty($line->fk_product) ? $line->fk_product : 0).'">';
		} else {
			if ($senderissupplier) {
				$form->select_produits_fournisseurs(!empty($line->fk_product) ? $line->fk_product : 0, 'productid');
			} else {
				echo $form->select_produits(!empty($line->fk_product) ? $line->fk_product : 0, 'productid');
			}
		}
		?>
		<br><br>
	...
	if (!$situationinvoicelinewithparent) {
		...
		echo '<textarea id="product_desc" class="flat" name="product_desc" readonly style="width: 200px; height:80px;">';
		echo GETPOSTISSET('product_desc') ? GETPOST('product_desc', 'restricthtml') : $line->description;
		echo '</textarea>';
	}
	...
	if (!empty($extrafields)) {
		...
		if (!empty($temps)) {
			echo '<div style="padding-top: 10px" id="extrafield_lines_area_edit" name="extrafield_lines_area_edit">';
			echo $temps;
			echo '</div>';
		}
	}
	...
	if ($object->element == 'supplier_proposal' || $object->element == 'order_supplier' || $object->element == 'invoice_supplier' || $object->element == 'invoice_supplier_rec') {	// We must have same test in printObjectLines
		...
		<td class="right linecolrefsupplier"><input id="fourn_ref" name="fourn_ref" class="flat minwidth50 maxwidth100 maxwidth125onsmartphone" value="<?php echo  GETPOSTISSET('fourn_ref') ? GETPOST('fourn_ref') : ($line->ref_supplier ? $line->ref_supplier : $line->ref_fourn); ?>"></td>
					<?php
					echo '<input type="hidden" id="fournprice" name="fournprice"  class="" value="'.$line->fk_fournprice.'">';
	}
	...
	if (!$situationinvoicelinewithparent) {
		echo '<td class="right">';
		echo $form->load_tva('tva_tx', GETPOSTISSET('tva_tx') ? GETPOST('tva_tx', 'alpha') : ($line->tva_tx.($line->vat_src_code ? (' ('.$line->vat_src_code.')') : '')), $seller, $buyer, 0, $line->info_bits, $line->product_type, false, 1, $type_tva);
		echo '</td>';
	} else {
		echo '<td class="right"><input size="1" type="text" class="flat right" name="tva_tx" value="'.price($line->tva_tx).'" readonly />%</td>';
	}

	$coldisplay++;
	echo '<td class="right"><input type="text" class="flat right width50" id="price_ht" name="price_ht" value="'.(GETPOSTISSET('price_ht') ? GETPOST('price_ht', 'alpha') : (isset($line->pu_ht) ? price($line->pu_ht, 0, '', 0) : price($line->subprice, 0, '', 0))).'"';
	if ($situationinvoicelinewithparent) {
		echo ' readonly';
	}
	echo '></td>';
	...
	if (isModEnabled("multicurrency") && $object->multicurrency_code != $conf->currency) {
		$coldisplay++;
		echo '<td class="right"><input rel="'.$object->multicurrency_tx.'" type="text" class="flat right width50" id="multicurrency_subprice" name="multicurrency_subprice" value="'.(GETPOSTISSET('multicurrency_subprice') ? GETPOST('multicurrency_subprice', 'alpha') : price($line->multicurrency_subprice)).'" /></td>';
	}
	...
	if (!empty($inputalsopricewithtax) && !getDolGlobalInt('MAIN_NO_INPUT_PRICE_WITH_TAX')) {
		...
		echo '<td class="right"><input type="text" class="flat right width50" id="price_ttc" name="price_ttc" value="'.(GETPOSTISSET('price_ttc') ? GETPOST('price_ttc') : (isset($upinctax) ? price($upinctax, 0, '', 0) : '')).'"';
		if ($situationinvoicelinewithparent) {
			echo ' readonly';
		}
		echo '></td>';
	...
	if (($line->info_bits & 2) != 2) {
		...
		echo '<input size="3" type="text" class="flat right" name="qty" id="qty" value="'.(GETPOSTISSET('qty') ? GETPOST('qty') : $line->qty).'"';
	...
	if (getDolGlobalString('PRODUCT_USE_UNITS')) {
		...
		$coldisplay++;
		echo '<td class="left">';
		echo $form->selectUnits(GETPOSTISSET('units') ? GETPOST('units') : $line->fk_unit, "units", 0, $unit_type);
		echo '</td>';
	}
	?>
	...
	$coldisplay++;
	if (($line->info_bits & 2) != 2) {
		echo '<input type="text" class="flat right width40" name="remise_percent" id="remise_percent" value="'.(GETPOSTISSET('remise_percent') ? GETPOST('remise_percent') : ($line->remise_percent ? $line->remise_percent : '')).'"';
		if ($situationinvoicelinewithparent) {
			echo ' readonly';
		}
		echo '><span class="hideonsmartphone opacitymedium">%</span>';
	} else { ?>
		&nbsp;
	<?php } ?>
	</td>
	...
	if ($object->situation_cycle_ref) {
		...
		if (getDolGlobalInt('INVOICE_USE_SITUATION') == 2) {
			...
			echo '<td class="nowrap right linecolcycleref"><input class="right" type="text" size="1" value="'.$fieldv.'" name="progress">%</td>';
		} else {
			echo '<td class="nowrap right linecolcycleref"><input class="right" type="text" size="1" value="' . (GETPOSTISSET('progress') ? GETPOST('progress') : $line->situation_percent) . '" name="progress">%</td>';
		}
		$coldisplay++;
		echo '<td></td>';
	}
...
<tr id="service_duration_area" class="treditedlinefordate">
	...
	echo $form->selectDate($line->date_start, 'date_start', $hourmin, $hourmin, $line->date_start ? 0 : 1, "updateline", 1, 0);
	echo ' '.$langs->trans('to').' ';
	echo $form->selectDate($line->date_end, 'date_end', $hourmin, $hourmin, $line->date_end ? 0 : 1, "updateline", 1, 0);
	if ($prefillDates) {
	...
	if (!$line->date_start) {
		if (isset($conf->global->MAIN_DEFAULT_DATE_START_HOUR)) {
			echo 'jQuery("#date_starthour").val("' . getDolGlobalString('MAIN_DEFAULT_DATE_START_HOUR').'");';
		}


		if (isset($conf->global->MAIN_DEFAULT_DATE_START_MIN)) {
			echo 'jQuery("#date_startmin").val("' . getDolGlobalString('MAIN_DEFAULT_DATE_START_MIN').'");';
		}
		...
		if ($res > 0) {
			if ($line->product->isMandatoryPeriod() && $line->product->isService()) {
				echo  'jQuery("#date_start").addClass("inputmandatory");';	// Do not add tag "required", this block the cancel action when value not set
			}
		}
	}
	if (!$line->date_end) {
		if (isset($conf->global->MAIN_DEFAULT_DATE_END_HOUR)) {
			echo 'jQuery("#date_endhour").val("' . getDolGlobalString('MAIN_DEFAULT_DATE_END_HOUR').'");';
		}
		if (isset($conf->global->MAIN_DEFAULT_DATE_END_MIN)) {
			echo 'jQuery("#date_endmin").val("' . getDolGlobalString('MAIN_DEFAULT_DATE_END_MIN').'");';
		}
		...
		if ($res  > 0) {
			if ($line->product->isMandatoryperiod() && $line->product->isService()) {
				echo  'jQuery("#date_end").addClass("inputmandatory");';	// Do not add tag "required", this block the cancel action when value not set
			}
		}
	}
	echo '</script>'

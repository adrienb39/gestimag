...
if (($line->info_bits & 2) == 2) {
	echo '<a href="'.DOL_URL_ROOT.'/comm/remx.php?id='.$this->socid.'">';
	$txt = '';
	...
	if ($line->fk_product > 0) {
		if (getDolGlobalInt('MAIN_ENABLE_AJAX_TOOLTIP')) {
			echo (!empty($line->fk_parent_line) ? img_picto('', 'rightarrow') : '') . $text;
			if (!getDolGlobalInt('PRODUIT_DESC_IN_FORM')) {
				echo $form->textwithpicto('', $description);
			}
		} else {
			echo $form->textwithtooltip($text, $description, 3, 0, '', $i, 0, (!empty($line->fk_parent_line) ? img_picto('', 'rightarrow') : ''));
		}
	} else {
		...
		if (!empty($line->label)) {
			...
			echo $form->textwithtooltip($text, dol_htmlentitiesbr($line->description), 3, 0, '', $i, 0, (!empty($line->fk_parent_line) ? img_picto('', 'rightarrow') : ''));
		} else {
	...
	if ($line->element == 'facturedetrec' || $line->element == 'invoice_supplier_det_rec') {
		...
		if ($line->date_start_fill || $line->date_end_fill) {
			echo '<div class="clearboth nowraponall daterangeofline-facturedetrec">';
		}
		if ($line->date_start_fill) {
			echo '<span class="opacitymedium" title="'.dol_escape_htmltag($langs->trans("AutoFillDateFrom")).'">'.$langs->trans('AutoFillDateFromShort').':</span> '.yn($line->date_start_fill);
		}
		if ($line->date_start_fill && $line->date_end_fill) {
			echo ' - ';
		}
		if ($line->date_end_fill) {
			echo '<span class="opacitymedium" title="'.dol_escape_htmltag($langs->trans("AutoFillDateTo")).'">'.$langs->trans('AutoFillDateToShort').':</span> '.yn($line->date_end_fill);
		}
		if ($line->date_start_fill || $line->date_end_fill) {
			echo '</div>';
		}
		...
		if ($line->date_start || $line->date_end) {
			echo '<div class="clearboth nowraponall opacitymedium daterangeofline">'.get_date_range($line->date_start, $line->date_end, $format).'</div>';
		}
			...
			if ($object->element == 'propal' || $object->element == 'order' || $object->element == 'facture' || $object->element == 'propal_supplier' || $object->element == 'supplier_proposal' || $object->element == 'commande') {
				...
				if ($res  > 0) {
					if ($line->product->isService() && $line->product->isMandatoryPeriod()) {
						echo '<div><span class="clearboth nowraponall warning">'.$langs->trans("mandatoryPeriodNeedTobeSet").'</span></div>';
					}
				}
			}
		}
		...
		if (basename($_SERVER["PHP_SELF"]) == 'card-rec.php') {
			...
			echo '<div class="clearboth nowraponall daterangeofline-facturedetrec">';
			echo '<span class="opacitymedium" title="'.dol_escape_htmltag($langs->trans("AutoFillDateFrom")).'">'.$langs->trans('AutoFillDateFromShort').':</span> '.yn($default_start_fill);
			echo ' - ';
			echo '<span class="opacitymedium" title="'.dol_escape_htmltag($langs->trans("AutoFillDateTo")).'">'.$langs->trans('AutoFillDateToShort').':</span> '.yn($default_end_fill);
			echo '</div>';
		}
	}
	...
	if ($line->fk_product > 0 && getDolGlobalInt('PRODUIT_DESC_IN_FORM_ACCORDING_TO_DEVICE')) {
		if ($line->element == 'facturedetrec') {
			echo (!empty($line->description) && $line->description != $line->product_label) ? (($line->date_start_fill || $line->date_end_fill) ? '' : '<br>').'<br>'.dol_htmlentitiesbr($line->description) : '';
		} elseif ($line->element == 'invoice_supplier_det_rec') {
			echo (!empty($line->description) && $line->description != $line->label) ? (($line->date_start || $line->date_end) ? '' : '<br>').'<br>'.dol_htmlentitiesbr($line->description) : '';
		} else {
			echo (!empty($line->description) && $line->description != $line->product_label) ? (($line->date_start || $line->date_end) ? '' : '<br>').'<br>'.dol_htmlentitiesbr($line->description) : '';
		}
	}
	...
	if (!empty($extrafields)) {
		...
		if (!empty($temps)) {
			echo '<div style="padding-top: 10px" id="extrafield_lines_area_'.$line->id.'" name="extrafield_lines_area_'.$line->id.'">';
			echo $temps;
			echo '</div>';
		}
	}
...
if ($user->hasRight('fournisseur', 'lire') && isset($line->fk_fournprice) && $line->fk_fournprice > 0 && !getDolGlobalString('SUPPLIER_HIDE_SUPPLIER_OBJECTLINES')) {
	...
	echo '<span class="opacitymedium">'.$langs->trans('Supplier').' : </span>'.$productfourn->getSocNomUrl(1, 'supplier').' - <span class="opacitymedium">'.$langs->trans('Ref').' : </span>';
	// Supplier ref
...
if ($object->element == 'supplier_proposal' || $object->element == 'order_supplier' || $object->element == 'invoice_supplier' || $object->element == 'invoice_supplier_rec') {	// We must have same test in printObjectLines
	echo '<td class="linecolrefsupplier">';
...
// VAT Rate
echo '<td class="linecolvat nowrap right">';
$coldisplay++;
$positiverates = '';
...
echo $tooltiponpriceend;
?></td>

	<td class="linecoluht nowraponall right"><?php $coldisplay++; ?><?php echo price($sign * $line->subprice); ?></td>

<?php if (isModEnabled("multicurrency") && $this->multicurrency_code != $conf->currency) { ?>
	<td class="linecoluht_currency nowraponall right"><?php $coldisplay++; ?><?php echo price($sign * $line->multicurrency_subprice); ?></td>
<?php }
...
if ((($line->info_bits & 2) != 2) && $line->special_code != 3) {
	...
	//if($line->qty > $line->stock) echo img_picto($langs->trans("StockTooLow"),"warning", 'style="vertical-align: bottom;"')." ";
...
if (getDolGlobalString('PRODUCT_USE_UNITS')) {
	echo '<td class="linecoluseunit nowrap left">';
...
if (!empty($line->remise_percent) && $line->special_code != 3) {
	echo '<td class="linecoldiscount right">';
	$coldisplay++;
	include_once DOL_DOCUMENT_ROOT.'/core/lib/functions2.lib.php';
	echo dol_print_reduction($line->remise_percent, $langs);
	echo '</td>';
} else {
	echo '<td class="linecoldiscount">&nbsp;</td>';
	$coldisplay++;
}
...
if (isset($this->situation_cycle_ref) && $this->situation_cycle_ref) {
	...
	if (getDolGlobalInt('INVOICE_USE_SITUATION') == 2) {
		...
		echo '<td class="linecolcycleref nowrap right">'.$current_progress.'%</td>';
		$coldisplay++;
		echo '<td  class="nowrap right">'.$line->situation_percent.'%</td>';
		$coldisplay++;
		...
		echo '<td class="linecolcycleref2 right nowrap">'.price($sign * $tmp[0]).'</td>';
	} else {
		echo '<td class="linecolcycleref nowrap right">'.$line->situation_percent.'%</td>';
		$coldisplay++;
		$locataxes_array = getLocalTaxesFromRate($line->tva.($line->vat_src_code ? ' ('.$line->vat_src_code.')' : ''), 0, ($senderissupplier ? $mysoc : $object->thirdparty), ($senderissupplier ? $object->thirdparty : $mysoc));
		$tmp = calcul_price_total($line->qty, $line->pu, $line->remise_percent, $line->txtva, -1, -1, 0, 'HT', $line->info_bits, $line->type, ($senderissupplier ? $object->thirdparty : $mysoc), $locataxes_array, 100, $object->multicurrency_tx, $line->multicurrency_subprice);
		echo '<td class="linecolcycleref2 right nowrap">'.price($sign * $tmp[0]).'</td>';
...
if ($line->special_code == 3) {
	...
	echo '<td class="linecoloption nowrap right"'.$colspanOptions.'>'.$langs->trans('Option').'</td>';
} else {
	echo '<td class="linecolht nowrap right">';
	$coldisplay++;
	echo $tooltiponprice;
	echo price($sign * $line->total_ht);
	echo $tooltiponpriceend;
	echo '</td>';
	if (isModEnabled("multicurrency") && $this->multicurrency_code != $conf->currency) {
		echo '<td class="linecolutotalht_currency nowrap right">'.price($sign * $line->multicurrency_total_ht).'</td>';
		$coldisplay++;
	}
}
...
if ($outputalsopricetotalwithtax) {
	echo '<td class="linecolht nowrap right">'.price($sign * $line->total_ttc).'</td>';
	$coldisplay++;
}
...
if ($this->status == 0 && !empty($object_rights->creer) && $action != 'selectlines') {
	...
	if (isModEnabled('asset') && $object->element == 'invoice_supplier') {
		echo '<td class="linecolasset center">';
		$coldisplay++;
		if (!empty($product_static->accountancy_code_buy) ||
			...
			if (!$resql_asset) {
				echo 'Error SQL: ' . $this->db->lasterror();
			} elseif ($obj = $this->db->fetch_object($resql_asset)) {
				if (!empty($obj->found)) {
					echo '<a class="reposition" href="' . DOL_URL_ROOT . '/asset/card.php?action=create&token='.newToken().'&supplier_invoice_id='.$object->id.'">';
					echo img_edit_add() . '</a>';
				}
			}
		}
	...
	if (($line->info_bits & 2) == 2 || !empty($disableedit)) {
		...
		<a class="editfielda reposition" href="<?php echo $_SERVER["PHP_SELF"].'?id='.$this->id.'&action=editline&token='.newToken().'&lineid='.$line->id; ?>">
		<?php echo img_edit().'</a>';
	}
	echo '</td>';
	...
	if (!$situationinvoicelinewithparent && empty($disableremove)) { // For situation invoice, deletion is not possible if there is a parent company.
		echo '<a class="reposition" href="'.$_SERVER["PHP_SELF"].'?id='.$this->id.'&action=ask_deleteline&token='.newToken().'&lineid='.$line->id.'">';
		echo img_delete();
		echo '</a>';
	}
	echo '</td>';
	...
	if ($num > 1 && $conf->browser->layout != 'phone' && ((property_exists($this, 'situation_counter') && $this->situation_counter == 1) || empty($this->situation_cycle_ref)) && empty($disablemove)) {
		...
		$coldisplay++;
		if ($i > 0) { ?>
			<a class="lineupdown" href="<?php echo $_SERVER["PHP_SELF"].'?id='.$this->id.'&action=up&token='.newToken().'&rowid='.$line->id; ?>">
			<?php echo img_up('default', 0, 'imgupforline'); ?>
			</a>
		<?php }
		if ($i < $num - 1) { ?>
			<a class="lineupdown" href="<?php echo $_SERVER["PHP_SELF"].'?id='.$this->id.'&action=down&token='.newToken().'&rowid='.$line->id; ?>">
...
echo "<!-- END PHP TEMPLATE objectline_view.tpl.php -->\n";

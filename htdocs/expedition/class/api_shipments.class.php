 ...
 */
class Shipments extends GestimagApi
{
	/**
	...
	public function get($id)
	{
		...
		if (!GestimagApi::_checkAccessToResource('expedition', $this->shipment->id)) {
	...
	public function post($request_data = null)
	{
		...
		if ($this->shipment->create(GestimagApiAccess::$user) < 0) {
	...
	*/
	...
	public function deleteLine($id, $lineid)
	{
		...
		$updateRes = $this->shipment->deleteLine(GestimagApiAccess::$user, $lineid);
		if ($updateRes > 0) {
			return array(
				'success' => array(
					'code' => 200,
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->shipment->update(GestimagApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('expedition', 'supprimer')) {
			throw new RestException(403);
		}
		$result = $this->shipment->fetch($id);
		if (!$result) {
		...
		if (!$this->shipment->delete(GestimagApiAccess::$user)) {
	...
	public function validate($id, $notrigger = 0)
	{
		...
		$result = $this->shipment->valid(GestimagApiAccess::$user, $notrigger);
		if ($result == 0) {
	...
	public function setinvoiced($id)
	{
		...
		$result = $this->shipment->classifyBilled(GestimagApiAccess::$user);
		if( $result < 0) {
	...
	*/
	...
	public function createShipmentFromOrder($orderid)
	{
		...
		$result = $this->shipment->createFromOrder($order, GestimagApiAccess::$user);
		if( $result < 0) {
				throw new RestException(405, $this->shipment->error);
		}
		$this->shipment->fetchObjectLinked();

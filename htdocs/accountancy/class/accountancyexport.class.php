...
class AccountancyExport
{
	...
	public function exportCiel($objectLines, $exportFile = null)
	{
		...
		foreach ($objectLines as $line) {
			...
			$tab[] = str_repeat(' ', 18); // Analytical accounting - Not managed in Gestimag
	 ...
	 * Help to import in Quadra: https://wiki.gestimag.org/index.php?title=Module_Comptabilit%C3%A9_en_Partie_Double#Import_vers_CEGID_Quadra
					...
					if (!empty($fileFoundList)) {
						...
						foreach ($fileFoundList as $fileFound) {
							if (strstr($fileFound['name'], $objectFileName)) {
								...
								// We want to retrieve an attachment representative of the supplier invoice, not a fake document generated by Gestimag.

...
if ($action == 'update') {
	...
	if (!empty($defaultjournal)) {
		if (!gestimag_set_const($db, 'ACCOUNTING_CLOSURE_DEFAULT_JOURNAL', $defaultjournal, 'chaine', 0, '', $conf->entity)) {
			$error++;
		}
	} else {
		$error++;
	...
	if (!empty($accountinggroupsusedforbalancesheetaccount)) {
		if (!gestimag_set_const($db, 'ACCOUNTING_CLOSURE_ACCOUNTING_GROUPS_USED_FOR_BALANCE_SHEET_ACCOUNT', $accountinggroupsusedforbalancesheetaccount, 'chaine', 0, '', $conf->entity)) {
			$error++;
		}
	} else {
		$error++;
	...
	if (!empty($accountinggroupsusedforincomestatement)) {
		if (!gestimag_set_const($db, 'ACCOUNTING_CLOSURE_ACCOUNTING_GROUPS_USED_FOR_INCOME_STATEMENT', $accountinggroupsusedforincomestatement, 'chaine', 0, '', $conf->entity)) {
			$error++;
		}
	} else {
		$error++;

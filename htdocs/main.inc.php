 ...
 *	\ingroup	core
 *	\brief      File that defines environment for Gestimag GUI pages only (file not required by scripts)
 */
...
if (GETPOST("DOL_AUTOSET_COOKIE")) {
	...
	if (PHP_VERSION_ID < 70300) {
		setcookie($cookiename, empty($cookievalue) ? '' : $cookievalue, empty($cookievalue) ? 0 : (time() + (86400 * 354)), '/', '', ((empty($gestimag_main_force_https) && isHTTPS() === false) ? false : true), true); // keep cookie 1 year and add tag httponly
	} else {
		...
		$cookieparams = array(
			...
			'secure' => ((empty($gestimag_main_force_https) && isHTTPS() === false) ? false : true),
			'httponly' => true,
...
// Init session. Name of session is specific to Gestimag instance.
// Must be done after the include of filefunc.inc.php so global variables of conf file are defined (like $gestimag_main_instance_unique_id or $gestimag_main_force_https).
...
if (!defined('NOSESSION')) {
	if (PHP_VERSION_ID < 70300) {
		session_set_cookie_params(0, '/', null, ((empty($gestimag_main_force_https) && isHTTPS() === false) ? false : true), true); // Add tag secure and httponly on session cookie (same as setting session.cookie_httponly into php.ini). Must be called before the session_start.
	} else {
		// Only available for php >= 7.3
...
// Force HTTPS if required ($conf->file->main_force_https is 0/1 or 'https gestimag root url')
...
if (!empty($conf->file->main_force_https) && !isHTTPS() && !defined('NOHTTPSREDIRECT')) {
	...
	if ($newurl) {
		...
		dol_syslog("main.inc: gestimag_main_force_https is on, we make a redirect to ".$newurl);
		header("Location: ".$newurl);
		exit;
	} else {
		dol_syslog("main.inc: gestimag_main_force_https is on but we failed to forge new https url so no redirect is done", LOG_WARNING);
	}
}

if (!defined('NOLOGIN') && !defined('NOIPCHECK') && !empty($gestimag_main_restrict_ip)) {
	$listofip = explode(',', $gestimag_main_restrict_ip);
	$found = false;
...
if ($checkifupgraderequired) {
	...
	$gestimagversionlastupgrade = preg_split('/[.-]/', $versiontocompare);
	$gestimagversionprogram = preg_split('/[.-]/', DOL_VERSION);
	$rescomp = versioncompare($gestimagversionprogram, $gestimagversionlastupgrade);
...
//dol_syslog("CSRF info: ".defined('NOCSRFCHECK')." - ".$gestimag_nocsrfcheck." - ".$conf->global->MAIN_SECURITY_CSRF_WITH_TOKEN." - ".$_SERVER['REQUEST_METHOD']." - ".GETPOST('token', 'alpha'));
...
if ((!defined('NOCSRFCHECK') && empty($gestimag_nocsrfcheck) && getDolGlobalInt('MAIN_SECURITY_CSRF_WITH_TOKEN')) || defined('CSRFCHECK_WITH_TOKEN')) {
		...
		if (!GETPOST('token', 'alpha') || GETPOST('token', 'alpha') == 'notrequired') {
			...
			if (GETPOSTINT('uploadform')) {
				...
				if (defined('CSRFCHECK_WITH_TOKEN')) {
					...
					print "If you access your server behind a proxy using url rewriting and the parameter is provided by caller, you might check that all HTTP header are propagated (or add the line \$gestimag_nocsrfcheck=1 into your conf.php file or MAIN_SECURITY_CSRF_WITH_TOKEN to 0";
...
if (!defined('NOLOGIN')) {
	...
	// Example: 'http', 'gestimag', 'ldap', 'http,forceuser', '...'

	if (defined('MAIN_AUTHENTICATION_MODE')) {
		$gestimag_main_authentication = constant('MAIN_AUTHENTICATION_MODE');
	} else {
		// Authentication mode
		if (empty($gestimag_main_authentication)) {
			$gestimag_main_authentication = 'gestimag';
		}
		// Authentication mode: forceuser
		if ($gestimag_main_authentication == 'forceuser' && empty($gestimag_auto_user)) {
			$gestimag_auto_user = 'auto';
		}
	}
	// Set authmode
	$authmode = explode(',', $gestimag_main_authentication);

	// No authentication mode
	if (!count($authmode)) {
		$langs->load('main');
		dol_print_error(null, $langs->trans("ErrorConfigParameterNotDefined", 'gestimag_main_authentication'));
		exit;
	}
	...
	if (!isset($_SESSION["dol_login"])) {
		...
		if (!empty($gestimag_main_demo) && $_SERVER['PHP_SELF'] == DOL_URL_ROOT.'/index.php') {  // We ask index page
		...
		// TODO Remove use of $_COOKIE['login_gestimag'] ? Replace $usertotest = with $usertotest = GETPOST("username", "alpha", $allowedmethodtopostusername);
		$usertotest = (!empty($_COOKIE['login_gestimag']) ? preg_replace('/[^a-zA-Z0-9_@\-\.]/', '', $_COOKIE['login_gestimag']) : GETPOST("username", "alpha", $allowedmethodtopostusername));
		...
		if ($gestimag_main_authentication == 'forceuser' && !empty($gestimag_auto_user)) {	// For automatic login with a forced user
			$goontestloop = true;
		}
		...
		if (!empty($_COOKIE['login_gestimag'])) {	// TODO For ? Remove this ?
			$goontestloop = true;
		}
		...
		if ($test && $goontestloop && (GETPOST('actionlogin', 'aZ09') == 'login' || $gestimag_main_authentication != 'gestimag')) {
			// Loop on each test mode defined into $authmode
			// $authmode is an array for example: array('0'=>'gestimag', '1'=>'googleoauth');
		...
		if ($resultFetchUser <= 0 || $user->isNotIntoValidityDateRange()) {
			...
			session_destroy();
			session_set_cookie_params(0, '/', null, (empty($gestimag_main_force_https) ? false : true), true); // Add tag secure and httponly on session cookie
			session_name($sessionname);
			dol_session_start();
			...
			if ($resultFetchUser == 0) {
				...
				$_SESSION["dol_loginmesg"] = $langs->transnoentitiesnoconv("ErrorCantLoadUserFromGestimagDatabase", $login);

				$user->context['audit'] = 'ErrorCantLoadUserFromGestimagDatabase - login='.$login;
...
function top_htmlhead($head, $title = '', $disablejs = 0, $disablehead = 0, $arrayofjs = array(), $arrayofcss = array(), $disableforlogin = 0, $disablenofollow = 0, $disablenoindex = 0)
{
	...
	if (empty($disablehead)) {
		...
		print '<meta name="author" content="Gestimag Development Team">'."\n";
		...
		// Favicon
		$favicon = DOL_URL_ROOT.'/theme/gestimag_256x256_color.png';
		...
		if (GETPOST('dol_resetcache')) {
			...
			gestimag_set_const($db, "MAIN_IHM_PARAMS_REV", getDolGlobalInt('MAIN_IHM_PARAMS_REV') + 1, 'chaine', 0, '', $conf->entity);
		}
...
function top_menu($head, $title = '', $target = '', $disablejs = 0, $disablehead = 0, $arrayofjs = array(), $arrayofcss = array(), $morequerystring = '', $helppagename = '')
{
	...
	global $gestimag_main_authentication, $gestimag_main_demo;
...
function top_menu_user($hideloginname = 0, $urllogout = '')
{
	...
	$dropdownBody .= '<br><b>'.$langs->trans("AuthenticationMode").':</b> '.$_SESSION["dol_authmode"].(empty($gestimag_main_demo) ? '' : ' (demo)');
...
function left_menu($menu_array_before, $helppagename = '', $notused = '', $menu_array_after = array(), $leftmenuwithoutmainarea = 0, $title = '', $acceptdelayedhtml = 0)
{
	...
	if (empty($conf->dol_hide_leftmenu) && (!defined('NOREQUIREMENU') || !constant('NOREQUIREMENU'))) {
		...
		// Gestimag version + help + bug report link
		print "\n";
		...
		if (getDolGlobalString('MAIN_SHOW_VERSION')) {    // Version is already on help picto and on login page.
			$doliurl = 'https://www.gestimag.org';
			//local communities
			if (preg_match('/fr/i', $langs->defaultlang)) {
				$doliurl = 'https://www.gestimag.fr';
			}
			if (preg_match('/es/i', $langs->defaultlang)) {
				$doliurl = 'https://www.gestimag.es';
			}
			if (preg_match('/de/i', $langs->defaultlang)) {
				$doliurl = 'https://www.gestimag.de';
			}
			if (preg_match('/it/i', $langs->defaultlang)) {
				$doliurl = 'https://www.gestimag.it';
			}
			if (preg_match('/gr/i', $langs->defaultlang)) {
				$doliurl = 'https://www.gestimag.gr';
			}
		...
		if (getDolGlobalString('MAIN_BUGTRACK_ENABLELINK')) {
			...
			if (getDolGlobalString('MAIN_BUGTRACK_ENABLELINK') == 'github') {
				$bugbaseurl = 'https://github.com/Gestimag/gestimag/issues/new?labels=Bug';
				$bugbaseurl .= '&title=';
				...
				$bugbaseurl .= urlencode("[*Verbose description*]\n");
				$bugbaseurl .= urlencode("\n");
				$bugbaseurl .= urlencode("## [Attached files](https://help.github.com/articles/issue-attachments) (Screenshots, screencasts, gestimag.log, debugging information…)\n");
...
function getHelpParamFor($helppagename, $langs)
{
	...
	if (preg_match('/^http/i', $helppagename)) {
		...
		$reg = array();
		if (preg_match('/^es/i', $langs->defaultlang)) {
			$helpbaseurl = 'http://wiki.gestimag.org/index.php/%s';
...
if (!function_exists("llxFooter")) {
	...
	function llxFooter($comment = '', $zone = 'private', $disabledoutputofmessages = 0)
	{
		...
		if (($_SERVER["PHP_SELF"] == DOL_URL_ROOT.'/index.php') || $forceping) {
			...
			$hash_unique_id = dol_hash('gestimag'.$conf->file->instance_unique_id, 'sha256');	// Note: if the global salt changes, this hash changes too so ping may be counted twice. We don't mind. It is for statistics purpose only.
			...
			|| $forceping) {
				...
				if (strpos('alpha', DOL_VERSION) > 0 && !$forceping) {
					...
					if (getDolGlobalString('MAIN_LAST_PING_KO_DATE') && substr($conf->global->MAIN_LAST_PING_KO_DATE, 0, 6) == dol_print_date(dol_now(), '%Y%m') && !$forceping) {
						...
						print "\n".'<!-- Includes JS for Ping of Gestimag forceping='.$forceping.' MAIN_FIRST_PING_OK_DATE='.getDolGlobalString("MAIN_FIRST_PING_OK_DATE").' MAIN_FIRST_PING_OK_ID='.getDolGlobalString("MAIN_FIRST_PING_OK_ID").' MAIN_LAST_PING_KO_DATE='.getDolGlobalString("MAIN_LAST_PING_KO_DATE").' -->'."\n";
						print "\n<!-- JS CODE TO ENABLE the anonymous Ping -->\n";
						$url_for_ping = getDolGlobalString('MAIN_URL_FOR_PING', "https://ping.gestimag.org/");
						// Try to guess the distrib used
						$distrib = 'standard';
						if ($_SERVER["SERVER_ADMIN"] == 'doliwamp@localhost') {
							$distrib = 'doliwamp';
						}
						if (!empty($gestimag_distrib)) {
							$distrib = $gestimag_distrib;
						}
						?>
							<script>
							jQuery(document).ready(function (tmp) {
								console.log("Try Ping with hash_unique_id is dol_hash('gestimag'+instance_unique_id, 'sha256')");
								$.ajax({
									  method: "POST",
					...
					gestimag_set_const($db, 'MAIN_FIRST_PING_OK_ID', 'disabled', 'chaine', 0, '', $conf->entity);
				}
			}
		}

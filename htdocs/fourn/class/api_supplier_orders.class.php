 ...
 */
class SupplierOrders extends GestimagApi
{
	/**
	 *
	...
	{
		if (!GestimagApiAccess::$user->hasRight("fournisseur", "commande", "lire")) {
		...
		if (!GestimagApi::_checkAccessToResource('fournisseur', $this->order->id, 'commande_fournisseur', 'commande')) {
	...
	public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $thirdparty_ids = '', $product_ids = '', $status = '', $sqlfilters = '', $sqlfilterlines = '', $properties = '')
	{
		...
		$search_sale = 0;
		if (!GestimagApiAccess::$user->hasRight("societe", "client", "voir") && !empty($socids)) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight("fournisseur", "commande", "creer") && !GestimagApiAccess::$user->hasRight("supplier_order", "creer")) {
		...
		if ($this->order->create(GestimagApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->order->update(GestimagApiAccess::$user)) {
			return $this->get($id);
		}
	...
	{
		if (!GestimagApiAccess::$user->hasRight("fournisseur", "commande", "creer")) {
			throw new RestException(403);
		}
	...
	{
		if (!GestimagApiAccess::$user->hasRight("fournisseur", "commande", "supprimer")) {
			throw new RestException(403);
		}
		$result = $this->order->fetch($id);
		if (!$result) {
		...
		if ($this->order->delete(GestimagApiAccess::$user) < 0) {
	...
	public function validate($id, $idwarehouse = 0, $notrigger = 0)
	{
		...
		$result = $this->order->valid(GestimagApiAccess::$user, $idwarehouse, $notrigger);
		if ($result == 0) {
	...
	public function approve($id, $idwarehouse = 0, $secondlevel = 0)
	{
		...
		$result = $this->order->approve(GestimagApiAccess::$user, $idwarehouse, $secondlevel);
		if ($result == 0) {
	...
	public function makeOrder($id, $date, $method, $comment = '')
	{
		...
		$result = $this->order->commande(GestimagApiAccess::$user, $date, $method, $comment);
		if ($result == 0) {
	...
	public function receiveOrder($id, $closeopenorder, $comment, $lines)
	{
		...
		$result = $this->order->calcAndSetStatusDispatch(GestimagApiAccess::$user, $closeopenorder, $comment);

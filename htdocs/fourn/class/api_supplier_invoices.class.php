 ...
 */
class SupplierInvoices extends GestimagApi
{
	/**
	 *
	...
	{
		if (!GestimagApiAccess::$user->hasRight("fournisseur", "facture", "lire")) {
			throw new RestException(403);
		}

		if (!GestimagApi::_checkAccessToResource('fournisseur', $id, 'facture_fourn', 'facture')) {
	...
	public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $thirdparty_ids = '', $status = '', $sqlfilters = '', $properties = '')
	{
		...
		$search_sale = 0;
		if (!GestimagApiAccess::$user->hasRight("societe", "client", "voir")) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight("fournisseur", "facture", "creer")) {
		...
		if ($this->invoice->create(GestimagApiAccess::$user) < 0) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight("fournisseur", "facture", "supprimer")) {
			throw new RestException(403);
		}
		if (!GestimagApi::_checkAccessToResource('fournisseur', $id, 'facture_fourn', 'facture')) {
			throw new RestException(403, 'Access not allowed for login ' . GestimagApiAccess::$user->login);
		...
		if ($this->invoice->delete(GestimagApiAccess::$user) < 0) {
	...
	public function validate($id, $idwarehouse = 0, $notrigger = 0)
	{
		...
		$result = $this->invoice->validate(GestimagApiAccess::$user, '', $idwarehouse, $notrigger);
		if ($result == 0) {
	...
	public function addPayment($id, $datepaye, $payment_mode_id, $closepaidinvoices, $accountid, $num_payment = '', $comment = '', $chqemetteur = '', $chqbank = '', $amount = null)
	{
		...
		$paiement_id = $paiement->create(GestimagApiAccess::$user, ($closepaidinvoices == 'yes' ? 1 : 0)); // This include closing invoices
		if ($paiement_id < 0) {
			$this->db->rollback();
		...
		if (isModEnabled("bank")) {
			$result = $paiement->addPaymentToBank(GestimagApiAccess::$user, 'payment_supplier', '(SupplierInvoicePayment)', $accountid, $chqemetteur, $chqbank);
			if ($result < 0) {
				$this->db->rollback();
	 ...
	 *
	 * Note: socid = gestimag_order_id, pu_ht = net price, remise = discount
	 *

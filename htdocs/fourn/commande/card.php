...
if (empty($reshook)) {
	...
	if ($action == 'webservice' && $permissiontoadd && GETPOST('mode', 'alpha') == "send" && !GETPOST('cancel', 'alpha')) {
		...
		$ws_ns = 'http://www.gestimag.org/ns/';
		$ws_authentication = array(
			'gestimagkey' => $ws_key,
			'sourceapplication' => 'GestimagWebServiceClient',
			'login' => $ws_user,
...
if ($action == 'create') {
	...
	if (empty($reshook)) {
		...
		print '<td>';
		$doleditor = new DolEditor('note_public', isset($note_public) ? $note_public : GETPOST('note_public', 'restricthtml'), '', 80, 'gestimag_notes', 'In', 0, false, !getDolGlobalString('FCKEDITOR_ENABLE_NOTE_PUBLIC') ? 0 : 1, ROWS_3, '90%');
		...
		print '<td>';
		$doleditor = new DolEditor('note_private', isset($note_private) ? $note_private : GETPOST('note_private', 'restricthtml'), '', 80, 'gestimag_notes', 'In', 0, false, !getDolGlobalString('FCKEDITOR_ENABLE_NOTE_PRIVATE') ? 0 : 1, ROWS_3, '90%');
		print $doleditor->Create(1);
		print '</td>';
				...
				if ($user_status_code == "OK") {
					...
					if (empty($ws_thirdparty)) {
						...
						foreach ($object->lines as $line) {
							...
							$supplier_price = price($result_product["product"]["price_net"]); //Price of client tab in supplier gestimag
							$local_price = null; //Price of supplier as stated in product suppliers tab on this gestimag, NULL if not found

...
class EmailCollector extends CommonObject
{
	...
	public function doCollectOneCollector($mode = 0)
	{
		...
		//$conf->global->SYSLOG_FILE = 'DOL_DATA_ROOT/gestimag_mydedicatedlofile.log';
		...
		if (!$error && !empty($arrayofemail) && count($arrayofemail) > 0) {
			...
			foreach ($richarrayofemail as $tmpval) {
				...
				// For example:
				// References: <1542377954.SMTPs-gestimag-thi649@8f6014fde11ec6cdec9a822234fc557e>
				// References: <1542377954.SMTPs-gestimag-tic649@8f6014fde11ec6cdec9a822234fc557e>
				// References: <1542377954.SMTPs-gestimag-abc649@8f6014fde11ec6cdec9a822234fc557e>
				$trackid = '';
				$objectid = 0;
				$objectemail = null;
				...
				foreach ($arrayofreferences as $reference) {
					...
					if (!empty($trackidfoundintorecipienttype)) {
						...
						$resultsearchtrackid = preg_match('/gestimag-([a-z]+)([0-9]+)@'.preg_quote($host, '/').'/', $reference, $reg);	// trackid found or not
						if (empty($resultsearchtrackid) && getDolGlobalString('EMAIL_ALTERNATIVE_HOST_SIGNATURE')) {
							$resultsearchtrackid = preg_match('/gestimag-([a-z]+)([0-9]+)@'.preg_quote(getDolGlobalString('EMAIL_ALTERNATIVE_HOST_SIGNATURE'), '/').'/', $reference, $reg);	// trackid found
						}
					}

					if (!empty($resultsearchtrackid)) {
						// We found a tracker (in recipient email or msgid or into a Reference matching the Gestimag server)
						$trackid = $reg[1].$reg[2];
						...
						if ($reg[1] == 'proj') {   // Project
							...
							$projectfoundby = 'TrackID gestimag-'.$trackid.'@...';
						}
						if ($reg[1] == 'tas') {   // Task
				...
				if ($mode < 2) {	// 0=Mode production, 1=Mode test (read IMAP and try SQL update then rollback), 2=Mode test with no SQL updates
					foreach ($this->actions as $operation) {
						...
						dol_syslog("Execute action fk_element_id=".$fk_element_id." fk_element_type=".$fk_element_type);	// If a Gestimag tracker id is found, we should now the id of object
							...
							if ($ticketalreadyexists == 0) {
								...
								if ($errorforthisaction) {
									...
									if (is_numeric($tickettocreate->ref) && $tickettocreate->ref <= 0) {
										...
										// Create ticket
										$tickettocreate->context['actionmsg2'] = $langs->trans("ActionAC_EMAIL_IN").' - '.$langs->trans("TICKET_CREATEInGestimag");
										$tickettocreate->context['actionmsg'] = $langs->trans("ActionAC_EMAIL_IN").' - '.$langs->trans("TICKET_CREATEInGestimag");

...
if ($action == 'edit') {
	...
	if (!empty($object->multilangs)) {
		foreach ($object->multilangs as $key => $value) {
			...
			$doleditor = new DolEditor("desc-$key", $desc, '', 160, 'gestimag_notes', '', false, true, isModEnabled('fckeditor') && getDolGlobalInt('FCKEDITOR_ENABLE_SOCIETE'), ROWS_3, '90%');
			$doleditor->Create();
			print '</td></tr>';
...
if ($action == 'add' && ($user->hasRight('produit', 'creer') || $user->hasRight('service', 'creer'))) {
	...
	$doleditor = new DolEditor('desc', GETPOST('desc', 'restricthtml'), '', 160, 'gestimag_notes', '', false, true, isModEnabled('fckeditor') && getDolGlobalInt('FCKEDITOR_ENABLE_SOCIETE'), ROWS_3, '90%');
	$doleditor->Create();
	print '</td></tr>';

 ...
 */
class Categories extends GestimagApi
{
	/**
	...
	public function get($id, $include_childs = false)
	{
		...
		if (!GestimagApi::_checkAccessToResource('categorie', $this->category->id)) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('categorie', 'creer')) {
			throw new RestException(403);
		}
		...
		if ($this->category->create(GestimagApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->category->update(GestimagApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('categorie', 'supprimer')) {
			throw new RestException(403);
		}
		$result = $this->category->fetch($id);
		if (!$result) {
		...
		if (!$this->category->delete(GestimagApiAccess::$user)) {
	...
	public function getListForObject($id, $type, $sortfield = "s.rowid", $sortorder = 'ASC', $limit = 0, $page = 0)
	{
		...
		if ($type == Categorie::TYPE_PRODUCT && !GestimagApiAccess::$user->hasRight('produit', 'lire') && !GestimagApiAccess::$user->hasRight('service', 'lire')) {
			throw new RestException(403);
		} elseif ($type == Categorie::TYPE_CONTACT && !GestimagApiAccess::$user->hasRight('contact', 'lire')) {
			throw new RestException(403);
		} elseif ($type == Categorie::TYPE_CUSTOMER && !GestimagApiAccess::$user->hasRight('societe', 'lire')) {
			throw new RestException(403);
		} elseif ($type == Categorie::TYPE_SUPPLIER && !GestimagApiAccess::$user->hasRight('fournisseur', 'lire')) {
			throw new RestException(403);
		} elseif ($type == Categorie::TYPE_MEMBER && !GestimagApiAccess::$user->hasRight('adherent', 'lire')) {
			throw new RestException(403);
		} elseif ($type == Categorie::TYPE_PROJECT && !GestimagApiAccess::$user->hasRight('projet', 'lire')) {
			throw new RestException(403);
		} elseif ($type == Categorie::TYPE_KNOWLEDGEMANAGEMENT && !GestimagApiAccess::$user->hasRight('knowledgemanagement', 'knowledgerecord', 'read')) {
			throw new RestException(403);
		} elseif ($type == Categorie::TYPE_ACTIONCOMM && !GestimagApiAccess::$user->hasRight('agenda', 'allactions', 'read')) {
			throw new RestException(403);
		}
	...
	public function linkObjectById($id, $type, $object_id)
	{
		...
		if ($type === Categorie::TYPE_PRODUCT) {
			if (!GestimagApiAccess::$user->hasRight('produit', 'creer') && !GestimagApiAccess::$user->hasRight('service', 'creer')) {
				throw new RestException(403);
			}
			...
			if (!GestimagApiAccess::$user->hasRight('societe', 'contact', 'creer')) {
				throw new RestException(403);
			}
			$object = new Contact($this->db);
		} elseif ($type === Categorie::TYPE_MEMBER) {
			if (!GestimagApiAccess::$user->hasRight('adherent', 'creer')) {
				throw new RestException(403);
			}
			$object = new Adherent($this->db);
		} elseif ($type === Categorie::TYPE_ACTIONCOMM) {
			if (!GestimagApiAccess::$user->hasRight('agenda', 'allactions', 'read')) {
				throw new RestException(403);
			}

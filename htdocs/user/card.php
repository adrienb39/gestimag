...
if ($action == 'create' || $action == 'adduserldap') {
	...
	if (isModEnabled('ldap') && (getDolGlobalInt('LDAP_SYNCHRO_ACTIVE') === Ldap::SYNCHRO_LDAP_TO_DOLIBARR)) {
		...
		// Show form to add an account from LDAP if sync LDAP -> Gestimag is set
		$ldap = new Ldap();
		$result = $ldap->connectBind();
		if ($result >= 0) {
			$required_fields = array(
	...
	print '<td>';
	$valuetoshow = '';
	if (preg_match('/ldap/', $gestimag_main_authentication)) {
		$valuetoshow .= ($valuetoshow ? ' + ' : '').$langs->trans("PasswordOfUserInLDAP").' (hidden)';
	}
	if (preg_match('/http/', $gestimag_main_authentication)) {
		$valuetoshow .= ($valuetoshow ? ' + ' : '').$langs->trans("HTTPBasicPassword");
	}
	if (preg_match('/gestimag/', $gestimag_main_authentication) || preg_match('/forceuser/', $gestimag_main_authentication)) {
		if (!empty($ldap_pass)) {	// For very old system comaptibilty. Now clear password can't be viewed from LDAP read
			$valuetoshow .= ($valuetoshow ? ' + ' : '').'<input type="hidden" name="password" value="'.dol_escape_htmltag($ldap_pass).'">'; // Gestimag password is preffiled with LDAP known password
			$valuetoshow .= preg_replace('/./i', '*', $ldap_pass);
		} else {
			// We do not use a field password but a field text to show new password to use.
			$valuetoshow .= ($valuetoshow ? ' + '.$langs->trans("GestimagPassword") : '').'<input class="minwidth300 maxwidth400 widthcentpercentminusx" maxlength="128" type="text" id="password" name="password" value="'.dol_escape_htmltag($password).'" autocomplete="new-password">';
			if (!empty($conf->use_javascript_ajax)) {
	...
	$doleditor = new DolEditor('signature', GETPOST('signature', 'restricthtml'), '', 138, 'gestimag_notes', 'In', true, $acceptlocallinktomedia, !getDolGlobalString('FCKEDITOR_ENABLE_USERSIGN') ? 0 : 1, ROWS_4, '90%');
	...
	print '</td><td>';
	require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
	$doleditor = new DolEditor('note_public', GETPOSTISSET('note_public') ? GETPOST('note_public', 'restricthtml') : '', '', 100, 'gestimag_notes', '', false, true, getDolGlobalString('FCKEDITOR_ENABLE_NOTE_PUBLIC'), ROWS_3, '90%');
	$doleditor->Create();
	print "</td></tr>\n";
	...
	print '</td><td>';
	require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
	$doleditor = new DolEditor('note_private', GETPOSTISSET('note_private') ? GETPOST('note_private', 'restricthtml') : '', '', 100, 'gestimag_notes', '', false, true, getDolGlobalString('FCKEDITOR_ENABLE_NOTE_PRIVATE'), ROWS_3, '90%');
	$doleditor->Create();
	print "</td></tr>\n";
	...
	if ($id > 0) {
		...
		if ($action != 'edit') {
			...
			if (!empty($object->ldap_sid) && $object->statut == 0) {
				...
				print $langs->trans("LoginAccountDisableInGestimag");
				print '</td>';
			} else {
				print '<td>';
				$addadmin = '';
			...
			if (isModEnabled('member')) {
				$langs->load("members");
				print '<tr><td>'.$langs->trans("LinkedToGestimagMember").'</td>';
				print '<td>';
				if ($object->fk_member) {
					$adh = new Adherent($db);
			...
			if (!empty($object->email_oauth2) && preg_match('/googleoauth/', $gestimag_main_authentication)) {
			...
			/*
			if (preg_match('/gestimag/', $gestimag_main_authentication)) {
				if ($object->pass) {
		...
		if ($action == 'edit' && ($canedituser || $caneditpasswordandsee)) {
			...
			if (preg_match('/http/', $gestimag_main_authentication)) {
				$valuetoshow .= ($valuetoshow ? (' '.$langs->trans("or").' ') : '').$form->textwithpicto($text, $langs->trans("GestimagInHttpAuthenticationSoPasswordUseless", $gestimag_main_authentication), 1, 'warning');
			}
			if (preg_match('/gestimag/', $gestimag_main_authentication) || preg_match('/forceuser/', $gestimag_main_authentication)) {
				if ($caneditpasswordandsee) {
			...
			*/
			...
			if ($caneditfield) {
				...
				$doleditor = new DolEditor('signature', $object->signature, '', 138, 'gestimag_notes', 'In', false, $acceptlocallinktomedia, !getDolGlobalString('FCKEDITOR_ENABLE_USERSIGN') ? 0 : 1, ROWS_4, '90%');
				print $doleditor->Create(1);
			} else {
				print dol_htmlentitiesbr($object->signature);
			}

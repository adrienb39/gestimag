...
/**
 *	Class to manage Gestimag users
 */
class User extends CommonObject
{
	...
	public function update($user, $notrigger = 0, $nosyncmember = 0, $nosyncmemberpass = 0, $nosynccontact = 0)
	{
		...
		$sql .= ", api_key = ".($this->api_key ? "'".$this->db->escape(dolEncrypt($this->api_key, '', '', 'gestimag'))."'" : "null");
	...
	public function getTooltipContentArray($params)
	{
		...
		if ($infologin > 0) {
			...
			$data['authentication'] = '<br><b>'.$langs->trans("AuthenticationMode").':</b> '.dol_string_nohtmltag($_SESSION["dol_authmode"].(empty($gestimag_main_demo) ? '' : ' (demo)'));
			...
			if (getDolGlobalString('DATABASE_PWD_ENCRYPTED')) {
				...
				if (!getDolGlobalString('MAIN_SECURITY_HASH_ALGO')) {
					if ($this->pass_indatabase_crypted && getDolGlobalString('LDAP_FIELD_PASSWORD_CRYPTED')) {
						$info[getDolGlobalString('LDAP_FIELD_PASSWORD_CRYPTED')] = dolGetLdapPasswordHash($this->pass_indatabase_crypted, 'md5frommd5'); // Create OpenLDAP MD5 password from Gestimag MD5 password
					}
				}
	 ...
	 *  @return int  				Return integer <0 if KO, >0 if OK
	 */
	public function update_ldap2gestimag(&$ldapuser)
	{
		// phpcs:enable
		...
		dol_syslog(get_class($this)."::update_ldap2gestimag result=".$result, LOG_DEBUG);

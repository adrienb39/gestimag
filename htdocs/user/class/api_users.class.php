 ...
 */
class Users extends GestimagApi
{
	/**
	...
	{
		if (!GestimagApiAccess::$user->hasRight('user', 'user', 'lire') && empty(GestimagApiAccess::$user->admin)) {
		...
		//$socid = GestimagApiAccess::$user->socid ? GestimagApiAccess::$user->socid : $societe;
	...
	{
		if (!GestimagApiAccess::$user->hasRight('user', 'user', 'lire') && empty(GestimagApiAccess::$user->admin) && $id != 0 && GestimagApiAccess::$user->id != $id) {
		...
		if ($id > 0 && !GestimagApi::_checkAccessToResource('user', $this->useraccount->id, 'user')) {
	...
	public function getByLogin($login, $includepermissions = 0)
	{
		...
		if (!GestimagApiAccess::$user->hasRight('user', 'user', 'lire') && empty(GestimagApiAccess::$user->admin) && GestimagApiAccess::$user->login != $login) {
			throw new RestException(403, 'Not allowed');
		}
		...
		if (!GestimagApi::_checkAccessToResource('user', $this->useraccount->id, 'user')) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}
	...
	public function getByEmail($email, $includepermissions = 0)
	{
		...
		if (!GestimagApiAccess::$user->hasRight('user', 'user', 'lire') && empty(GestimagApiAccess::$user->admin) && GestimagApiAccess::$user->email != $email) {
			throw new RestException(403, 'Not allowed');
		}
	...
	{
		if (!GestimagApiAccess::$user->hasRight('user', 'self', 'creer') && !GestimagApiAccess::$user->hasRight('user', 'user', 'lire') && empty(GestimagApiAccess::$user->admin)) {
			throw new RestException(403, 'Not allowed');
		}

		$apiUser = GestimagApiAccess::$user;
	...
	public function post($request_data = null)
	{
		...
		if (!GestimagApiAccess::$user->hasRight('user', 'creer') && empty(GestimagApiAccess::$user->admin)) {
			throw new RestException(403, "User creation not allowed for login ".GestimagApiAccess::$user->login);
		}
		...
		foreach ($request_data as $field => $value) {
			...
			/*if ($field == 'pass') {
				if (!GestimagApiAccess::$user->hasRight('user', 'user', 'password')) {
		...
		if ($this->useraccount->create(GestimagApiAccess::$user) < 0) {
	...
	{
		// Check user authorization
		if (!GestimagApiAccess::$user->hasRight('user', 'user', 'creer') && empty(GestimagApiAccess::$user->admin)) {
		...
		foreach ($request_data as $field => $value) {
			...
			if ($field == 'pass') {
				if ($this->useraccount->id != GestimagApiAccess::$user->id && !GestimagApiAccess::$user->hasRight('user', 'user', 'password')) {
					throw new RestException(403, 'You are not allowed to modify password of other users');
				}
				if ($this->useraccount->id == GestimagApiAccess::$user->id && !GestimagApiAccess::$user->hasRight('user', 'self', 'password')) {
			...
			if (GestimagApiAccess::$user->admin) {	// If user for API is admin
		...
		if ($this->useraccount->update(GestimagApiAccess::$user) >= 0) {
			return $this->get($id);
		} else {
	...
	public function setGroup($id, $group, $entity = 1)
	{
		...
		if (isModEnabled('multicompany') && getDolGlobalString('MULTICOMPANY_TRANSVERSE_MODE') && !empty(GestimagApiAccess::$user->admin) && empty(GestimagApiAccess::$user->entity)) {
			...
			$entity = (GestimagApiAccess::$user->entity > 0 ? GestimagApiAccess::$user->entity : $conf->entity);
		}
	...
	public function listGroups($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $group_ids = '0', $sqlfilters = '', $properties = '')
	{
		...
		if ((!getDolGlobalString('MAIN_USE_ADVANCED_PERMS') && !GestimagApiAccess::$user->hasRight('user', 'user', 'lire') && empty(GestimagApiAccess::$user->admin)) ||
			getDolGlobalString('MAIN_USE_ADVANCED_PERMS') && !GestimagApiAccess::$user->hasRight('user', 'group_advance', 'read') && empty(GestimagApiAccess::$user->admin)) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('user', 'user', 'supprimer') && empty(GestimagApiAccess::$user->admin)) {
			throw new RestException(403, 'Not allowed');
		}
		$result = $this->useraccount->fetch($id);
		if (!$result) {
		...
		if (!$this->useraccount->delete(GestimagApiAccess::$user)) {
	...
	protected function _cleanObjectDatas($object)
	{
		...
		$canreadsalary = ((isModEnabled('salaries') && GestimagApiAccess::$user->hasRight('salaries', 'read')) || !isModEnabled('salaries'));

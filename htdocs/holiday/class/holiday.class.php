...
class Holiday extends CommonObject
{
	 ...
	 *    @param      boolean   		$type			If true, read Gestimag user list, if false, return vacation balance list.
	...
	public function fetchUsers($stringlist = true, $type = true, $filters = '')
	{
		...
		if ($stringlist) {
			...
			// List for Gestimag users
			if ($type) {
				// If we need users of Gestimag
				$sql = "SELECT";
				if (isModEnabled('multicompany') && getDolGlobalString('MULTICOMPANY_TRANSVERSE_MODE')) {
					$sql .= " DISTINCT";
				}
	...
	/**
	 *	Compte le nombre d'utilisateur actifs dans Gestimag
	 *
	...
	/**
	 *	Compte le nombre d'utilisateur actifs dans Gestimag sans CP
	 *
	 *  @return     int      retourne le nombre d'utilisateur
	 */
	...
	/**
	 *  Compare le nombre d'utilisateur actif de Gestimag à celui des utilisateurs des congés payés
	 *
	 *  @param    int	$userGestimagWithoutCP	Number of active users in Gestimag without holidays
	 *  @param    int	$userCP    				Number of active users into table of holidays
	 *  @return   int							Return integer <0 if KO, >0 if OK
	 */
	public function verifNbUsers($userGestimagWithoutCP, $userCP)
	{
		if (empty($userCP)) {
			$userCP = 0;
		}
		dol_syslog(get_class($this).'::verifNbUsers userGestimag='.$userGestimagWithoutCP.' userCP='.$userCP);
		return 1;
	}

...
if ((empty($id) && empty($ref)) || $action == 'create' || $action == 'add') {
	...
	if ((in_array($fuserid, $childids) && !$user->hasRight('holiday', 'write')) || (!in_array($fuserid, $childids) && ((getDolGlobalString('MAIN_USE_ADVANCED_PERMS') && !$user->hasRight('holiday', 'writeall_advance') || !$user->hasRight('holiday', 'writeall'))))) {
		...
		$doleditor = new DolEditor('description', GETPOST('description', 'restricthtml'), '', 80, 'gestimag_notes', 'In', 0, false, isModEnabled('fckeditor'), ROWS_3, '90%');
		...
		if (($id > 0) || $ref) {
			...
			if ($canread) {
				...
				if (!$edit) {
					...
					print '<td>'.$langs->trans('DescCP').'</td>';
					print '<td class="tdtop">';
					$doleditor = new DolEditor('description', $object->description, '', 80, 'gestimag_notes', 'In', 0, false, isModEnabled('fckeditor'), ROWS_3, '90%');
					print $doleditor->Create(1);
					print '</td></tr>';
				}

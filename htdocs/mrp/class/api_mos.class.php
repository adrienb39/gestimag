...
class Mos extends GestimagApi
{
	...
	{
		if (!GestimagApiAccess::$user->hasRight('mrp', 'read')) {
		...
		if (!GestimagApi::_checkAccessToResource('mrp', $this->mo->id, 'mrp_mo')) {
	...
	public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $sqlfilters = '', $properties = '')
	{
		...
		$socid = GestimagApiAccess::$user->socid ? GestimagApiAccess::$user->socid : 0;

		$restrictonsocid = 0; // Set to 1 if there is a field socid in table of object

		// If the internal user must only see his customers, force searching by him
		$search_sale = 0;
		if ($restrictonsocid && !GestimagApiAccess::$user->hasRight('societe', 'client', 'voir') && !$socid) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('mrp', 'write')) {
			throw new RestException(403);
		}
		...
		if (!$this->mo->create(GestimagApiAccess::$user)) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->mo->update(GestimagApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('mrp', 'delete')) {
			throw new RestException(403);
		}
		$result = $this->mo->fetch($id);
		if (!$result) {
		...
		if (!$this->mo->delete(GestimagApiAccess::$user)) {
					...
					if ($value["qty"] != 0) {
						...
						if (!$error && $value["fk_warehouse"] > 0) {
							...
							if ($arrayname == 'arraytoconsume') {
								...
								$moline->fk_user_creat = GestimagApiAccess::$user->id;

								$resultmoline = $moline->create(GestimagApiAccess::$user);
								if ($resultmoline <= 0) {
									$error++;
									throw new RestException(500, $moline->error);
								}
								$idstockmove = $stockmove->livraison(GestimagApiAccess::$user, $value["objectid"], $value["fk_warehouse"], $qtytoprocess, 0, $labelmovement, dol_now(), '', '', $tmpproduct->status_batch, $id_product_batch, $codemovement);
							} else {
								$moline = new MoLine($this->db);
								$moline->fk_mo = $this->mo->id;
								$moline->position = $pos;
								...
								$idstockmove = $stockmove->reception(GestimagApiAccess::$user, $value["objectid"], $value["fk_warehouse"], $qtytoprocess, 0, $labelmovement, dol_now(), '', '', $tmpproduct->status_batch, $id_product_batch, $codemovement);
							}
							if ($idstockmove < 0) {
								$error++;
				...
				if ($line->role == 'toconsume') {
					...
					if ($line->qty != 0) {
						...
						if (!$error && $line->fk_warehouse > 0) {
							...
							if ($qtytoprocess >= 0) {
								$idstockmove = $stockmove->livraison(GestimagApiAccess::$user, $line->fk_product, $line->fk_warehouse, $qtytoprocess, 0, $labelmovement, dol_now(), '', '', $tmpproduct->status_batch, $id_product_batch, $codemovement);
							} else {
								$idstockmove = $stockmove->reception(GestimagApiAccess::$user, $line->fk_product, $line->fk_warehouse, $qtytoprocess, 0, $labelmovement, dol_now(), '', '', $tmpproduct->status_batch, $id_product_batch, $codemovement);
							}
							if ($idstockmove < 0) {
								$error++;
								throw new RestException(500, $stockmove->error);
		...
		foreach ($arrayofarrayname as $arrayname) {
			foreach (${$arrayname} as $value) {
				...
				if ($molinetoprocess->disable_stock_change == false) {
					...
					if ($arrayname == "arraytoconsume") {
						if ($qtytoprocess >= 0) {
							$idstockmove = $stockmove->livraison(GestimagApiAccess::$user, $molinetoprocess->fk_product, $fk_warehousetoprocess, $qtytoprocess, 0, $labelmovement, dol_now(), '', '', $tmpproduct->status_batch, $id_product_batch, $codemovement);
						} else {
							$idstockmove = $stockmove->reception(GestimagApiAccess::$user, $molinetoprocess->fk_product, $fk_warehousetoprocess, $qtytoprocess, 0, $labelmovement, dol_now(), '', '', $tmpproduct->status_batch, $id_product_batch, $codemovement);
						}
					} else {
						if ($qtytoprocess >= 0) {
							$idstockmove = $stockmove->reception(GestimagApiAccess::$user, $molinetoprocess->fk_product, $fk_warehousetoprocess, $qtytoprocess, $pricetoproduce, $labelmovement, dol_now(), '', '', $tmpproduct->status_batch, $id_product_batch, $codemovement);
						} else {
							$idstockmove = $stockmove->livraison(GestimagApiAccess::$user, $molinetoprocess->fk_product, $fk_warehousetoprocess, $qtytoprocess, 0, $labelmovement, dol_now(), '', '', $tmpproduct->status_batch, $id_product_batch, $codemovement);
						}
					}

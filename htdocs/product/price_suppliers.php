		...
		if ($action != 'edit' && $action != 're-edit') {
			...
			if (($action == 'create_price' || $action == 'edit_price') && $usercancreate) {
				...
				if ($rowid) {
					...
					if (empty($reshook)) {
						if (empty($form->result)) {
							...
							print img_picto($langs->trans("CreateGestimagThirdPartySupplier"), 'add', 'class="marginleftonly"');
							print '</a>';
						}
					}
				...
				if (getDolGlobalString('PRODUIT_FOURN_TEXTS')) {
					...
					$doleditor = new DolEditor('supplier_description', $object->desc_supplier, '', 160, 'gestimag_details', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_DETAILS'), ROWS_4, '90%');
					$doleditor->Create();

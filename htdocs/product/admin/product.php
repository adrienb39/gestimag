...
if (getDolGlobalString('PRODUIT_MULTIPRICES') && !getDolGlobalString('PRODUIT_MULTIPRICES_LIMIT')) {
	gestimag_set_const($db, 'PRODUIT_MULTIPRICES_LIMIT', 5, 'chaine', 0, '', $conf->entity);
}
...
if ($action == 'setcodeproduct') {
	if (gestimag_set_const($db, "PRODUCT_CODEPRODUCT_ADDON", $value, 'chaine', 0, '', $conf->entity) > 0) {
...
if ($action == 'other' && GETPOST('value_PRODUIT_LIMIT_SIZE') >= 0) {
	$res = gestimag_set_const($db, "PRODUIT_LIMIT_SIZE", GETPOST('value_PRODUIT_LIMIT_SIZE'), 'chaine', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
}
if ($action == 'other' && GETPOST('value_PRODUIT_MULTIPRICES_LIMIT') > 0) {
	$res = gestimag_set_const($db, "PRODUIT_MULTIPRICES_LIMIT", GETPOST('value_PRODUIT_MULTIPRICES_LIMIT'), 'chaine', 0, '', $conf->entity);
	if (!($res > 0)) {
		$error++;
	}
}
	...
	foreach ($select_pricing_rules as $tmprule => $tmplabel) { // Loop on each possible mode
		if ($tmprule == $princingrules) { // We are on selected rule, we enable it
			if ($princingrules == 'PRODUCT_PRICE_UNIQ') { // For this case, we disable entries manually
				$res = gestimag_set_const($db, 'PRODUIT_MULTIPRICES', 0, 'chaine', 0, '', $conf->entity);
				$res = gestimag_set_const($db, 'PRODUIT_CUSTOMER_PRICES_BY_QTY', 0, 'chaine', 0, '', $conf->entity);
				$res = gestimag_set_const($db, 'PRODUIT_CUSTOMER_PRICES', 0, 'chaine', 0, '', $conf->entity);
				gestimag_set_const($db, 'PRODUCT_PRICE_UNIQ', 1, 'chaine', 0, '', $conf->entity);
			} else {
				$multirule = explode('&', $princingrules);
				foreach ($multirule as $rulesselected) {
					$res = gestimag_set_const($db, $rulesselected, 1, 'chaine', 0, '', $conf->entity);
				}
			}
		} else { // We clear this mode
			if (strpos($tmprule, '&') === false) {
				$res = gestimag_set_const($db, $tmprule, 0, 'chaine', 0, '', $conf->entity);
			}
		}
	}

	$value = GETPOST('price_base_type', 'alpha');
	$res = gestimag_set_const($db, "PRODUCT_PRICE_BASE_TYPE", $value, 'chaine', 0, '', $conf->entity);

	/*$value = GETPOST('PRODUIT_SOUSPRODUITS', 'alpha');
	$res = gestimag_set_const($db, "PRODUIT_SOUSPRODUITS", $value, 'chaine', 0, '', $conf->entity);*/

	$value = GETPOST('PRODUIT_DESC_IN_FORM', 'alpha');
	$res = gestimag_set_const($db, "PRODUIT_DESC_IN_FORM", $value, 'chaine', 0, '', $conf->entity);

	$value = GETPOST('activate_viewProdTextsInThirdpartyLanguage', 'alpha');
	$res = gestimag_set_const($db, "PRODUIT_TEXTS_IN_THIRDPARTY_LANGUAGE", $value, 'chaine', 0, '', $conf->entity);

	$value = GETPOST('activate_mergePropalProductCard', 'alpha');
	$res = gestimag_set_const($db, "PRODUIT_PDF_MERGE_PROPAL", $value, 'chaine', 0, '', $conf->entity);

	$value = GETPOST('activate_usesearchtoselectproduct', 'alpha');
	$res = gestimag_set_const($db, "PRODUIT_USE_SEARCH_TO_SELECT", $value, 'chaine', 0, '', $conf->entity);

	$value = GETPOST('activate_FillProductDescAuto', 'alpha');
	$res = gestimag_set_const($db, "PRODUIT_AUTOFILL_DESC", $value, 'chaine', 0, '', $conf->entity);

	if (GETPOSTISSET('PRODUIT_FOURN_TEXTS')) {
		$value = GETPOST('PRODUIT_FOURN_TEXTS', 'alpha');
		$res = gestimag_set_const($db, "PRODUIT_FOURN_TEXTS", $value, 'chaine', 0, '', $conf->entity);
	}

	if (GETPOSTISSET('PRODUCT_USE_SUPPLIER_PACKAGING')) {
		$value = GETPOST('PRODUCT_USE_SUPPLIER_PACKAGING', 'alpha');
		$res = gestimag_set_const($db, "PRODUCT_USE_SUPPLIER_PACKAGING", $value, 'chaine', 0, '', $conf->entity);
	}
}
...
if ($action == 'del') {
	...
	if ($ret > 0) {
		if (getDolGlobalString('PRODUCT_ADDON_PDF') == "$value") {
			gestimag_del_const($db, 'PRODUCT_ADDON_PDF', $conf->entity);
		}
	}
}
...
// Set default model
if ($action == 'setdoc') {
	if (gestimag_set_const($db, "PRODUCT_ADDON_PDF", $value, 'chaine', 0, '', $conf->entity)) {
...
if ($action == 'set') {
	...
	if (GETPOST('value', 'alpha')) {
		$res = gestimag_set_const($db, $const, $value, 'chaine', 0, '', $conf->entity);
	} else {
		$res = gestimag_del_const($db, $const, $conf->entity);
	}
	if (!($res > 0)) {
		$error++;
	}
...
if (preg_match('/set_(.+)/', $action, $reg)) {
	...
	if ($keyforvar) {
		$value = 1;
		$res = gestimag_set_const($db, $keyforvar, $value, 'chaine', 0, '', $conf->entity);
	}
}
...
if (preg_match('/del_(.+)/', $action, $reg)) {
	$keyforvar = $reg[1];
	if ($keyforvar) {
		$res = gestimag_del_const($db, $keyforvar, $conf->entity);
	}
}

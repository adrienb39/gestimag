 ...
 */
class StockMovements extends GestimagApi
{
	/**
	...
	public function get($id)
	{
		...
		if (!GestimagApi::_checkAccessToResource('warehouse',$this->stockmovement->id)) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('stock', 'creer')) {
			throw new RestException(403);
		}
		...
		if ($this->stockmovement->_create(GestimagApiAccess::$user, $product_id, $warehouse_id, $qty, $type, $price, $movementlabel, $movementcode, $dateMvt, $eatBy, $sellBy, $lot) <= 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if( ! GestimagApi::_checkAccessToResource('stock',$this->stockmovement->id)) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}
		...
		if($this->stockmovement->update($id, GestimagApiAccess::$user))
			return $this->get ($id);
	...
	{
		if (! GestimagApiAccess::$user->hasRight('stock', 'supprimer')) {
			throw new RestException(403);
		}
		$result = $this->stockmovement->fetch($id);
		if (! $result ) {
		...
		if (! $this->stockmovement->delete(GestimagApiAccess::$user)) {

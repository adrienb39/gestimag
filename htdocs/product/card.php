...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($canvasdisplayaction)) {
	...
	if ($action == 'create' && $usercancreate) {
		...
		if (empty($reshook)) {
			...
			$doleditor = new DolEditor('desc', GETPOST('desc', 'restricthtml'), '', 160, 'gestimag_details', '', false, true, getDolGlobalString('FCKEDITOR_ENABLE_DETAILS'), ROWS_4, '90%');
			$doleditor->Create();
			print "</td></tr>";
			...
			// We use gestimag_details as type of DolEditor here, because we must not accept images as description is included into PDF and not accepted by TCPDF.
			$doleditor = new DolEditor('note_private', GETPOST('note_private', 'restricthtml'), '', 140, 'gestimag_details', '', false, true, getDolGlobalString('FCKEDITOR_ENABLE_NOTE_PRIVATE'), ROWS_8, '90%');
			$doleditor->Create();
		...
		if ($action == 'edit' && $usercancreate) {
			...
			if (empty($reshook)) {
				...
				// We use gestimag_details as type of DolEditor here, because we must not accept images, as description is included into PDF and external links are not accepted by TCPDF.
				$doleditor = new DolEditor('desc', GETPOSTISSET('desc') ? GETPOST('desc', 'restricthtml') : $object->description, '', 160, 'gestimag_details', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_DETAILS'), ROWS_4, '90%');
				$doleditor->Create();
				...
				if (getDolGlobalString('MAIN_DISABLE_NOTES_TAB')) {
					...
					$doleditor = new DolEditor('note_private', $object->note_private, '', 140, 'gestimag_notes', '', false, true, getDolGlobalInt('FCKEDITOR_ENABLE_NOTE_PRIVATE'), ROWS_4, '90%');
					$doleditor->Create();

...
class Cronjob extends CommonObject
{
	...
	public function run_jobs(string $userlogin)
	{
		...
		if ($this->jobtype == 'command') {
			global $gestimag_cron_allow_cli;

			if (empty($gestimag_cron_allow_cli)) {
				$langs->load("errors");
				$this->error      = $langs->trans("FailedToExecutCommandJob");
				$this->lastoutput = '';
				$this->lastresult = $langs->trans("ErrorParameterMustBeEnabledToAllwoThisFeature", 'gestimag_cron_allow_cli');
			} else {

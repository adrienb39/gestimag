 ...
 *		\ingroup    website
 *		\brief      Wrapper to output pages when website is powered by Gestimag instead of a native web server
 */
...
if ($pageid == 'css') {   // No more used ?
	...
	//if (empty($gestimag_nocache)) header('Cache-Control: max-age=3600, public, must-revalidate');
	//else
	header('Cache-Control: no-cache');
	$original_file = $gestimag_main_data_root.($conf->entity > 1 ? '/'.$conf->entity : '').'/website/'.$websitekey.'/styles.css.php';
} else {
	$original_file = $gestimag_main_data_root.($conf->entity > 1 ? '/'.$conf->entity : '').'/website/'.$websitekey.'/page'.$pageid.'.tpl.php';
}

...
if (empty($reshook) && $action == 'add') {
					...
					if (!empty((float) $project->price_booth)) {
						...
						if (!$error) {
							...
							/*if (!$error) {
								...
								$reftouse = $facture->id;
								$redirection = $gestimag_main_url_root.'/public/payment/newpayment.php?source='.$sourcetouse.'&ref='.$reftouse.'&booth='.$conforbooth->id;
	...
	if (!$error) {
		...
		$redirection = $gestimag_main_url_root.'/public/eventorganization/subscriptionok.php?id='.$id.'&securekey='.$securekeyurl;
		header("Location: ".$redirection);
		exit;
	} else {
		$db->rollback();
...
echo load_fiche_titre($langs->trans("NewSuggestionOfBooth"), '', '', 0, 0, 'center');
// Welcome message
...
// Label
echo '<tr><td>'.$langs->trans("LabelOfBooth").'<span class="star">*</span></td>'."\n";
echo '</td><td><input type="text" name="label" class="minwidth150" value="'.dol_escape_htmltag(GETPOST('label')).'"></td></tr>'."\n";
// Note
echo '<tr><td>'.$langs->trans("Description").'<span class="star">*</span></td>'."\n";
echo '<td><textarea name="note" id="note" wrap="soft" class="quatrevingtpercent" rows="'.ROWS_3.'">'.dol_escape_htmltag(GETPOST('note', 'restricthtml'), 0, 1).'</textarea></td></tr>'."\n";
...
echo '<div class="center">';
echo '<br>';
echo '<input type="submit" value="'.$langs->trans("SuggestBooth").'" name="suggestbooth" id="suggestbooth" class="button">';
echo '</div>';
echo '<br><br>';

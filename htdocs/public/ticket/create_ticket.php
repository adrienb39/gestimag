...
if (empty($reshook)) {
	...
	if ($action == 'create_ticket' && GETPOST('save', 'alpha')) {
		...
		if (!$error) {
			...
			$object->context['createdfrompublicinterface'] = 1; // To make a difference between a ticket created from the public interface and a ticket directly created from gestimag

...
if (getDolGlobalString('STRIPE_USER_ACCOUNT_FOR_ACTIONS')) {
	// We set the user to use for all ipn actions in Gestimag
	$user = new User($db);
...
if (getDolGlobalString('STRIPE_DEBUG')) {
	$fh = fopen(DOL_DATA_ROOT.'/gestimag_stripeipn_payload.log', 'w+');
	if ($fh) {
		fwrite($fh, dol_print_date(dol_now('gmt'), 'standard').' IPN Called. service='.$service.' HTTP_STRIPE_SIGNATURE='.$sig_header."\n");
		fwrite($fh, $payload);
		fclose($fh);
		dolChmod(DOL_DATA_ROOT.'/gestimag_stripeipn_payload.log');
	}
}
...
if ($event->type == 'payout.created') {
	...
	$result = gestimag_set_const($db, $service."_NEXTPAYOUT", date('Y-m-d H:i:s', $event->data->object->arrival_date), 'chaine', 0, '', $conf->entity);
	...
	$error = 0;
	$result = gestimag_set_const($db, $service."_NEXTPAYOUT", null, 'chaine', 0, '', $conf->entity);
	if ($result) {
		$langs->load("errors");
	...
	$paymentTypeCodeInGestimag = "";	// payment type according to Gestimag
	$payment_amount = 0;
	$payment_amountInGestimag = 0;
		...
		if ($obj) {
			if ($obj->type == 'ban') {
				if ($obj->traite == 1) {
					...
					$payment_amountInGestimag = $obj->amount;
					$paymentTypeCodeInGestimag = $obj->type;
			...
			if ($obj->type == 'card' || empty($obj->type)) {
				if ($obj->traite == 0) {
					...
					$invoice_id = $obj->fk_facture;
					$payment_amountInGestimag = $obj->amount;
					$paymentTypeCodeInGestimag = empty($obj->type) ? 'card' : $obj->type;
	...
	if ($paymentTypeCodeInGestimag) {
		...
		$payment_amount = $payment_amountInGestimag;
		// TODO Check payment_amount in Stripe (received) is same than the one in Gestimag

		$postactionmessages = array();

		if ($paymentTypeCode == "CB" && ($paymentTypeCodeInGestimag == 'card' || empty($paymentTypeCodeInGestimag))) {
		...
		} elseif ($paymentTypeCode == "PRE" && $paymentTypeCodeInGestimag == 'ban') {
			...
			dol_syslog("The payment mode of this payment is ".$paymentTypeCode." in Stripe and ".$paymentTypeCodeInGestimag." in Gestimag. This case is not managed by the IPN");
		}
	} else {
		dol_syslog("Nothing to do in database because we don't know paymentTypeIdInGestimag");
	}

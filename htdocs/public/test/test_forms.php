...
// Security
if ($gestimag_main_prod) {
	accessforbidden('Access forbidden when $gestimag_main_prod is set to 1');
}
...
This page is a sample of page using Gestimag HTML widget methods. It is designed to make test with<br>
...
echo "Test 1a: We must have here current date and hour for user (must match hour on browser). Note: Check your are logged so user TZ and DST are known.";
$offsettz = (empty($_SESSION['dol_tz']) ? 0 : $_SESSION['dol_tz']) * 60 * 60;
$offsetdst = (empty($_SESSION['dol_dst']) ? 0 : $_SESSION['dol_dst']) * 60 * 60;
echo " (dol_tz=".$offsettz." dol_dst=".$offsetdst.")<br>\n";
echo $form->selectDate(dol_now(), 'test1a', 1, 1, 0);

echo '<br><br>'."\n";

echo "Test 1b: We must have here current date with hours to 00:00.<br>";
echo $form->selectDate('', 'test1b', 1, 1, 0);

echo '<br><br>'."\n";

// Test2: form->selectDate using tzuser date
echo "Test 2: We must have here 1970-01-01 selected (fields can be empty)<br>\n";
echo $form->selectDate(dol_get_first_day(1970, 1, false), 'test2', 1, 1, 1);

echo '<br><br>'."\n";

// Test3: form->selectDate for 1970-01-01 00:00:00
echo "Test 3: We must have here 1970-01-01 00:00:00 selected (fields are mandatory)<br>\n";
echo $form->selectDate(dol_get_first_day(1970, 1, false), 'test3', 1, 1, 0);

echo '<br><br>'."\n";

// Test4a: a select
echo "Test 4a: a select<br>\n";
...
$selected = 3;
echo $form->selectarray('testselecta', $array, $selected, 1, 0, 0, '', 0, 0, 0, '', 'minwidth100', 1);
echo '<br><br>';
echo "Test 4b: a select<br>\n";
$array = array(1=>'Value 1', 2=>'Value 2', 3=>'Value 3');
$selected = 3;
echo $form->selectarray('testselectb', $array, $selected, 1, 0, 0, '', 0, 0, 0, '', 'minwidth100', 1);
echo '<br><br>'."\n";
echo "Test 4c: Select array with addjscombo not forced<br>\n";
$array = array(1=>'Value 1', 2=>'Value 2', 3=>'Value 3');
echo $form->selectarray('selectarray', $array, $selected, 1, 0, 0, '', 0, 0, 0, '', 'minwidth100', 0);

echo '<br><br>'."\n";

echo "Test 4d: a select with ajax refresh and with onchange call of url<br>\n";
$selected = -1;
echo $form->selectArrayAjax('testselectc', DOL_URL_ROOT.'/core/ajax/selectsearchbox.php', $selected, '', '', 0, 1, 'minwidth100', 1);

echo '<br><br>'."\n";

// Test5a: form->select_thirdparty
echo "Test 5a: Select thirdparty<br>\n";
echo $form->select_company(0, 'thirdpartytest', '', '', 0, 0, null, 0, 'minwidth100');

echo '<br><br>'."\n";

// Test5b: form->select_product
echo "Test 5b: Select product (using ajax)<br>\n";
$form->select_produits(0, 'producttest', '', 20, 0, 1, 2, '', 0, null, 0, '1', 0, 'minwidth100');

echo '<br><br>'."\n";

// Test5c: a multiselect
echo "Test 5c: a multiselect<br>\n";
$array = array(1=>'Value 1', 2=>'Value 2', 3=>'Value 3');
$arrayselected = array(1, 3);
echo $form->multiselectarray('testmulti', $array, $arrayselected, '', 0, 'minwidth100');

echo '<br><br>'."\n";

// Test6a: Upload of big files
echo "Test 6a: Upload of big files<br>\n";
echo "The file will be uploaded in the directory: documents/test/temp/<br>\n";

if (is_file(DOL_DOCUMENT_ROOT.'/includes/flowjs/flow.js')) {
	echo '<button id="buttonbigupload" type="button">Browse...</button>';
	echo '&nbsp;<span id="filespan">No file selected.</span>';
	echo '<br><div class="progress-bar filepgbar taligncenter" role="progressbar" style="width:1%;display:none"><span class="small valigntop">0%</span></div>';
	echo '<br><button type="button" style="display:none;" data-fileidentifier="" class="btn green-haze btn-circle cancelfileinput" id="filecancel">Cancel</button>';
	echo '<script src="'.DOL_URL_ROOT.'/includes/flowjs/flow.js"></script>';
	echo '<script>
	...
	echo "If this message displays, please add flow.js and flow.min.js files which can be found here: https://github.com/flowjs/flow.js and place the js lib in htdocs/includes/flowjs/<br>\n";
}

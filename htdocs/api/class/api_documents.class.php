 ...
 */
class Documents extends GestimagApi
{
	/**
	 * Constructor
	 */
	...
	public function index($modulepart, $original_file = '')
	{
		...
		$check_access = dol_check_secure_access_document($modulepart, $relativefile, $entity, GestimagApiAccess::$user, '', 'read');
	...
	public function builddoc($modulepart, $original_file = '', $doctemplate = '', $langcode = '')
	{
		...
		$check_access = dol_check_secure_access_document($modulepart, $relativefile, $entity, GestimagApiAccess::$user, '', 'write');
		$accessallowed              = $check_access['accessallowed'];
		$sqlprotectagainstexternals = $check_access['sqlprotectagainstexternals'];
		$original_file              = $check_access['original_file'];
	...
	public function getDocumentsListByElement($modulepart, $id = 0, $ref = '', $sortfield = '', $sortorder = '')
	{
		...
		if ($modulepart == 'societe' || $modulepart == 'thirdparty') {
			...
			if (!GestimagApiAccess::$user->hasRight('user', 'user', 'lire') && GestimagApiAccess::$user->id != $id) {
				throw new RestException(403);
			}
			...
			if (!GestimagApiAccess::$user->hasRight('adherent', 'lire')) {
				throw new RestException(403);
			}
			...
			if (!GestimagApiAccess::$user->hasRight('propal', 'lire')) {
				throw new RestException(403);
			}
			...
			if (!GestimagApiAccess::$user->hasRight('supplier_proposal', 'read')) {
				throw new RestException(403);
			}
			...
			if (!GestimagApiAccess::$user->hasRight('fournisseur', 'commande', 'lire') && !GestimagApiAccess::$user->hasRight('supplier_order', 'lire')) {
				throw new RestException(403);
			}
			...
			if (!GestimagApiAccess::$user->hasRight('expedition', 'lire')) {
				throw new RestException(403);
			}
			...
			if (!GestimagApiAccess::$user->hasRight('fournisseur', 'facture', 'lire') && !GestimagApiAccess::$user->hasRight('supplier_invoice', 'lire')) {
				throw new RestException(403);
			}
			...
			if (!GestimagApiAccess::$user->hasRight('agenda', 'myactions', 'read') && !GestimagApiAccess::$user->hasRight('agenda', 'allactions', 'read')) {
				throw new RestException(403);
			}
			...
			if (!GestimagApiAccess::$user->hasRight('expensereport', 'read') && !GestimagApiAccess::$user->hasRight('expensereport', 'read')) {
				throw new RestException(403);
			}
			...
			if (!GestimagApiAccess::$user->hasRight('knowledgemanagement', 'knowledgerecord', 'read') && !GestimagApiAccess::$user->hasRight('knowledgemanagement', 'knowledgerecord', 'read')) {
				throw new RestException(403);
			}
			...
			// if (!GestimagApiAccess::$user->hasRight('ecm', 'read')) {
			// 	throw new RestException(403);
			// }
	...
	public function post($filename, $modulepart, $ref = '', $subdir = '', $filecontent = '', $fileencoding = '', $overwriteifexists = 0, $createdirifnotexists = 1)
	{
		...
		if ($ref) {
			...
			if ($modulepart != 'ecm') {
				$relativefile = $tmpreldir.dol_sanitizeFileName($object->ref);
				$tmp = dol_check_secure_access_document($modulepart, $relativefile, $entity, GestimagApiAccess::$user, $ref, 'write');
				...
				if (!GestimagApiAccess::$user->hasRight('ecm', 'upload')) {
			...
			if ($modulepart != 'ecm') {
				$relativefile = $subdir;
				$tmp = dol_check_secure_access_document($modulepart, $relativefile, $entity, GestimagApiAccess::$user, '', 'write');
				$upload_dir = $tmp['original_file']; // No dirname here, tmp['original_file'] is already the dir because dol_check_secure_access_document was called with param original_file that is only the dir
			} else {
				if (!GestimagApiAccess::$user->hasRight('ecm', 'upload')) {
					throw new RestException(403, 'Missing permission to upload files in ECM module');

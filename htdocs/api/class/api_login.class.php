...
class Login
{
	...
	{
		global $conf, $gestimag_main_authentication, $gestimag_auto_user;
		...
		if ($gestimag_main_authentication == 'forceuser') {
			if (empty($gestimag_auto_user)) {
				$gestimag_auto_user = 'auto';
			}
			if ($gestimag_auto_user != $login) {
				dol_syslog("Warning: your instance is set to use the automatic forced login '".$gestimag_auto_user."' that is not the requested login. API usage is forbidden in this mode.");
				throw new RestException(403, "Your instance is set to use the automatic login '".$gestimag_auto_user."' that is not the requested login. API usage is forbidden in this mode.");
			}
		}
		...
		if (empty($tmpuser->api_key) || $reset) {
			...
			$sql .= " SET api_key = '".$this->db->escape(dolEncrypt($token, '', '', 'gestimag'))."'";
		...
		return array(
			'success' => array(
				...
				'message' => 'Welcome '.$login.($reset ? ' - Token is new' : ' - This is your token (recorded for your user). You can use it to make any REST API call, or enter it into the DOLAPIKEY field to use the Gestimag API explorer.')
			)
		);
	}
}

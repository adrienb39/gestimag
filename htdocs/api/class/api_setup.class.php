 ...
 */
class Setup extends GestimagApi
{
	...
	public function getPaymentTypes($sortfield = "code", $sortorder = 'ASC', $limit = 100, $page = 0, $active = 1, $sqlfilters = '')
	{
		...
		if (!GestimagApiAccess::$user->hasRight('propal', 'lire') && !GestimagApiAccess::$user->hasRight('commande', 'lire') && !GestimagApiAccess::$user->hasRight('facture', 'lire')) {
			throw new RestException(403);
		}
	...
	public function getListOfRegions($sortfield = "code_region", $sortorder = 'ASC', $limit = 100, $page = 0, $country = 0, $filter = '', $sqlfilters = '')
	{
		...
		if ($sqlfilters) {
			$errormessage = '';
			if (!GestimagApi::_checkFilters($sqlfilters, $errormessage)) {
				throw new RestException(400, 'Error when validating parameter sqlfilters -> '.$errormessage);
			}
			$regexstring = '\(([^:\'\(\)]+:[^:\'\(\)]+:[^\(\)]+)\)';
			$sql .= " AND (".preg_replace_callback('/'.$regexstring.'/', 'GestimagApi::_forge_criteria_callback', $sqlfilters).")";
		}
	 ...
	 * code supported by Gestimag, for example 'en_US' or 'fr_FR'.
	...
	public function getListOfExtrafields($sortfield = "t.pos", $sortorder = 'ASC', $elementtype = '', $sqlfilters = '')
	{
		...
		if (!GestimagApiAccess::$user->admin) {
	...
	public function postExtrafields($attrname, $elementtype, $request_data = null)
	{
		...
		$entity = GestimagApiAccess::$user->entity;
		if (empty($entity)) {
			$entity = 1;
		}
	...
	public function getCompany()
	{
		...
		if (!GestimagApiAccess::$user->admin
			&& (!getDolGlobalString('API_LOGINS_ALLOWED_FOR_GET_COMPANY') || GestimagApiAccess::$user->login != $conf->global->API_LOGINS_ALLOWED_FOR_GET_COMPANY)) {
	...
	public function getConf($constantname)
	{
		...
		if (!GestimagApiAccess::$user->admin
			&& (!getDolGlobalString('API_LOGINS_ALLOWED_FOR_CONST_READ') || GestimagApiAccess::$user->login != getDolGlobalString('API_LOGINS_ALLOWED_FOR_CONST_READ'))) {
	 ...
	 *
	 * @param string	$target			Can be 'local' or 'default' or Url of the signatures file to use for the test. Must be reachable by the tested Gestimag.
	...
	public function getCheckIntegrity($target)
	{
		...
		if (!GestimagApiAccess::$user->admin
			&& (!getDolGlobalString('API_LOGINS_ALLOWED_FOR_INTEGRITY_CHECK') || GestimagApiAccess::$user->login != $conf->global->API_LOGINS_ALLOWED_FOR_INTEGRITY_CHECK)) {
		...
		if (empty($xmlremote)) {
			$xmlremote = 'https://www.gestimag.org/files/stable/signatures/filelist-'.DOL_VERSION.'.xml';
		}
		...
		if ($xml) {
			...
			// Forced constants
			if (is_object($xml->gestimag_constants[0])) {
				...
				$i = 0;
				foreach ($xml->gestimag_constants[0]->constant as $constant) {    // $constant is a simpleXMLElement
			...
			// Scan htdocs
			if (is_object($xml->gestimag_htdocs_dir[0])) {
				$includecustom = (empty($xml->gestimag_htdocs_dir[0]['includecustom']) ? 0 : $xml->gestimag_htdocs_dir[0]['includecustom']);
				...
				$ret = getFilesUpdated($file_list, $xml->gestimag_htdocs_dir[0], '', DOL_DOCUMENT_ROOT, $checksumconcat); // Fill array $file_list
				...
				throw new RestException(500, 'Error: Failed to found gestimag_htdocs_dir into XML file '.$xmlfile);
			}
			...
			$checksumtoget = trim((string) $xml->gestimag_htdocs_dir_checksum);
	...
	public function getModules()
	{
		...
		if (!GestimagApiAccess::$user->admin
			&& (!getDolGlobalString('API_LOGINS_ALLOWED_FOR_GET_MODULES') || GestimagApiAccess::$user->login != $conf->global->API_LOGINS_ALLOWED_FOR_GET_MODULES)) {

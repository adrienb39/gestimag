...
/**
 * Gestimag API access class
 *
 */
class GestimagApiAccess implements iAuthenticate
{
	const REALM = 'Restricted Gestimag API';
	...
	public function __isAllowed()
	{
		...
		if ($api_key) {
			...
			$sql .= " WHERE u.api_key = '".$this->db->escape($api_key)."' OR u.api_key = '".$this->db->escape(dolEncrypt($api_key, '', '', 'gestimag'))."'";
		...
		Resources::$accessControlFunction = 'GestimagApiAccess::verifyAccess';
	...
	{
		$requires = isset($m['class']['GestimagApiAccess']['properties']['requires'])
				? $m['class']['GestimagApiAccess']['properties']['requires']
				: false;

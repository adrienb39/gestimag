 ...
 */
class Orders extends GestimagApi
{
	/**
	...
	private function _fetch($id, $ref = '', $ref_ext = '', $contact_list = 1)
	{
		...
		if (!GestimagApi::_checkAccessToResource('commande', $this->commande->id)) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('commande', 'creer')) {
		...
		if ($this->commande->create(GestimagApiAccess::$user) < 0) {
	...
	public function deleteLine($id, $lineid)
	{
		...
		$updateRes = $this->commande->deleteLine(GestimagApiAccess::$user, $lineid, $id);
		if ($updateRes > 0) {
			return $this->get($id);
		} else {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->commande->update(GestimagApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('commande', 'supprimer')) {
			throw new RestException(403);
		}
		$result = $this->commande->fetch($id);
		if (!$result) {
		...
		if (!$this->commande->delete(GestimagApiAccess::$user)) {
	...
	public function validate($id, $idwarehouse = 0, $notrigger = 0)
	{
		...
		$result = $this->commande->valid(GestimagApiAccess::$user, $idwarehouse, $notrigger);
		if ($result == 0) {
	...
	public function reopen($id)
	{
		...
		$result = $this->commande->set_reopen(GestimagApiAccess::$user);
		if ($result < 0) {
			throw new RestException(405, $this->commande->error);
		} elseif ($result == 0) {
	...
	public function setinvoiced($id)
	{
		...
		$result = $this->commande->classifyBilled(GestimagApiAccess::$user);
		if ($result < 0) {
			throw new RestException(400, $this->commande->error);
		}
	...
	public function close($id, $notrigger = 0)
	{
		...
		$result = $this->commande->cloture(GestimagApiAccess::$user, $notrigger);
		if ($result == 0) {
	...
	public function settodraft($id, $idwarehouse = -1)
	{
		...
		$result = $this->commande->setDraft(GestimagApiAccess::$user, $idwarehouse);
		if ($result == 0) {
	...
	public function createOrderFromProposal($proposalid)
	{
		...
		$result = $this->commande->createFromProposal($propal, GestimagApiAccess::$user);
		if ($result < 0) {
			throw new RestException(405, $this->commande->error);
		}
		$this->commande->fetchObjectLinked();
	...
	{
		require_once DOL_DOCUMENT_ROOT.'/expedition/class/expedition.class.php';
		if (!GestimagApiAccess::$user->hasRight('expedition', 'creer')) {
			throw new RestException(403);
		}
		if ($warehouse_id <= 0) {
		...
		$result = $shipment->create(GestimagApiAccess::$user);
		if ($result <= 0) {

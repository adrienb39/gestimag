...
if ($search_all) {
	...
	echo '<!-- Search done like if RECRUITMENT_QUICKSEARCH_ON_FIELDS = '.$setupstring.' -->'."\n";
...
if (getDolGlobalString('MAIN_CHECKBOX_LEFT_COLUMN')) {
	echo '<td class="liste_titre maxwidthsearch center">';
...
foreach ($object->fields as $key => $val) {
	...
	if (!empty($arrayfields['t.'.$key]['checked'])) {
		...
		if (!empty($val['arrayofkeyval']) && is_array($val['arrayofkeyval'])) {
			echo $form->selectarray('search_'.$key, $val['arrayofkeyval'], (isset($search[$key]) ? $search[$key] : ''), $val['notnull'], 0, 0, '', 1, 0, 0, '', 'maxwidth100'.($key == 'status' ? ' search_status width100 onrightofpage' : ''), 1);
...
while ($i < $imaxinloop) {
	...
	if ($mode == 'kanban') {
		...
		echo $object->getKanbanView('', array('jobpositionlink' => $recruitment->getNomUrl(1), 'selected' => $selected));
		if ($i == ($imaxinloop - 1)) {
			echo '</div>';
			echo '</td></tr>';
		}
...
if (in_array('builddoc', array_keys($arrayofmassactions)) && ($nbtotalofrecords === '' || $nbtotalofrecords)) {
	...
	echo $formfile->showdocuments('massfilesarea_recruitment', '', $filedir, $urlsource, 0, $delallowed, '', 1, 1, 0, 48, 1, $param, $title, '', '', '', null, $hidegeneratedfilelistifempty);
}

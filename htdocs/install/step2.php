...
if ($gestimag_main_db_type == "sqlite") {
	$choix = 4;
}
if ($gestimag_main_db_type == "sqlite3") {
	$choix = 5;
}
//if (empty($choix)) dol_print_error(null,'Database type '.$gestimag_main_db_type.' not supported into step2.php page');
...
gestimag_install_syslog("--- step2: entering step2.php page");

'@phan-var-force string $gestimag_main_db_prefix';  // From configuraiotn file or install/inc.php
...
pHeader($langs->trans("GestimagSetup").' - '.$langs->trans("CreateDatabaseObjects"), "step4");
...
if ($action == "set") {
	...
	// http://localhost/gestimagnew/install/step2.php?action=set&token='.newToken().'&createtables=0&createkeys=0&createfunctions=0&createdata=llx_20_c_departements
	...
	if ($ok && $createtables) {
		...
		$ok = 0;
		$handle = opendir($dir);
		gestimag_install_syslog("step2: open tables directory ".$dir." handle=".(is_bool($handle) ? json_encode($handle) : $handle));
		$tablefound = 0;
		$tabledata = array();
		...
		foreach ($tabledata as $file) {
			...
			if ($fp) {
				...
				if ($gestimag_main_db_prefix != 'llx_') {
	...
	if ($ok && $createkeys) {
		...
		$okkeys = 0;
		$handle = opendir($dir);
		gestimag_install_syslog("step2: open keys directory ".$dir." handle=".(is_bool($handle) ? json_encode($handle) : $handle));
		$tablefound = 0;
		$tabledata = array();
		if (is_resource($handle)) {
			while (($file = readdir($handle)) !== false) {
	...
	if ($ok && $createfunctions) {
		...
		if (file_exists($dir.$file)) {
			$fp = fopen($dir.$file, "r");
			gestimag_install_syslog("step2: open function file ".$dir.$file." handle=".(is_bool($fp) ? json_encode($fp) : $fp));
			if ($fp) {
				$buffer = '';
				while (!feof($fp)) {
					$buf = fgets($fp, 4096);
	...
	if ($ok && $createdata) {
		...
		// Insert data
		$handle = opendir($dir);
		gestimag_install_syslog("step2: open directory data ".$dir." handle=".(is_bool($handle) ? json_encode($handle) : $handle));
		$tablefound = 0;
		$tabledata = array();
		if (is_resource($handle)) {
			while (($file = readdir($handle)) !== false) {
		...
		foreach ($tabledata as $file) {
			$name = substr($file, 0, dol_strlen($file) - 4);
			$fp = fopen($dir.$file, "r");
			gestimag_install_syslog("step2: open data file ".$dir.$file." handle=".(is_bool($fp) ? json_encode($fp) : $fp));
			if ($fp) {
				...
				gestimag_install_syslog("step2: found ".$linefound." records, defined ".count($arrayofrequests)." group(s).");
...
gestimag_install_syslog("- step2: end");
...
$out  = '<input type="checkbox" name="gestimagpingno" id="gestimagpingno"'.((getDolGlobalString('MAIN_FIRST_PING_OK_ID') == 'disabled') ? '' : ' value="checked" checked="true"').'> ';
$out .= '<label for="gestimagpingno">'.$langs->trans("MakeAnonymousPing").'</label>';
...
$out .= '  jQuery("#gestimagpingno").click(function() {';

...
require_once $gestimag_main_document_root.'/core/lib/security.lib.php'; // for dol_hash
require_once $gestimag_main_document_root.'/core/lib/functions2.lib.php';
...
if (@file_exists($forcedfile)) {
	...
	if ($force_install_noedit == 2) {
		if (!empty($force_install_gestimaglogin)) {
			$login = $force_install_gestimaglogin;
		}
	}
}

gestimag_install_syslog("--- step5: entering step5.php page ".$versionfrom." ".$versionto);
...
if ($action == "set" || empty($action) || preg_match('/upgrade/i', $action)) {
	...
	if ($action == "set") {
		...
		gestimag_install_syslog('step5: load module user '.DOL_DOCUMENT_ROOT."/core/modules/".$file, LOG_INFO);
		...
		if ($db->connected) {
			...
			if ($resql) {
				...
				if ($numrows == 0) {
					...
					gestimag_set_const($db, "DATABASE_PWD_ENCRYPTED", "1", 'chaine', 0, '', $conf->entity);
					gestimag_set_const($db, "MAIN_SECURITY_SALT", dol_print_date(dol_now(), 'dayhourlog'), 'chaine', 0, '', 0); // All entities
					if (function_exists('password_hash')) {
						gestimag_set_const($db, "MAIN_SECURITY_HASH_ALGO", 'password_hash', 'chaine', 0, '', 0); // All entities
					} else {
						gestimag_set_const($db, "MAIN_SECURITY_HASH_ALGO", 'sha1md5', 'chaine', 0, '', 0); // All entities
					}
				}

				gestimag_install_syslog('step5: DATABASE_PWD_ENCRYPTED = ' . getDolGlobalString('DATABASE_PWD_ENCRYPTED').' MAIN_SECURITY_HASH_ALGO = ' . getDolGlobalString('MAIN_SECURITY_HASH_ALGO'), LOG_INFO);
			}
			...
			if ($success) {
				...
				$db->begin();
				gestimag_install_syslog('step5: set MAIN_VERSION_FIRST_INSTALL const to '.$targetversion, LOG_DEBUG);
				$resql = $db->query("INSERT INTO ".MAIN_DB_PREFIX."const(name, value, type, visible, note, entity) values(".$db->encrypt('MAIN_VERSION_FIRST_INSTALL').", ".$db->encrypt($targetversion).", 'chaine', 0, 'Gestimag version when first install', 0)");
				if ($resql) {
				...
				gestimag_install_syslog('step5: set MAIN_VERSION_LAST_INSTALL const to '.$targetversion, LOG_DEBUG);
				...
				$resql = $db->query("INSERT INTO ".MAIN_DB_PREFIX."const(name,value,type,visible,note,entity) values(".$db->encrypt('MAIN_VERSION_LAST_INSTALL').", ".$db->encrypt($targetversion).", 'chaine', 0, 'Gestimag version when last install', 0)");
				if (!$resql) {
					dol_print_error($db, 'Error in setup program');
				}
				...
				if ($useforcedwizard) {
					gestimag_install_syslog('step5: set MAIN_REMOVE_INSTALL_WARNING const to 1', LOG_DEBUG);
				...
				if (!empty($force_install_module)) {
					if (!defined('DOL_DOCUMENT_ROOT') && !empty($gestimag_main_document_root)) {
						define('DOL_DOCUMENT_ROOT', $gestimag_main_document_root);
					}
				...
				// Search modules dirs
				$modulesdir[] = $gestimag_main_document_root.'/core/modules/';
				...
				gestimag_install_syslog('step5: remove MAIN_NOT_INSTALLED const');
				...
				gestimag_install_syslog('step5: set the default language');
		...
		if ($db->connected) {
			...
			if ($tagdatabase) {
				gestimag_install_syslog('step5: set MAIN_VERSION_LAST_UPGRADE const to value '.$targetversion);
				$resql = $db->query("DELETE FROM ".MAIN_DB_PREFIX."const WHERE ".$db->decrypt('name')." = 'MAIN_VERSION_LAST_UPGRADE'");
				if (!$resql) {
					dol_print_error($db, 'Error in setup program');
				}
				$resql = $db->query("INSERT INTO ".MAIN_DB_PREFIX."const(name, value, type, visible, note, entity) VALUES (".$db->encrypt('MAIN_VERSION_LAST_UPGRADE').", ".$db->encrypt($targetversion).", 'chaine', 0, 'Gestimag version for last upgrade', 0)");
				if (!$resql) {
					dol_print_error($db, 'Error in setup program');
				}
				$conf->global->MAIN_VERSION_LAST_UPGRADE = $targetversion;
			} else {
				gestimag_install_syslog('step5: we run an upgrade to version '.$targetversion.' but database was already upgraded to ' . getDolGlobalString('MAIN_VERSION_LAST_UPGRADE').'. We keep MAIN_VERSION_LAST_UPGRADE as it is.');

				// Force the delete of the flag that say installation is not complete
				gestimag_install_syslog('step5: remove MAIN_NOT_INSTALLED const after upgrade process (should not exists but this is a security)');
				$resql = $db->query("DELETE FROM ".MAIN_DB_PREFIX."const WHERE ".$db->decrypt('name')." = 'MAIN_NOT_INSTALLED'");
				if (!$resql) {
					dol_print_error($db, 'Error in setup program');
				}
...
gestimag_install_syslog("--- step5: Gestimag setup finished");

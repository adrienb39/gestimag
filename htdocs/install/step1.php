...
$gestimag_main_distrib = 'standard';
...
if (@file_exists($forcedfile)) {
	...
	if ($force_install_noedit) {
		$main_dir = detect_gestimag_main_document_root();
		if (!empty($argv[3])) {
		...
		if (!empty($force_install_main_data_root)) {
			...
			$main_data_dir = detect_gestimag_main_data_root($main_dir);
		}
		if (!empty($argv[4])) {
			$main_data_dir = $argv[4]; // override when executing the script in command line
		}
		$main_url = detect_gestimag_main_url_root();
		if (!empty($argv[5])) {
	...
	if (!empty($force_install_distrib)) {
		$gestimag_main_distrib = $force_install_distrib;
	}
}
...
gestimag_install_syslog("--- step1: entering step1.php page");

pHeader($langs->trans("GestimagSetup").' - '.$langs->trans("ConfigurationFile"), "step2");
...
if (!$error && $db->connected && $action == "set") {
	...
	if (is_array($_POST)) {
		foreach ($_POST as $key => $value) {
			if (!preg_match('/^db_pass/i', $key)) {
				gestimag_install_syslog("step1: choice for ".$key." = ".$value);
			}
		}
	}
	...
	if (!$error) {
		gestimag_install_syslog("step1: directory '".$main_dir."' exists");
	}
	...
	if (!$error) {
		...
		if (!is_dir($main_data_dir)) {
			...
			if (!file_exists($pathhtaccess)) {
				gestimag_install_syslog("step1: .htaccess file did not exist, we created it in '".$main_data_dir."'");
				...
				if ($handlehtaccess) {
					...
					gestimag_install_syslog("step1: .htaccess file created");
				}
			}
				...
				if ($db->connected) {
					...
					if (empty($gestimag_main_db_pass)) {
						...
						if ($databasefortest == 'mysql') {
							if (!in_array($gestimag_main_db_host, array('127.0.0.1', '::1', 'localhost', 'localhost.local'))) {
								$resultbis = $db->DDLCreateUser('%', $gestimag_main_db_user, $gestimag_main_db_pass, $gestimag_main_db_name);
							}
						}
		...
		if (!$error && (isset($db_create_database) && ($db_create_database == "1" || $db_create_database == "on"))) {
			...
			if ($newdb->connected) {
				...
				if ($result) {
					...
					gestimag_install_syslog('step1: new database is using charset='.$check1.' collation='.$check2);

					// If values differs, we save conf file again
					//if ($check1 != $gestimag_main_db_character_set) gestimag_install_syslog('step1: value for character_set is not the one asked for database creation', LOG_WARNING);
...
gestimag_install_syslog("--- step1: end");
...
function write_conf_file($conffile)
{
	...
	global $gestimag_main_url_root, $gestimag_main_document_root, $gestimag_main_data_root, $gestimag_main_db_host;
	global $gestimag_main_db_port, $gestimag_main_db_name, $gestimag_main_db_user, $gestimag_main_db_pass;
	global $gestimag_main_db_type, $gestimag_main_db_character_set, $gestimag_main_db_collation, $gestimag_main_authentication;
	global $gestimag_main_distrib;
	...
	global $force_gestimag_lib_NUSOAP_PATH;
	global $force_gestimag_lib_FPDF_PATH, $force_gestimag_lib_TCPDF_PATH, $force_gestimag_lib_FPDI_PATH;
	global $force_gestimag_lib_GEOIP_PATH;
	global $force_gestimag_lib_ODTPHP_PATH, $force_gestimag_lib_ODTPHP_PATHTOPCLZIP;
	global $force_gestimag_js_CKEDITOR, $force_gestimag_js_JQUERY, $force_gestimag_js_JQUERY_UI;
	global $force_gestimag_font_DOL_DEFAULT_TTF, $force_gestimag_font_DOL_DEFAULT_TTF_BOLD;
	...
	if ($fp) {
		...
		fwrite($fp, '<?php'."\n");
		fwrite($fp, '//'."\n");
		fwrite($fp, '// File generated by Gestimag installer '.DOL_VERSION.' on '.dol_print_date(dol_now(), '')."\n");
		fwrite($fp, '//'."\n");
		...
		fwrite($fp, '$gestimag_main_url_root=\''.dol_escape_php(trim($main_url), 1).'\';');
		fwrite($fp, "\n");

		fwrite($fp, '$gestimag_main_document_root="'.dol_escape_php(dol_sanitizePathName(trim($main_dir))).'";');
		fwrite($fp, "\n");

		fwrite($fp, $main_use_alt_dir.'$gestimag_main_url_root_alt=\''.dol_escape_php(trim("/".$main_alt_dir_name), 1).'\';');
		fwrite($fp, "\n");

		fwrite($fp, $main_use_alt_dir.'$gestimag_main_document_root_alt="'.dol_escape_php(dol_sanitizePathName(trim($main_dir."/".$main_alt_dir_name))).'";');
		fwrite($fp, "\n");

		fwrite($fp, '$gestimag_main_data_root="'.dol_escape_php(dol_sanitizePathName(trim($main_data_dir))).'";');
		fwrite($fp, "\n");

		fwrite($fp, '$gestimag_main_db_host=\''.dol_escape_php(trim($db_host), 1).'\';');
		fwrite($fp, "\n");

		fwrite($fp, '$gestimag_main_db_port=\''.((int) $db_port).'\';');
		fwrite($fp, "\n");

		fwrite($fp, '$gestimag_main_db_name=\''.dol_escape_php(trim($db_name), 1).'\';');
		fwrite($fp, "\n");

		fwrite($fp, '$gestimag_main_db_prefix=\''.dol_escape_php(trim($main_db_prefix), 1).'\';');
		fwrite($fp, "\n");

		fwrite($fp, '$gestimag_main_db_user=\''.dol_escape_php(trim($db_user), 1).'\';');
		fwrite($fp, "\n");
		fwrite($fp, '$gestimag_main_db_pass=\''.dol_escape_php(trim($db_pass), 1).'\';');
		fwrite($fp, "\n");

		fwrite($fp, '$gestimag_main_db_type=\''.dol_escape_php(trim($db_type), 1).'\';');
		fwrite($fp, "\n");

		fwrite($fp, '$gestimag_main_db_character_set=\''.dol_escape_php(trim($db_character_set), 1).'\';');
		fwrite($fp, "\n");

		fwrite($fp, '$gestimag_main_db_collation=\''.dol_escape_php(trim($db_collation), 1).'\';');
		fwrite($fp, "\n");
		...
		fwrite($fp, '$gestimag_main_authentication=\'gestimag\';');
		fwrite($fp, "\n\n");

		fwrite($fp, '//$gestimag_main_demo=\'autologin,autopass\';');
		fwrite($fp, "\n");

		fwrite($fp, '// Security settings');
		fwrite($fp, "\n");

		fwrite($fp, '$gestimag_main_prod=\'0\';');
		fwrite($fp, "\n");

		fwrite($fp, '$gestimag_main_force_https=\''.dol_escape_php($main_force_https, 1).'\';');
		fwrite($fp, "\n");

		fwrite($fp, '$gestimag_main_restrict_os_commands=\'mariadb-dump, mariadb, mysqldump, mysql, pg_dump, pg_restore, clamdscan, clamdscan.exe\';');
		fwrite($fp, "\n");

		fwrite($fp, '$gestimag_nocsrfcheck=\'0\';');
		fwrite($fp, "\n");

		fwrite($fp, '$gestimag_main_instance_unique_id=\''.dol_escape_php($key, 1).'\';');
		fwrite($fp, "\n");

		fwrite($fp, '$gestimag_mailing_limit_sendbyweb=\'0\';');
		fwrite($fp, "\n");
		fwrite($fp, '$gestimag_mailing_limit_sendbycli=\'0\';');
		fwrite($fp, "\n");

		// Write params to overwrites default lib path
		fwrite($fp, "\n");
		if (empty($force_gestimag_lib_FPDF_PATH)) {
			fwrite($fp, '//');
			$force_gestimag_lib_FPDF_PATH = '';
		}
		fwrite($fp, '$gestimag_lib_FPDF_PATH="'.dol_escape_php(dol_sanitizePathName($force_gestimag_lib_FPDF_PATH)).'";');
		fwrite($fp, "\n");
		if (empty($force_gestimag_lib_TCPDF_PATH)) {
			fwrite($fp, '//');
			$force_gestimag_lib_TCPDF_PATH = '';
		}
		fwrite($fp, '$gestimag_lib_TCPDF_PATH="'.dol_escape_php(dol_sanitizePathName($force_gestimag_lib_TCPDF_PATH)).'";');
		fwrite($fp, "\n");
		if (empty($force_gestimag_lib_FPDI_PATH)) {
			fwrite($fp, '//');
			$force_gestimag_lib_FPDI_PATH = '';
		}
		fwrite($fp, '$gestimag_lib_FPDI_PATH="'.dol_escape_php(dol_sanitizePathName($force_gestimag_lib_FPDI_PATH)).'";');
		fwrite($fp, "\n");
		if (empty($force_gestimag_lib_TCPDI_PATH)) {
			fwrite($fp, '//');
			$force_gestimag_lib_TCPDI_PATH = '';
		}
		fwrite($fp, '$gestimag_lib_TCPDI_PATH="'.dol_escape_php(dol_sanitizePathName($force_gestimag_lib_TCPDI_PATH)).'";');
		fwrite($fp, "\n");
		if (empty($force_gestimag_lib_GEOIP_PATH)) {
			fwrite($fp, '//');
			$force_gestimag_lib_GEOIP_PATH = '';
		}
		fwrite($fp, '$gestimag_lib_GEOIP_PATH="'.dol_escape_php(dol_sanitizePathName($force_gestimag_lib_GEOIP_PATH)).'";');
		fwrite($fp, "\n");
		if (empty($force_gestimag_lib_NUSOAP_PATH)) {
			fwrite($fp, '//');
			$force_gestimag_lib_NUSOAP_PATH = '';
		}
		fwrite($fp, '$gestimag_lib_NUSOAP_PATH="'.dol_escape_php(dol_sanitizePathName($force_gestimag_lib_NUSOAP_PATH)).'";');
		fwrite($fp, "\n");
		if (empty($force_gestimag_lib_ODTPHP_PATH)) {
			fwrite($fp, '//');
			$force_gestimag_lib_ODTPHP_PATH = '';
		}
		fwrite($fp, '$gestimag_lib_ODTPHP_PATH="'.dol_escape_php(dol_sanitizePathName($force_gestimag_lib_ODTPHP_PATH)).'";');
		fwrite($fp, "\n");
		if (empty($force_gestimag_lib_ODTPHP_PATHTOPCLZIP)) {
			fwrite($fp, '//');
			$force_gestimag_lib_ODTPHP_PATHTOPCLZIP = '';
		}
		fwrite($fp, '$gestimag_lib_ODTPHP_PATHTOPCLZIP="'.dol_escape_php(dol_sanitizePathName($force_gestimag_lib_ODTPHP_PATHTOPCLZIP)).'";');
		fwrite($fp, "\n");
		if (empty($force_gestimag_js_CKEDITOR)) {
			fwrite($fp, '//');
			$force_gestimag_js_CKEDITOR = '';
		}
		fwrite($fp, '$gestimag_js_CKEDITOR=\''.dol_escape_php($force_gestimag_js_CKEDITOR, 1).'\';');
		fwrite($fp, "\n");
		if (empty($force_gestimag_js_JQUERY)) {
			fwrite($fp, '//');
			$force_gestimag_js_JQUERY = '';
		}
		fwrite($fp, '$gestimag_js_JQUERY=\''.dol_escape_php($force_gestimag_js_JQUERY, 1).'\';');
		fwrite($fp, "\n");
		if (empty($force_gestimag_js_JQUERY_UI)) {
			fwrite($fp, '//');
			$force_gestimag_js_JQUERY_UI = '';
		}
		fwrite($fp, '$gestimag_js_JQUERY_UI=\''.dol_escape_php($force_gestimag_js_JQUERY_UI, 1).'\';');
		fwrite($fp, "\n");

		// Write params to overwrites default font path
		fwrite($fp, "\n");
		if (empty($force_gestimag_font_DOL_DEFAULT_TTF)) {
			fwrite($fp, '//');
			$force_gestimag_font_DOL_DEFAULT_TTF = '';
		}
		fwrite($fp, '$gestimag_font_DOL_DEFAULT_TTF=\''.dol_escape_php($force_gestimag_font_DOL_DEFAULT_TTF, 1).'\';');
		fwrite($fp, "\n");
		if (empty($force_gestimag_font_DOL_DEFAULT_TTF_BOLD)) {
			fwrite($fp, '//');
			$force_gestimag_font_DOL_DEFAULT_TTF_BOLD = '';
		}
		fwrite($fp, '$gestimag_font_DOL_DEFAULT_TTF_BOLD=\''.dol_escape_php($force_gestimag_font_DOL_DEFAULT_TTF_BOLD, 1).'\';');
		fwrite($fp, "\n");

		// Other
		fwrite($fp, '$gestimag_main_distrib=\''.dol_escape_php(trim($gestimag_main_distrib), 1).'\';');
		fwrite($fp, "\n");

...
gestimag_install_syslog("--- upgrade: entering upgrade.php page ".$versionfrom." ".$versionto);
if (!is_object($conf)) {
	gestimag_install_syslog("upgrade: conf file not initialized", LOG_ERR);
}
...
if (!GETPOST('action', 'aZ09') || preg_match('/upgrade/i', GETPOST('action', 'aZ09'))) {
	...
	if ($ok && preg_match('/mysql/', $db->type)) {
		...
		&& versioncompare($versioncommande, $versionarray) <= 0) {	// Si mysql >= 4.0
			gestimag_install_syslog("Clean database from bad named constraints");
	...
	if ($ok) {
		...
		if (count($filelist) == 0) {
			...
			foreach ($filelist as $file) {
				...
				// Scan if there is migration scripts that depends of Gestimag version
				// for modules htdocs/module/sql or htdocs/custom/module/sql (files called "gestimag_x.y.z-a.b.c.sql" or "gestimag_always.sql")
				$modulesfile = array();
					...
					if (is_resource($handlemodule)) {
						while (($filemodule = readdir($handlemodule)) !== false) {
							if (!preg_match('/\./', $filemodule) && is_dir($dirroot.'/'.$filemodule.'/sql')) {	// We exclude filemodule that contains . (are not directories) and are not directories.
								...
								if (is_file($dirroot.'/'.$filemodule.'/sql/gestimag_allversions.sql')) {
									$modulesfile[$dirroot.'/'.$filemodule.'/sql/gestimag_allversions.sql'] = '/'.$filemodule.'/sql/gestimag_allversions.sql';
								}
							}
						}
...
gestimag_install_syslog("--- upgrade: end ".((int) (!$ok && !GETPOST("ignoreerrors")))." dirmodule=".$dirmodule);

...
gestimag_install_syslog("- check: Gestimag install/upgrade process started");
...
if (is_readable($conffile) && filesize($conffile) > 8) {
	gestimag_install_syslog("check: conf file '".$conffile."' already defined");
	$confexists = 1;
	...
	// If not, we create it
	gestimag_install_syslog("check: we try to create conf file '".$conffile."'");
	$confexists = 0;

	// First we try by copying example
	if (@copy($conffile.".example", $conffile)) {
		// Success
		gestimag_install_syslog("check: successfully copied file ".$conffile.".example into ".$conffile);
	} else {
		// If failed, we try to create an empty file
		gestimag_install_syslog("check: failed to copy file ".$conffile.".example into ".$conffile.". We try to create it.", LOG_WARNING);
		...
		if ($fp) {
			...
			gestimag_install_syslog("check: failed to create a new file ".$conffile." into current dir ".getcwd().". Please check permissions.", LOG_ERR);
		}
	}
...
if (!file_exists($conffile)) {
	...
	if ($checksok) {
				...
				if (!file_exists($gestimag_main_document_root."/core/lib/admin.lib.php")) {
					...
					if (preg_match('/crypted:/i', $gestimag_main_db_pass) || !empty($gestimag_main_db_encrypted_pass)) {
						...
						if (preg_match('/crypted:/i', $gestimag_main_db_pass)) {
							$gestimag_main_db_encrypted_pass = preg_replace('/crypted:/i', '', $gestimag_main_db_pass); // We need to set this as it is used to know the password was initially encrypted
		...
		if ($ok) {
			...
			$gestimagversiontoinstallarray = versiongestimagarray();
		}
		...
		gestimag_install_syslog("Scan sql files for migration files in ".$dir);
		...
		foreach ($migrationscript as $migarray) {
			...
			$gestimagversionfromarray = preg_split('/[\.-]/', $versionfrom);
			$gestimagversiontoarray = preg_split('/[\.-]/', $versionto);
			...
			$newversionfrombis = '';
			if (versioncompare($gestimagversiontoarray, $versionarray) < -2) {	// From x.y.z -> x.y.z+1
				$newversionfrombis = ' '.$langs->trans("or").' '.$versionto;
			}

			if ($ok) {
				if (count($gestimaglastupgradeversionarray) >= 2) {	// If database access is available and last upgrade version is known
					// Now we check if this is the first qualified choice
					if ($allowupgrade && empty($foundrecommandedchoice) &&
						(versioncompare($gestimagversiontoarray, $gestimaglastupgradeversionarray) > 0 || versioncompare($gestimagversiontoarray, $versionarray) < -2)
					) {
						$foundrecommandedchoice = 1; // To show only once
...
gestimag_install_syslog("- check: end");

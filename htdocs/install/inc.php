...
if (!defined('DONOTLOADCONF') && file_exists($conffile) && filesize($conffile) > 8) { // Test on filesize is to ensure that conf file is more that an empty template with just <?php in first line
	...
	if ($result) {
		if (empty($gestimag_main_db_type)) {
			$gestimag_main_db_type = 'mysqli'; // For backward compatibility
		}
		...
		if (!isset($gestimag_main_db_port) && ($gestimag_main_db_type == 'mysqli')) {
			$gestimag_main_db_port = '3306'; // For backward compatibility
		}

		// Clean parameters
		$gestimag_main_data_root = isset($gestimag_main_data_root) ? trim($gestimag_main_data_root) : DOL_DOCUMENT_ROOT.'/../documents';
		$gestimag_main_url_root         = isset($gestimag_main_url_root) ? trim($gestimag_main_url_root) : '';
		$gestimag_main_url_root_alt     = isset($gestimag_main_url_root_alt) ? trim($gestimag_main_url_root_alt) : '';
		$gestimag_main_document_root    = isset($gestimag_main_document_root) ? trim($gestimag_main_document_root) : '';
		$gestimag_main_document_root_alt = isset($gestimag_main_document_root_alt) ? trim($gestimag_main_document_root_alt) : '';

		// Remove last / or \ on directories or url value
		if (!empty($gestimag_main_document_root) && !preg_match('/^[\\/]+$/', $gestimag_main_document_root)) {
			$gestimag_main_document_root = preg_replace('/[\\/]+$/', '', $gestimag_main_document_root);
		}
		if (!empty($gestimag_main_url_root) && !preg_match('/^[\\/]+$/', $gestimag_main_url_root)) {
			$gestimag_main_url_root = preg_replace('/[\\/]+$/', '', $gestimag_main_url_root);
		}
		if (!empty($gestimag_main_data_root) && !preg_match('/^[\\/]+$/', $gestimag_main_data_root)) {
			$gestimag_main_data_root = preg_replace('/[\\/]+$/', '', $gestimag_main_data_root);
		}
		if (!empty($gestimag_main_document_root_alt) && !preg_match('/^[\\/]+$/', $gestimag_main_document_root_alt)) {
			$gestimag_main_document_root_alt = preg_replace('/[\\/]+$/', '', $gestimag_main_document_root_alt);
		}
		if (!empty($gestimag_main_url_root_alt) && !preg_match('/^[\\/]+$/', $gestimag_main_url_root_alt)) {
			$gestimag_main_url_root_alt = preg_replace('/[\\/]+$/', '', $gestimag_main_url_root_alt);
		}

		// Create conf object
		if (!empty($gestimag_main_document_root)) {
			$result = conf($gestimag_main_document_root);
		}
		// Load database driver
		if ($result) {
			if (!empty($gestimag_main_document_root) && !empty($gestimag_main_db_type)) {
				$result = include_once $gestimag_main_document_root."/core/db/".$gestimag_main_db_type.'.class.php';
				if (!$result) {
					$includeconferror = 'ErrorBadValueForGestimagMainDBType';
				}
			}
		} else {
			$includeconferror = 'ErrorBadValueForGestimagMainDocumentRoot';
		}
	} else {
...
// Define prefix
if (!isset($gestimag_main_db_prefix) || !$gestimag_main_db_prefix) {
	$gestimag_main_db_prefix = 'llx_';
}
define('MAIN_DB_PREFIX', (isset($gestimag_main_db_prefix) ? $gestimag_main_db_prefix : ''));

define('DOL_CLASS_PATH', 'class/'); // Filesystem path to class dir
define('DOL_DATA_ROOT', (isset($gestimag_main_data_root) ? $gestimag_main_data_root : DOL_DOCUMENT_ROOT.'/../documents'));
define('DOL_MAIN_URL_ROOT', (isset($gestimag_main_url_root) ? $gestimag_main_url_root : '')); // URL relative root
...
if (empty($conf->db->gestimag_main_db_collation)) {
	$conf->db->gestimag_main_db_collation = 'utf8_unicode_ci';
}
if (empty($conf->db->gestimag_main_db_encryption)) {
	$conf->db->gestimag_main_db_encryption = 0;
}
if (empty($conf->db->gestimag_main_db_cryptkey)) {
	$conf->db->gestimag_main_db_cryptkey = '';
}
...
if (!empty($gestimag_main_document_root_alt)) {
	// gestimag_main_document_root_alt contains several directories
	$values = preg_split('/[;,]/', $gestimag_main_document_root_alt);
...
if (!defined('SYSLOG_FILE')) {	// To avoid warning on systems with constant already defined
	if (@is_writable('/tmp')) {
		define('SYSLOG_FILE', '/tmp/gestimag_install.log');
	} elseif (!empty($_ENV["TMP"]) && @is_writable($_ENV["TMP"])) {
		define('SYSLOG_FILE', $_ENV["TMP"].'/gestimag_install.log');
	} elseif (!empty($_ENV["TEMP"]) && @is_writable($_ENV["TEMP"])) {
		define('SYSLOG_FILE', $_ENV["TEMP"].'/gestimag_install.log');
	} elseif (@is_writable('../../../../') && @file_exists('../../../../startdoliwamp.bat')) {
		define('SYSLOG_FILE', '../../../../gestimag_install.log'); // For DoliWamp
	} elseif (@is_writable('../../')) {
 ...
 *
 * @param	string		$gestimag_main_document_root		Root directory of Gestimag bin files
 * @return	int												Return integer <0 if KO, >0 if OK
 */
function conf($gestimag_main_document_root)
{
	global $conf;
	global $gestimag_main_db_type;
	global $gestimag_main_db_host;
	global $gestimag_main_db_port;
	global $gestimag_main_db_name;
	global $gestimag_main_db_user;
	global $gestimag_main_db_pass;
	global $gestimag_main_db_collation;
	global $gestimag_main_db_character_set;
	global $character_set_client;
	global $gestimag_main_instance_unique_id;
	global $gestimag_main_cookie_cryptkey;

	$return = include_once $gestimag_main_document_root.'/core/class/conf.class.php';
	if (!$return) {
		return -1;
	}

	$conf = new Conf();
	$conf->db->type = trim($gestimag_main_db_type);
	$conf->db->host = trim($gestimag_main_db_host);
	$conf->db->port = trim($gestimag_main_db_port);
	$conf->db->name = trim($gestimag_main_db_name);
	$conf->db->user = trim($gestimag_main_db_user);
	$conf->db->pass = (empty($gestimag_main_db_pass) ? '' : trim($gestimag_main_db_pass));
	...
	$conf->file->instance_unique_id = empty($gestimag_main_instance_unique_id) ? (empty($gestimag_main_cookie_cryptkey) ? '' : $gestimag_main_cookie_cryptkey) : $gestimag_main_instance_unique_id;
	if (empty($gestimag_main_db_character_set)) {
		$gestimag_main_db_character_set = ($conf->db->type == 'mysqli' ? 'utf8' : '');
	}
	$conf->db->character_set = $gestimag_main_db_character_set;
	if (empty($gestimag_main_db_collation)) {
		$gestimag_main_db_collation = ($conf->db->type == 'mysqli' ? 'utf8_unicode_ci' : '');
	}
	$conf->db->gestimag_main_db_collation = $gestimag_main_db_collation;
 ...
 * @return	void
 */
function gestimag_install_syslog($message, $level = LOG_DEBUG)
{
...
/**
 * Automatically detect Gestimag's main document root
 *
 * @return string
 */
function detect_gestimag_main_document_root()
{
	...
	if ($_SERVER["SCRIPT_FILENAME"] == 'php' || preg_match('/[\\/]php$/i', $_SERVER["SCRIPT_FILENAME"]) || preg_match('/php\.exe$/i', $_SERVER["SCRIPT_FILENAME"])) {
		$gestimag_main_document_root = $_SERVER["DOCUMENT_ROOT"];

		if (!preg_match('/[\\/]gestimag[\\/]htdocs$/i', $gestimag_main_document_root)) {
			$gestimag_main_document_root .= "/gestimag/htdocs";
		}
	} else {
		// We assume /install to be under /htdocs, so we get the parent directory of the current directory
		$gestimag_main_document_root = dirname(dirname($_SERVER["SCRIPT_FILENAME"]));
	}

	return $gestimag_main_document_root;
}

/**
 * Automatically detect Gestimag's main data root
 *
 * @param string $gestimag_main_document_root Current main document root
 * @return string
 */
function detect_gestimag_main_data_root($gestimag_main_document_root)
{
	$gestimag_main_data_root = preg_replace("/\/htdocs$/", "", $gestimag_main_document_root);
	$gestimag_main_data_root .= "/documents";
	return $gestimag_main_data_root;
}

/**
 * Automatically detect Gestimag's main URL root
 *
 * @return string
 */
function detect_gestimag_main_url_root()
{
	// If defined (Ie: Apache with Linux)
	if (isset($_SERVER["SCRIPT_URI"])) {
		$gestimag_main_url_root = $_SERVER["SCRIPT_URI"];
	} elseif (isset($_SERVER["SERVER_URL"]) && isset($_SERVER["DOCUMENT_URI"])) {
		// If defined (Ie: Apache with Caudium)
		$gestimag_main_url_root = $_SERVER["SERVER_URL"].$_SERVER["DOCUMENT_URI"];
	} else {
		...
		$gestimag_main_url_root = $proto."://".$serverport.$_SERVER["SCRIPT_NAME"];
	}
	// Clean proposed URL
	// We assume /install to be under /htdocs, so we get the parent path of the current URL
	$gestimag_main_url_root = dirname(dirname($gestimag_main_url_root));

	return $gestimag_main_url_root;
}

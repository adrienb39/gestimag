 ...
 *       \ingroup    install
 *       \brief      Ask all information required to build Gestimag htdocs/conf/conf.php file (will be written to disk on next page step1)
 */
...
gestimag_install_syslog("- fileconf: entering fileconf.php page");

// You can force preselected values of the config step of Gestimag by adding a file
...
if ($conffile == "/etc/gestimag/conf.php") {
	$forcedfile = "/etc/gestimag/install.forced.php"; // Must be after inc.php
}
...
pHeader($langs->trans("GestimagSetup").' - '.$langs->trans("ConfigurationFile"), "step1", "set", "", (empty($force_gestimag_js_JQUERY) ? '' : $force_gestimag_js_JQUERY.'/'), 'main-inside-bis');
...
?>
		...
		<ul>
			...
			<li>C:/wwwroot/gestimag/htdocs</li>
		</ul>
		</td>
	</tr>
	...
	<tr>
		...
		if (empty($gestimag_main_data_root)) {
			$gestimag_main_data_root = GETPOSTISSET('main_data_dir') ? GETPOST('main_data_dir') : detect_gestimag_main_data_root($gestimag_main_document_root);
		}
		?>
		<td class="label">
			<input type="text"
...
<?php if (!empty($force_install_noedit)) {
	print ' disabled';
		...
		<ul>
			...
			<li>C:/My Documents/gestimag/documents</li>
		</ul>
		</td>
	</tr>

	<!-- Root URL $gestimag_main_url_root -->
	<?php
	if (empty($gestimag_main_url_root)) {
		$gestimag_main_url_root = GETPOSTISSET('main_url') ? GETPOST('main_url') : detect_gestimag_main_url_root();
	}
	?>
	<tr>
...
?>
		...
		<ul>
			...
			<li>https://www.myvirtualforgestimag.com/</li>
		</ul>
		</td>
	</tr>
	...
	<?php
	if (!isset($gestimag_main_db_host)) {
		$gestimag_main_db_host = "localhost";
	}
	?>
	<tr>
		<!-- Driver type -->
		...
		$defaultype = !empty($gestimag_main_db_type) ? $gestimag_main_db_type : (empty($force_install_type) ? 'mysqli' : $force_install_type);

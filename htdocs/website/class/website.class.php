...
class Website extends CommonObject
{
	...
	public function createFromClone($user, $fromid, $newref, $newlang = '')
	{
		...
		$pathofwebsiteold = $gestimag_main_data_root.($conf->entity > 1 ? '/'.$conf->entity : '').'/website/'.dol_sanitizeFileName($oldref);
		$pathofwebsitenew = $gestimag_main_data_root.($conf->entity > 1 ? '/'.$conf->entity : '').'/website/'.dol_sanitizeFileName($newref);
	...
	public function exportWebSite()
	{
		...
		$line = '-- File generated by Gestimag '.DOL_VERSION.' --;'."\n";
		$line .= "\n";
		fwrite($fp, $line);
	...
	protected function showDifferences($str1, $str2, $exceptNumPge = array())
	{
		...
		if (empty($linesShouldChange)) {
			$linesShouldChange[0] = '<meta name="gestimag:pageid" content="'.$exceptNumPge[0].'" />';

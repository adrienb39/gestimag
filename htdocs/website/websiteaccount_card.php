...
if ($object->id > 0) {
	if ($object->site == 'gestimag_website') {
	...
	} elseif ($object->site == 'gestimag_portal') {
...
if (!empty($action) && $action != 'view') {
	if (!empty($object->fields['site']['arrayofkeyval'])) {
		if (isset($object->fields['site']['arrayofkeyval']['gestimag_website'])) {
			if ($action == 'delete' || $action == 'confirm_delete') {
				if (!$user->hasRight('website', 'delete')) {
					unset($object->fields['site']['arrayofkeyval']['gestimag_website']);
				}
			} else {
		...
		if (isset($object->fields['site']['arrayofkeyval']['gestimag_portal'])) {
			if (!$user->hasRight('webportal', 'write')) {
				unset($object->fields['site']['arrayofkeyval']['gestimag_portal']);
			}
		}
	}
	if (empty($object->fields['site']['arrayofkeyval'])) {
...
if ($action == 'create' || $action == 'edit') {
	if (!empty($object->fields['site']['visible']) && !empty($object->fields['fk_website']['visible'])) {
		...
		$site_type_js .= '		if (site_type == "gestimag_website") {';

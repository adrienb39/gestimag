 ...
 */
class Zapier extends GestimagApi
{
	/**
	...
	{
		if (!GestimagApiAccess::$user->hasRight('zapier', 'read')) {
		...
		if (!GestimagApi::_checkAccessToResource('hook', $this->hook->id)) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('zapier', 'write')) {
			throw new RestException(403);
		}
		...
		$this->hook->fk_user = GestimagApiAccess::$user->id;
		// we create the hook into database
		if (!$this->hook->create(GestimagApiAccess::$user)) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('zapier', 'delete')) {
			throw new RestException(403);
		}
		...
		if (!$this->hook->delete(GestimagApiAccess::$user)) {

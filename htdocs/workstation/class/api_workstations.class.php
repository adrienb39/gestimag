 ...
 */
class Workstations extends GestimagApi
{
	/**
	...
	public function index($sortfield = "t.ref", $sortorder = 'ASC', $limit = 100, $page = 0, $sqlfilters = '', $properties = '')
	{
		...
		if (!GestimagApiAccess::$user->rights->workstation->workstation->read) {
	...
	private function _fetch($id, $ref = '')
	{
		...
		if (!GestimagApi::_checkAccessToResource('workstation', $this->workstation->id)) {
			throw new RestException(401, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}

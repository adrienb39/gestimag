 ...
 */
class Tickets extends GestimagApi
{
	/**
	...
	{
		if (!GestimagApiAccess::$user->hasRight('ticket', 'read')) {
		...
		if (!GestimagApi::_checkAccessToResource('ticket', $this->ticket->id)) {
	...
	public function index($socid = 0, $sortfield = "t.rowid", $sortorder = "ASC", $limit = 100, $page = 0, $sqlfilters = '', $properties = '')
	{
		...
		$socid = GestimagApiAccess::$user->socid ? GestimagApiAccess::$user->socid : $socid;
	...
	public function post($request_data = null)
	{
		...
		if (!GestimagApiAccess::$user->hasRight('ticket', 'write')) {
			throw new RestException(403);
		}
		...
		if ($this->ticket->create(GestimagApiAccess::$user) < 0) {
	...
	public function postNewMessage($request_data = null)
	{
		...
		if (!$this->ticket->createTicketMessage(GestimagApiAccess::$user)) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->ticket->update(GestimagApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('ticket', 'delete')) {
			throw new RestException(403);
		}
		$result = $this->ticket->fetch($id);
		if (!$result) {
		...
		if (!$this->ticket->delete(GestimagApiAccess::$user)) {

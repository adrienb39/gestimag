 ...
 */
class Boms extends GestimagApi
{
	/**
	...
	{
		if (!GestimagApiAccess::$user->hasRight('bom', 'read')) {
		...
		if (!GestimagApi::_checkAccessToResource('bom', $this->bom->id, 'bom_bom')) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('bom', 'write')) {
			throw new RestException(403);
		}
		...
		if (!$this->bom->create(GestimagApiAccess::$user)) {
	...
	public function put($id, $request_data = null)
	{
		...
		if ($this->bom->update(GestimagApiAccess::$user) > 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('bom', 'delete')) {
			throw new RestException(403);
		}
		$result = $this->bom->fetch($id);
		if (!$result) {
		...
		if (!$this->bom->delete(GestimagApiAccess::$user)) {
	...
	public function getLines($id)
	{
		...
		if (!GestimagApi::_checkAccessToResource('bom_bom', $this->bom->id)) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}
	...
	public function deleteLine($id, $lineid)
	{
		...
		$updateRes = $this->bom->deleteLine(GestimagApiAccess::$user, $lineid);
		if ($updateRes > 0) {
			return array(
				'success' => array(
					'code' => 200,

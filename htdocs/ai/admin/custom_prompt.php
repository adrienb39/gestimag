...
if ($action == 'update' && !GETPOST('cancel')) {
	...
	$result = gestimag_set_const($db, 'AI_CONFIGURATIONS_PROMPT', $newConfigurationsJson, 'chaine', 0, '', $conf->entity);
	if (!$error) {
		if ($result) {
...
if ($action == 'confirm_deleteproperty' && GETPOST('confirm') == 'yes') {
	...
	if (isset($currentConfigurations[$key])) {
		...
		$newConfigurationsJson = json_encode($currentConfigurations, JSON_UNESCAPED_UNICODE);
		$res = gestimag_set_const($db, 'AI_CONFIGURATIONS_PROMPT', $newConfigurationsJson, 'chaine', 0, '', $conf->entity);
		if ($res) {
			header("Location: ".$_SERVER['PHP_SELF']);

 ...
 */
class Members extends GestimagApi
{
	/**
	...
	public function get($id)
	{
		...
		if (!GestimagApi::_checkAccessToResource('adherent', $member->id) && $id > 0) {
	...
	public function getByThirdparty($thirdparty)
	{
		...
		if (!GestimagApi::_checkAccessToResource('adherent', $member->id)) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}
	...
	public function post($request_data = null)
	{
		...
		if ($member->create(GestimagApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if (!GestimagApi::_checkAccessToResource('member', $member->id)) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}
		...
		if ($member->update(GestimagApiAccess::$user) >= 0) {
			return $this->get($id);
		} else {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('adherent', 'supprimer')) {
			throw new RestException(403);
		}
		$member = new Adherent($this->db);
		$result = $member->fetch($id);
		...
		$res = $member->delete(GestimagApiAccess::$user);
		if ($res < 0) {
	...
	{
		if (!GestimagApiAccess::$user->hasRight('adherent', 'cotisation', 'lire')) {
			throw new RestException(403);
		}
	...
	{
		if (!GestimagApiAccess::$user->hasRight('adherent', 'cotisation', 'creer')) {
			throw new RestException(403);
		}
	...
	public function getType($id)
	{
		...
		if (!GestimagApi::_checkAccessToResource('member', $membertype->id, 'adherent_type')) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}
	...
	{
		if (!GestimagApiAccess::$user->hasRight('adherent', 'configurer')) {
			throw new RestException(403);
		}
		// Check mandatory fields
		...
		if ($membertype->create(GestimagApiAccess::$user) < 0) {
	...
	public function putType($id, $request_data = null)
	{
		...
		// If there is no error, update() returns the number of affected rows
		// so if the update is a no op, the return value is zero.
		if ($membertype->update(GestimagApiAccess::$user) >= 0) {
			return $this->get($id);
		} else {
	...
	public function deleteType($id)
	{
		...
		$res = $membertype->delete(GestimagApiAccess::$user);
		if ($res < 0) {
			throw new RestException(500, "Can't delete, error occurs");
		}

	...
	public function assign_values(&$action, $id)
	{
		...
		if ($action == 'view' || $action == 'edit' || $action == 'delete') {
			...
			if ($this->object->user_id) {
				$gestimag_user = new User($this->db);
				$result = $gestimag_user->fetch($this->object->user_id);
				$this->tpl['gestimag_user'] = $gestimag_user->getLoginUrl(1);
			} else {
				$this->tpl['gestimag_user'] = $langs->trans("NoGestimagAccess");
			}
		}
		...
		if ($action == 'create_user') {
			...
			$this->tpl['action_create_user'] = $form->formconfirm($_SERVER["PHP_SELF"]."?id=".$this->object->id, $langs->trans("CreateGestimagLogin"), $langs->trans("ConfirmCreateAdherent"), "confirm_create_user", $formquestion, 'no');
		}
	}

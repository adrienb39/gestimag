...
<tr>
	<td><?php echo  $langs->trans("GestimagLogin"); ?></td>
	<td colspan="3"><?php echo  $this->control->tpl['gestimag_user']; ?></td>
</tr>
...
if (empty($user->socid)) {
	...
	if (!$this->control->tpl['user_id'] && $user->hasRight('user', 'user', 'creer')) {
		echo  '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$this->control->tpl['id'].'&action=create_user&token='.newToken().'&canvas='.$canvas.'">'.$langs->trans("CreateGestimagLogin").'</a>';
	}

	if ($user->hasRight('adherent', 'supprimer')) {
		echo dolGetButtonAction($langs->trans("Delete"), '', 'delete', $_SERVER["PHP_SELF"].'?id='.$this->control->tpl['id'].'&action=delete&token='.newToken().'&canvas='.$canvas, 'delete', $user->hasRight('adherent', 'supprimer'));
	}

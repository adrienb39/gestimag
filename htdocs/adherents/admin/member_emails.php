...
if ($action == 'updateall') {
	...
	foreach ($constantes as $constname => $value) {
		...
		$res = gestimag_set_const($db, $constname, $constvalue, $consttype, 0, $constnote, $conf->entity);
		if ($res <= 0) {
			$error++;
			$action = 'list';
		}
...
if ($action == 'update' || $action == 'add') {
	...
	$res = gestimag_set_const($db, $constname, $constvalue, $typetouse, 0, $constnote, $conf->entity);
...
echo dol_get_fiche_head($head, 'emails', $langs->trans("Members"), -1, 'user');
...
echo '<div class="center"><input type="submit" class="button" value="'.$langs->trans("Update").'" name="update"></div>';
echo '</form>';

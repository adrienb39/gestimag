		...
		if (!$error) {
			...
			if ($object->email && $sendalsoemail) {   // $object is 'Adherent'
				...
				if (empty($reshook)) {
					...
					$moreinheader = 'X-Gestimag-Info: send_an_email by adherents/subscription.php'."\r\n";
...
echo dol_get_fiche_head($head, 'subscription', $langs->trans("Member"), -1, 'user');
...
echo '<tr><td>'.$form->textwithpicto($langs->trans("PublicFile"), $langs->trans("Public", getDolGlobalString('MAIN_INFO_SOCIETE_NOM'), $linkofpubliclist), 1, 'help', '', 0, 3, 'publicfile').'</td><td class="valeur">'.yn($object->public).'</td></tr>';
...
if (isModEnabled('societe')) {
	...
	echo $langs->trans("LinkedToGestimagThirdParty");
	echo '</td>';
	if ($action != 'editthirdparty' && $user->hasRight('adherent', 'creer')) {
		echo '<td class="right"><a class="editfielda" href="'.$_SERVER["PHP_SELF"].'?action=editthirdparty&token='.newToken().'&rowid='.$object->id.'">'.img_edit($langs->trans('SetLinkToThirdParty'), 1).'</a></td>';
	}
	echo '</tr></table>';
	...
	if ($action == 'editthirdparty') {
		...
		echo '<tr><td>';
		echo $form->select_company($object->fk_soc, 'socid', '', 1);
		echo '</td>';
		echo '<td class="left"><input type="submit" class="button button-edit" value="'.$langs->trans("Modify").'"></td>';
		echo '</tr></table></form>';
	} else {
		if ($object->fk_soc) {
			$company = new Societe($db);
...
echo '<tr><td>';
echo '<table class="nobordernopadding" width="100%"><tr><td>';
echo $langs->trans("LinkedToGestimagUser");
echo '</td>';
if ($action != 'editlogin' && $user->hasRight('adherent', 'creer')) {
	echo '<td class="right">';
	if ($user->hasRight("user", "user", "creer")) {
		echo '<a class="editfielda" href="'.$_SERVER["PHP_SELF"].'?action=editlogin&token='.newToken().'&rowid='.$object->id.'">'.img_edit($langs->trans('SetLinkToUser'), 1).'</a>';
	}
	echo '</td>';
}
echo '</tr></table>';
...
if ($user->hasRight('adherent', 'cotisation', 'creer')) {
	if ($action != 'addsubscription' && $action != 'create_thirdparty') {
		...
		if ($object->statut > 0) {
			echo '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?rowid='.$rowid.'&action=addsubscription&token='.newToken().'">'.$langs->trans("AddSubscription")."</a></div>";
		} else {
			echo '<div class="inline-block divButAction"><a class="butActionRefused classfortooltip" href="#" title="'.dol_escape_htmltag($langs->trans("ValidateBefore")).'">'.$langs->trans("AddSubscription").'</a></div>';
		}
...
if ($action != 'addsubscription' && $action != 'create_thirdparty') {
	...
	if ($result) {
		...
		while ($i < $num) {
			...
			echo '<td>'.$subscriptionstatic->getNomUrl(1).'</td>';
			echo '<td class="center">'.dol_print_date($db->jdate($objp->datec), 'dayhour')."</td>\n";
			echo '<td class="center">';
			if ($typeid > 0) {
				echo $adht->getNomUrl(1);
			}
			echo '</td>';
			echo '<td class="center">'.dol_print_date($db->jdate($objp->dateh), 'day')."</td>\n";
			echo '<td class="center">'.dol_print_date($db->jdate($objp->datef), 'day')."</td>\n";
			echo '<td class="right amount">'.price($objp->subscription).'</td>';
			if (isModEnabled('bank')) {
				echo '<td class="right">';
				if ($objp->bid) {
...
if (($action != 'addsubscription' && $action != 'create_thirdparty')) {
	...
	if ($useonlinepayment) {
		...
		echo showOnlinePaymentUrl('membersubscription', $object->ref);
		echo '<br>';
	}
}
...
if (($action == 'addsubscription' || $action == 'create_thirdparty') && $user->hasRight('adherent', 'cotisation', 'creer')) {
	...
	echo load_fiche_titre($langs->trans("NewCotisation"));
	...
	if ($conf->use_javascript_ajax) {
		...
		echo '$(document).ready(function () {
					...
					';
		if (GETPOST('paymentsave')) {
			echo '$("#'.GETPOST('paymentsave', 'aZ09').'").prop("checked", true);';
		}
		echo '});';
		echo '</script>'."\n";
	}
	...
	if ($action == 'create_thirdparty') {
		...
		echo $form->formconfirm($_SERVER["PHP_SELF"]."?rowid=".$object->id, $langs->trans("CreateGestimagThirdParty"), $langs->trans("ConfirmCreateThirdParty"), "confirm_create_thirdparty", $formquestion, 1);
	}


	echo '<form name="subscription" method="POST" action="'.$_SERVER["PHP_SELF"].'">';
	echo '<input type="hidden" name="token" value="'.newToken().'">';
	echo '<input type="hidden" name="action" value="subscription">';
	echo '<input type="hidden" name="rowid" value="'.$rowid.'">';
	echo '<input type="hidden" name="memberlabel" id="memberlabel" value="'.dol_escape_htmltag($object->getFullName($langs)).'">';
	echo '<input type="hidden" name="thirdpartylabel" id="thirdpartylabel" value="'.dol_escape_htmltag($object->company).'">';
	...
	echo '<td class="fieldrequired">'.$langs->trans("DateSubscription").'</td><td>';
	if (GETPOST('reday')) {
	...
	echo $form->selectDate($datefrom, '', 0, 0, 0, "subscription", 1, 1);
	echo "</td></tr>";
	...
	echo '<tr><td>'.$langs->trans("DateEndSubscription").'</td><td>';
	echo $form->selectDate($dateto, 'end', 0, 0, 0, "subscription", 1, 0);
	echo "</td></tr>";

	if ($adht->subscription) {
		// Amount
		echo '<tr><td class="fieldrequired">'.$langs->trans("Amount").'</td><td><input type="text" name="subscription" size="6" value="'.(GETPOSTISSET('subscription') ? GETPOST('subscription') : price($adht->amount, 0, '', 0)).'"> '.$langs->trans("Currency".$conf->currency) .'</td></tr>';

		// Label
		echo '<tr><td>'.$langs->trans("Label").'</td>';
		echo '<td><input name="label" type="text" size="32" value="';
		if (!getDolGlobalString('MEMBER_NO_DEFAULT_LABEL')) {
			echo $langs->trans("Subscription").' '.dol_print_date(($datefrom ? $datefrom : time()), "%Y");
		}
		echo '"></td></tr>';
		...
		if ((isModEnabled('bank') || isModEnabled('invoice')) && !getDolGlobalString('ADHERENT_SUBSCRIPTION_HIDECOMPLEMENTARYACTIONS')) {
			...
			// No more action
			echo '<tr><td class="tdtop fieldrequired">'.$langs->trans('MoreActions');
			echo '</td>';
			echo '<td class="line-height-large">';

			echo '<input type="radio" class="moreaction" id="none" name="paymentsave" value="none"'.(empty($bankdirect) && empty($invoiceonly) && empty($bankviainvoice) ? ' checked' : '').'>';
			echo '<label for="none"> '.$langs->trans("None").'</label><br>';
			// Add entry into bank account
			if (isModEnabled('bank')) {
				echo '<input type="radio" class="moreaction" id="bankdirect" name="paymentsave" value="bankdirect"'.(!empty($bankdirect) ? ' checked' : '');
				echo '><label for="bankdirect">  '.$langs->trans("MoreActionBankDirect").'</label><br>';
			}
			// Add invoice with no payments
			if (isModEnabled('societe') && isModEnabled('invoice')) {
				echo '<input type="radio" class="moreaction" id="invoiceonly" name="paymentsave" value="invoiceonly"'.(!empty($invoiceonly) ? ' checked' : '');
				//if (empty($object->fk_soc)) echo ' disabled';
				echo '><label for="invoiceonly"> '.$langs->trans("MoreActionInvoiceOnly");
				if ($object->fk_soc) {
					echo ' ('.$langs->trans("ThirdParty").': '.$company->getNomUrl(1).')';
				} else {
					echo ' (';
					if (empty($object->fk_soc)) {
						echo img_warning($langs->trans("NoThirdPartyAssociatedToMember"));
					}
					echo $langs->trans("NoThirdPartyAssociatedToMember");
					echo ' - <a href="'.$_SERVER["PHP_SELF"].'?rowid='.$object->id.'&amp;action=create_thirdparty">';
					echo $langs->trans("CreateGestimagThirdParty");
					echo '</a>)';
				}
				if (!getDolGlobalString('ADHERENT_VAT_FOR_SUBSCRIPTIONS') || getDolGlobalString('ADHERENT_VAT_FOR_SUBSCRIPTIONS') != 'defaultforfoundationcountry') {
					echo '. <span class="opacitymedium">'.$langs->trans("NoVatOnSubscription", 0).'</span>';
				}
				if (getDolGlobalString('ADHERENT_PRODUCT_ID_FOR_SUBSCRIPTIONS') && (isModEnabled('product') || isModEnabled('service'))) {
					...
					echo '. '.$langs->transnoentitiesnoconv("ADHERENT_PRODUCT_ID_FOR_SUBSCRIPTIONS", $prodtmp->getNomUrl(1)); // must use noentitiesnoconv to avoid to encode html into getNomUrl of product
				}
				echo '</label><br>';
			}
			// Add invoice with payments
			if (isModEnabled('bank') && isModEnabled('societe') && isModEnabled('invoice')) {
				echo '<input type="radio" class="moreaction" id="bankviainvoice" name="paymentsave" value="bankviainvoice"'.(!empty($bankviainvoice) ? ' checked' : '');
				//if (empty($object->fk_soc)) echo ' disabled';
				echo '><label for="bankviainvoice">  '.$langs->trans("MoreActionBankViaInvoice");
				if ($object->socid) {
					echo ' ('.$langs->trans("ThirdParty").': '.$company->getNomUrl(1).')';
				} else {
					echo ' (';
			...
			// Bank account
			echo '<tr class="bankswitchclass"><td class="fieldrequired">'.$langs->trans("FinancialAccount").'</td><td>';
			echo img_picto('', 'bank_account', 'class="pictofixedwidth"');
			$form->select_comptes(GETPOST('accountid'), 'accountid', 0, '', 2, '', 0, 'minwidth200');
			echo "</td></tr>\n";

			// Payment mode
			echo '<tr class="bankswitchclass"><td class="fieldrequired">'.$langs->trans("PaymentMode").'</td><td>';
			echo $form->select_types_paiements(GETPOST('operation'), 'operation', '', 2, 1, 0, 0, 1, 'minwidth200', 1);
			echo "</td></tr>\n";

			// Date of payment
			echo '<tr class="bankswitchclass"><td class="fieldrequired">'.$langs->trans("DatePayment").'</td><td>';
			echo $form->selectDate(isset($paymentdate) ? $paymentdate : -1, 'payment', 0, 0, 1, 'subscription', 1, 1);
			echo "</td></tr>\n";

			echo '<tr class="bankswitchclass2"><td>'.$langs->trans('Numero');
			echo ' <em>('.$langs->trans("ChequeOrTransferNumber").')</em>';
			echo '</td>';
			echo '<td><input id="fieldnum_chq" name="num_chq" type="text" size="8" value="'.(!GETPOST('num_chq') ? '' : GETPOST('num_chq')).'"></td></tr>';

			echo '<tr class="bankswitchclass2 fieldrequireddyn"><td>'.$langs->trans('CheckTransmitter');
			echo ' <em>('.$langs->trans("ChequeMaker").')</em>';
			echo '</td>';
			echo '<td><input id="fieldchqemetteur" name="chqemetteur" size="32" type="text" value="'.(!GETPOST('chqemetteur') ? '' : GETPOST('chqemetteur')).'"></td></tr>';

			echo '<tr class="bankswitchclass2"><td>'.$langs->trans('Bank');
			echo ' <em>('.$langs->trans("ChequeBank").')</em>';
			echo '</td>';
			echo '<td><input id="chqbank" name="chqbank" size="32" type="text" value="'.(!GETPOST('chqbank') ? '' : GETPOST('chqbank')).'"></td></tr>';
		}
	}

	echo '<tr><td></td><td></td></tr>';

	echo '<tr><td>'.$langs->trans("SendAcknowledgementByMail").'</td>';
	echo '<td>';
	if (!$object->email) {
		echo $langs->trans("NoEMail");
	} else {
		$adht = new AdherentType($db);
		$adht->fetch($object->typeid);
		...
		echo $form->textwithpicto($tmp, $helpcontent, 1, 'help', '', 0, 2, 'helpemailtosend');
	}
	echo '</td></tr>';
	echo '</tbody>';
	echo '</table>';
	...
	if (empty($reshook)) {
		echo '<input type="submit" class="button" name="add" value="'.$langs->trans("AddSubscription").'">';
		echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';

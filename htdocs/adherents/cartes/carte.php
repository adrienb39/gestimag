...
echo load_fiche_titre($langs->trans("LinkToGeneratedPages"), '', $adherentstatic->picto);

echo '<span class="opacitymedium">'.$langs->trans("LinkToGeneratedPagesDesc").'</span><br>';
echo '<br>';
...
echo img_picto('', 'card').' '.$langs->trans("DocForAllMembersCards", getDolGlobalString('ADHERENT_CARD_TYPE', $langs->transnoentitiesnoconv("None"))).' ';
echo '<form action="'.$_SERVER["PHP_SELF"].'" method="POST">';
echo '<input type="hidden" name="token" value="'.newToken().'">';
echo '<input type="hidden" name="foruserid" value="all">';
echo '<input type="hidden" name="mode" value="card">';
echo '<input type="hidden" name="action" value="builddoc">';
echo $langs->trans("DescADHERENT_CARD_TYPE").' ';
...
asort($arrayoflabels);
echo $form->selectarray('modelcard', $arrayoflabels, (GETPOST('modelcard') ? GETPOST('modelcard') : getDolGlobalString('ADHERENT_CARD_TYPE')), 1, 0, 0, '', 0, 0, 0, '', '', 1);
echo '<br><input type="submit" class="button small" value="'.$langs->trans("BuildDoc").'">';
echo '</form>';

echo '<br><br>';

echo img_picto('', 'card').' '.$langs->trans("DocForOneMemberCards", getDolGlobalString('ADHERENT_CARD_TYPE', $langs->transnoentitiesnoconv("None"))).' ';
echo '<form action="'.$_SERVER["PHP_SELF"].'" method="POST">';
echo '<input type="hidden" name="token" value="'.newToken().'">';
echo '<input type="hidden" name="mode" value="cardlogin">';
echo '<input type="hidden" name="action" value="builddoc">';
echo $langs->trans("DescADHERENT_CARD_TYPE").' ';
// List of possible labels (defined into $_Avery_Labels variable set into format_cards.lib.php)
$arrayoflabels = array();
...
asort($arrayoflabels);
echo $form->selectarray('model', $arrayoflabels, (GETPOST('model') ? GETPOST('model') : getDolGlobalString('ADHERENT_CARD_TYPE')), 1, 0, 0, '', 0, 0, 0, '', '', 1);
echo '<br>'.$langs->trans("Login").': <input class="with100" type="text" name="foruserlogin" value="'.GETPOST('foruserlogin').'">';
echo '<br><input type="submit" class="button small" value="'.$langs->trans("BuildDoc").'">';
echo '</form>';

echo '<br><br>';

echo img_picto('', 'card').' '.$langs->trans("DocForLabels", getDolGlobalString('ADHERENT_ETIQUETTE_TYPE')).' ';
echo '<form action="'.$_SERVER["PHP_SELF"].'" method="POST">';
echo '<input type="hidden" name="token" value="'.newToken().'">';
echo '<input type="hidden" name="mode" value="label">';
echo '<input type="hidden" name="action" value="builddoc">';
echo $langs->trans("DescADHERENT_ETIQUETTE_TYPE").' ';
// List of possible labels (defined into $_Avery_Labels variable set into format_cards.lib.php)
$arrayoflabels = array();
foreach (array_keys($_Avery_Labels) as $codecards) {
	$arrayoflabels[$codecards] = $_Avery_Labels[$codecards]['name'];
...
asort($arrayoflabels);
echo $form->selectarray('modellabel', $arrayoflabels, (GETPOST('modellabel') ? GETPOST('modellabel') : getDolGlobalString('ADHERENT_ETIQUETTE_TYPE')), 1, 0, 0, '', 0, 0, 0, '', '', 1);
echo '<br><input type="submit" class="button small" value="'.$langs->trans("BuildDoc").'">';
echo '</form>';

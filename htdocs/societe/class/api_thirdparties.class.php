 ...
 *
 */
class Thirdparties extends GestimagApi
{
	/**
	 *
	...
	public function index($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 100, $page = 0, $mode = 0, $category = 0, $sqlfilters = '', $properties = '')
	{
		...
		if (!GestimagApiAccess::$user->hasRight('societe', 'lire')) {
			throw new RestException(403);
		}

		// case of external user, we force socids
		$socids = GestimagApiAccess::$user->socid ? (string) GestimagApiAccess::$user->socid : '';

		// If the internal user must only see his customers, force searching by him
		$search_sale = 0;
		if (!GestimagApiAccess::$user->hasRight('societe', 'client', 'voir') && !$socids) {
			$search_sale = GestimagApiAccess::$user->id;
		}
	...
	{
		if (!GestimagApiAccess::$user->hasRight('societe', 'creer')) {
			throw new RestException(403);
		}
		...
		if ($this->company->create(GestimagApiAccess::$user) < 0) {
	...
	public function put($id, $request_data = null)
	{
		...
		if (!GestimagApi::_checkAccessToResource('societe', $this->company->id)) {
		...
		if ($this->company->update($id, GestimagApiAccess::$user, 1, '', '', 'update', 1) > 0) {
			return $this->get($id);
		} else {
	...
	public function merge($id, $idtodelete)
	{
		...
		if (!GestimagApi::_checkAccessToResource('societe', $companytoremove->id)) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}
	...
	{
		if (!GestimagApiAccess::$user->hasRight('societe', 'supprimer')) {
			throw new RestException(403);
		}
		$result = $this->company->fetch($id);
		if (!$result) {
	...
	public function setThirdpartyPriceLevel($id, $priceLevel)
	{
		...
		if (!GestimagApiAccess::$user->hasRight('societe', 'creer')) {
			throw new RestException(403, 'Access to thirdparty '.$id.' not allowed for login '.GestimagApiAccess::$user->login);
		}
		...
		if (empty(GestimagApi::_checkAccessToResource('societe', $this->company->id))) {
			throw new RestException(403, 'Access to thirdparty '.$id.' not allowed for login '.GestimagApiAccess::$user->login);
		}

		$result = $this->company->setPriceLevel($priceLevel, GestimagApiAccess::$user);
		if ($result <= 0) {
	...
	public function addRepresentative($id, $representative_id)
	{
		...
		$result = $this->company->add_commercial(GestimagApiAccess::$user, $representative_id);
	...
	public function deleteRepresentative($id, $representative_id)
	{
		...
		$result = $this->company->del_commercial(GestimagApiAccess::$user, $representative_id);
	...
	public function addCategory($id, $category_id)
	{
		...
		if (!GestimagApi::_checkAccessToResource('category', $category->id)) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}
	...
	public function getOutStandingProposals($id, $mode = 'customer')
	{
		...
		if (!GestimagApi::_checkAccessToResource('societe', $id)) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login);
		}
	...
	public function getSalesRepresentatives($id, $mode = 0)
	{
		...
		$result = $this->company->getSalesRepresentatives(GestimagApiAccess::$user, $mode);
	...
	{
		if (!GestimagApiAccess::$user->hasRight('facture', 'lire')) {
			throw new RestException(403);
		}
		if (empty($id)) {
			throw new RestException(400, 'Thirdparty ID is mandatory');
	...
	public function createCompanyNotification($id, $request_data = null)
	{
		...
		if ($notification->create(GestimagApiAccess::$user) < 0) {
			throw new RestException(500, 'Error creating Thirdparty Notification');
		}

		if ($notification->update(GestimagApiAccess::$user) < 0) {
	...
	public function deleteCompanyNotification($id, $notification_id)
	{
		...
		if ($socid == $id) {
			return $notification->delete(GestimagApiAccess::$user);
		} else {
	...
	public function createCompanyBankAccount($id, $request_data = null)
	{
		...
		if ($account->create(GestimagApiAccess::$user) < 0) {
		...
		if ($account->update(GestimagApiAccess::$user) < 0) {
			throw new RestException(500, 'Error updating values');
		}
	...
	public function deleteCompanyBankAccount($id, $bankaccount_id)
	{
		...
		if ($socid == $id) {
			return $account->delete(GestimagApiAccess::$user);
		} else {
			throw new RestException(403, "Not allowed due to bad consistency of input data");
		}
	}
	...
	public function generateBankAccountDocument($id, $companybankid = null, $model = 'sepamandate')
	{
		...
		$this->company->setDocModel(GestimagApiAccess::$user, $model);
	 ...
	 * See <a href="https://wiki.gestimag.org/index.php/Table_llx_societe_account">Table llx_societe_account</a> wiki page for more information<br><br>
	...
	public function deleteSocieteAccount($id, $site)
	{
		...
		if ($result && $this->db->num_rows($result) == 0) {
			...
			if ($account->delete(GestimagApiAccess::$user) < 0) {
	 ...
	 * @param    string	$ref_ext    External reference of third party (Warning, this information is a free field not provided by Gestimag)
	 * @param    string	$barcode    Barcode of third party to load
	...
	private function _fetch($rowid, $ref = '', $ref_ext = '', $barcode = '', $idprof1 = '', $idprof2 = '', $idprof3 = '', $idprof4 = '', $idprof5 = '', $idprof6 = '', $email = '', $ref_alias = '')
	{
		...
		if (!GestimagApiAccess::$user->hasRight('societe', 'lire')) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login.'. No read permission on thirdparties.');
		}
		...
		if (!GestimagApi::_checkAccessToResource('societe', $this->company->id)) {
			throw new RestException(403, 'Access not allowed for login '.GestimagApiAccess::$user->login.' on this thirdparty');
		}
		if (isModEnabled('mailing')) {

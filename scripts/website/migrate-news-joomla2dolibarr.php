...
/**
 * \file scripts/website/migrate_news_joomla2gestimag.php
 * \ingroup scripts
 * \brief Migrate news from a Joomla database into a Gestimag website
 */
...
define('EVEN_IF_ONLY_LOGIN_ALLOWED', 1); // Set this define to 0 if you want to lock your script when gestimag setup is "locked to admin user only".
...
$image = 'image/__WEBSITE_KEY__/images/stories/gestimag.png';
...
if (empty($argv[3]) || !in_array($argv[1], array('test', 'confirm')) || empty($websiteref)) {
	...
	print "\n";
	print "Load joomla news and create them into Gestimag database (if they don't already exist).\n";
	exit(1);
}
	...
	if ($obj) {
		...
		if ($result <= 0) {
			...
			if ($pageid > 0) {	// We must also regenerate page on disk
				...
				$pathofwebsite = $gestimag_main_data_root.'/website/'.$websiteref;

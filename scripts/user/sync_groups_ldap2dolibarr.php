...
/**
 * \file scripts/user/sync_groups_ldap2gestimag.php
 * \ingroup ldap member
 * \brief Script to update groups into Gestimag from LDAP
 */
...
if (!getDolGlobalString('LDAP_GROUP_DN')) {
	print $langs->trans("Error").': '.$langs->trans("LDAP setup for groups not defined inside Gestimag");
	exit(1);
}
...
if ($result >= 0) {
	...
	// We disable synchro Gestimag-LDAP
	...
	if (is_array($ldaprecords)) {
		...
		foreach ($ldaprecords as $key => $ldapgroup) {
			...
			// 1 - Association of users in the LDAP group with the Gestimag group
			$userList = array();
			$userIdList = array();
			...
			// 2 - Delete users from the Gestimag group that are no longer in the LDAP group
